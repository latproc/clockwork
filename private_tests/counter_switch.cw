...
# this is probably superseded by the version in git: ../plugins/
...
CounterSwitch MACHINE Count {
  ...
  COMMAND mark { Mark := SetPoint + Count.VALUE; SET SELF TO waiting; }
...
Counter MACHINE {
	OPTION Count 0;
	up WHEN TIMER>1000; ENTER up { INC Count; LOG Count; }
	waiting DEFAULT;
}
...
int CheckStates(PluginScope *scope)
{
	...
	if (!data) { 
		...
		Value &count = scope->getValue("Mark");
