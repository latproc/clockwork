...
template <class T> void display(T *p, size_t len) {
    ...
    for (size_t i = min; i < len; ++i)
        std::cout << std::setw(2 * sizeof(T)) << std::setfill('0') << std::hex << (unsigned int)p[i]
                  << std::dec;
}
...
class ModbusValueBit : public ModbusValue {
  public:
    ...
    ModbusValueBit(unsigned int len, const std::string &format) : ModbusValue(len, format), val(0) {
        val = new uint8_t[len];
    }
...
class ModbusMonitor {
  public:
    ModbusMonitor(std::string name, unsigned int group, unsigned int address, unsigned int len,
                  const std::string &format, bool read_only = true);
