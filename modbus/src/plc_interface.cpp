...
std::ostream &PLCMapping::operator<<(std::ostream &out) const {
    return out << code_ << ": " << start_ << " " << end_ << " " << group_ << ":" << std::setw(4)
               << std::setfill('0') << address_;
}

std::ostream &operator<<(std::ostream &out, const PLCMapping &m) { return m.operator<<(out); }

PLCMapping::PLCMapping(const char *code, unsigned int start, unsigned int end, unsigned int group,
                       unsigned int address)
    : code_(code), start_(start), end_(end), group_(group), address_(address) {}

PLCMapping::PLCMapping(const PLCMapping &other)
    : code_(other.code_), start_(other.start_), end_(other.end_), group_(other.group_),
...
std::pair<int, int> PLCInterface::decode(const char *address) {
    try {
        ...
        if (errno == -1) {
            std::cerr << "argument error: " << address << " is invalid\n";
        }
        else {
...
unsigned int fromOctal(const char *s) {
    ...
    x = strtol(s, &p, 8);
    if (errno != 0)
        std::cerr << "strtol conversion error: " << strerror(errno) << "\n";
    if (*p)
        std::cerr << "warning data left over after number conversion: " << p << "\n";
    return x;
}
