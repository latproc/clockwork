...
template <class T> class BufferMonitor {
  public:
    ...
    BufferMonitor(const char *buffer_name, const Options &global_options)
        : name(buffer_name), last_data(0), cmp_data(0), dbg_mask(0), buflen(0), max_read_len(0),
    ...
    void check(size_t size, T *upd_data, unsigned int base_address,
        ...
        if (size) {
            ...
            for (size_t ii = 0; ii < size; ++ii) {
                ...
                if (*q != *p) {
                    ...
                    if (mm) {
                        changes.insert(mm);
                    } //if (options.verbose) std::cout << "found change " << mm->name() << "\n"; }
                }
                *q++ = *p++; // & *msk++;
            }
