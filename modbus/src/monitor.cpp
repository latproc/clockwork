...
ModbusValue::ModbusValue(unsigned int len, const std::string &format)
    : length(len), format_(format) {}
...
ModbusValueWord::ModbusValueWord(unsigned int len, const std::string &form)
    : ModbusValue(len, form), val(0) {
    val = new uint16_t[len];
}

std::ostream &ModbusMonitor::operator<<(std::ostream &out) const {
    return out << name_ << " " << group_ << ":" << std::setw(4) << std::setfill('0') << address_
               << " " << len_ << " value-length: " << value->length;
}

std::ostream &operator<<(std::ostream &out, const ModbusMonitor &m) { return m.operator<<(out); }

ModbusMonitor::ModbusMonitor(std::string name, unsigned int group, unsigned int address,
                             unsigned int len, const std::string &format, bool readonly)
    : name_(name), group_(group), address_(address), len_(len), format_(format), value(0),
      read_only(readonly) {
...
ModbusMonitor::ModbusMonitor(const ModbusMonitor &other)
    : name_(other.name_), group_(other.group_), address_(other.address_), len_(other.len_),
      format_(other.format_), value(0), read_only(other.read_only) {
    if (group_ == 0 || group_ == 1) {
        value = new ModbusValueBit(len_);
...
ModbusMonitor::~ModbusMonitor() { delete value; }
...
void MonitorConfiguration::createSimulator(const char *filename) {
    ...
    if (simfile.good()) {
        ...
        while (iter != monitors.end()) {
            ...
            simfile << mm.name();
            if (mm.group() == 1)
                simfile << " FLAG(export:ro);";
            else if (mm.group() == 0)
                simfile << " FLAG(export:rw); ";
            else if (mm.group() == 3) {
                if (mm.length() == 1)
    ...
    if (mbfile.good()) {
        ...
        while (iter != monitors.end()) {
            ...
            if (mm.group() == 1 || mm.group() == 0)
                mbfile << "Discrete\t1\n";
            else if (mm.group() == 3 || mm.group() == 4) {
                if (mm.length() == 1)
                    mbfile << "Signed_int_16";
                else if (mm.length() == 2)
                    mbfile << "Signed_int_32";
                else
                    mbfile << "Ascii_String";
                mbfile << "\t" << mm.length() << "\n";
            }
