...
void loadRemoteConfiguration(zmq::socket_t &iod, std::string &chn_instance_name, PLCInterface &plc,
    ...
    if (obj) {
        ...
        while (item) {
            ...
            if (type == "INPUTBIT")
                group = 1;
            else if (type == "OUTPUTBIT") {
                group = 0;
                readonly = false;
            }
            else if (type == "INPUTREGISTER")
                group = 3;
            else if (type == "OUTPUTREGISTER") {
                group = 4;
                readonly = false;
            }
            ...
            if (addr_js && addr_js->type == cJSON_String) {
                ...
                if (plc_addr.first >= 0)
                    group = plc_addr.first;
                if (plc_addr.second >= 0)
...
int main(int argc, const char *argv[]) {
    ...
    std::cerr << "(linked): " << libmodbus_version_major << "." << libmodbus_version_minor << "."
    ...
    if (options.configFileName()) {
        ...
        cJSON *obj = nullptr;
        {
            ...
            if (!response) {
                ...
                FileLogger fl(program_name);
                fl.f() << "empty response to channel request. exiting\n";
                sleep(2);
                exit(2);
            }
            if (options.verbose)
    ...
    while (program_state != s_finished) {
        ...
        if (cmd == "STATE") {
            ...
            try {
                std::map<std::string, ModbusMonitor>::iterator found =
                    mc.monitors.find(params[1].asString());
                ...
                if (!m.readOnly() && m.group() == 4) {
                    if (options.verbose)
                        std::cerr << "setting " << m.name() << " (" << m.format() << ")\n";
                            ...
                            if (params[3].asFloat(dval)) {
                                ...
                                if (m.length() == 2) {
                                    ...
                                    std::cerr << "Error: cannot set float value for " << params[1]
                                              << " into a field of length " << m.length() << "\n";
                                }
                            }
                            else {
                                std::cerr << "Error: could not convert '" << params[3]
                                          << "' to a float\n";
                            }
                        }
                        else {
