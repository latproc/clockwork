...
void Options::usage(const char *prog) {
    std::cout
        << prog << " [-h hostname] [ -p port] [ -c modbus_config ] [ --channel channel_name ] "
    ...
    std::cout
        << "  setting hostname or port activates tcp mode, setting an rtu setting avtivates rtu mode\n";
    std::cout
        << "\n  other optional parameters:\n\n\t-s\tsimfile\t to create a clockwork configuration for simulation\n";
}

Options::Options()
    : verbose(false), multireg_write(true), singlereg_write(true), mt(mt_unknown), settings_tcp(0),
      settings_rtu(0), channel_name("PLC_MONITOR"), config_filename(""), valid_options(false) {
...
const char *Options::channelName() const { return channel_name.c_str(); }
...
const SerialSettings *Options::serial() const { return serial_; }

bool Options::valid() const { return valid_options; }

const char *Options::configFileName() const { return config_filename; }

const char *Options::simulatorName() const { return sim_name; }

uint64_t Options::getTimeout() const { return timeout_secs * 1000000 + timeout_usecs; }
...
bool Options::parseArgs(int argc, const char *argv[]) {
    ...
    while (arg < argc) {
        if (strcmp(argv[arg], "-h") == 0 && arg + 1 < argc) {
            ...
            if (verbose)
                std::cout << "reporting status to property " << status_property << " of "
                          << status_machine << "\n";
        }
            ...
            if (res == 0) {
                if (verbose)
                    std::cout << "tty settings: baud: " << settings_rtu->serial.baud
