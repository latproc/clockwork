...
3. **`select` / `order` / `limit` / `where`** — **landed.** Equality, `null`/`is` null, `eq`/`neq`/`gt`/`lt`/`ge`/`le`. `order` identifiers, `-col` DESC, `limit`. Named views are the join path. Delete replies return the keys.
