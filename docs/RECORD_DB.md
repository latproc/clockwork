...
`PERSISTENT` and `LOCAL` are mutually exclusive (parse error). `KEY` / `UNIQUE` / `NOT NULL` belong on a bare `OPTION` (database column), not on `PERSISTENT OPTION` or `LOCAL OPTION`. `PERSISTENT OPTION` is **landed** (iod-9): per-field persist.dat, non-column, not APPLYed.
...
| iod-9 | `cwlang.ypp` `record_section` | `PERSISTENT OPTION` is not accepted in a RECORD body, though the grammar sketch and OPTIONS = columns describe its semantics. | Doc drift | **Landed** — `PERSISTENT OPTION` parses in MACHINE and RECORD bodies; per-field persist.dat, non-column, mutually exclusive with `LOCAL` |
| iod-10 | `cwlang.ypp` `QUERY … INTO` | The `INTO` target is discarded (`(void)$4`); the SEND goes out but the named LIST is never filled. | Gap | **Landed** (PR 7: QUERY injects `respond_to` = issuing machine `.response`; PR 8: `json AS LIST`; **auto-fill**: `INTO <list>` installs its own `response_changed` fill, so the list is populated without an explicit handler) |
...
7. **QUERY INTO** — **landed.** `QUERY q INTO list` SENDs JSON property `q` to `DATABASE_CHANNEL` (same as INTERFACE load); `QUERY JSON_VALUE { … } INTO list` SENDs that object. `QUERY` injects `respond_to` = `<issuing machine>.response` so dbd routes the reply back there; dbd routes the `response` **payload** (the row array, not the `{status,request,response}` envelope). The scan cannot wait for dbsvr, so `QUERY` does not fill synchronously; instead the parser installs an automatic `response_changed` fill for the `INTO` target (clear + `PUSH ITEMS FROM response`, i.e. `list := response AS LIST`). If the machine declares its own `RECEIVE response_changed`, that handler wins and no fill is installed (the author fills/drains the list). No `Class#key` spawn.
