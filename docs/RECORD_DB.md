...
5. **Two Clockworks, one datastore** — **notify path landed.** `dbsvr` PUB after COMMIT. dbd SUB applies even if CHANNEL is down (`test_two_dbd`: two dbd, two command REPs; live `dbsvr` insert when `DBSVR` is set). `test_two_process_apply` is two RECORD holders on one PUB. Plant iod binaries still later.
