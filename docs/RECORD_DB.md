...
| iod-10 | `cwlang.ypp` `QUERY … INTO` | The `INTO` target is discarded (`(void)$4`); the SEND goes out but the named LIST is never filled. | Gap | **Landed** (PR 7: `INTO list` names the LIST the reply becomes via `AS LIST`; QUERY now also injects `respond_to` = issuing machine `.response`) |
...
7. **QUERY INTO** — **landed.** `QUERY q INTO list` SENDs JSON property `q` to `DATABASE_CHANNEL` (same as INTERFACE load); `QUERY JSON_VALUE { … } INTO list` SENDs that object. `QUERY` injects `respond_to` = `<issuing machine>.response` so dbd routes the reply back there; dbd now routes the `response` **payload** (the row array, not the `{status,request,response}` envelope). The scan cannot wait for dbsvr, so `INTO list` names the LIST the reply is turned into: `RECEIVE response_changed { list := response AS LIST; }` (PR 8) or `PUSH ITEMS FROM response TO list`. No `Class#key` spawn.
