...
| iod-15 | `dbd.cpp` | dbsvr request failure (timeout) drops the request with no error back to Clockwork; this is the Q11 `WAITFOR` hang. | Q11 | **Open** — Clockwork-side timeout landed ([PR 11](#clockwork-pr-11-waitfor--call-timeout-q11-iod-15--proposed)); remaining work is for dbd to report the drop rather than relying solely on the WAITFOR/CALL timeout |
