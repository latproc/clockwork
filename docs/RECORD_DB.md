...
**Related idea (Martin, larger):** a block-level `TRY { … } WHEN TIMER >= timeout { ABORT | THROW | RETURN }` with `CATCH <msg>`. **First cut landed (2026-08-31, `[common]`):** `TRY` is now a token, the grammar `TRY { body } WHEN predicate { handler }` parses (`TryActionTemplate`/`TryAction`, `tests/try_catch.cw` as `parse_try_catch`), and a **synchronous** body completes correctly. **Known limitation:** a *blocking* body (`WAITFOR`/`CALL` inside the TRY) does not resume correctly — the nested body `MachineCommand` suspends and captures the machine's action stack, so the outer `TryAction::checkComplete()` never re-evaluates the timeout predicate. Making nested blocking actions yield back to the TRY (or scheduling the `TIMER >= timeout` predicate as a timer event) is the remaining work.
