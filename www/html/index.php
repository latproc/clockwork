       ...
       if ($username != '' && $password != '') {
           ...
           $password = $_REQUEST['password'];
               ...
               try {
                   ...
                   $_SESSION['user'] = $user;
                   unset($message);
                   header('Location: index.php'); // redirect to strip the login parameters
               } catch (Exception $e) {
...
if (User::count() == 0 || isset($user)) {
   if ( (User::count() == 0) || ($user != null && array_key_exists('registration', $_REQUEST))) {
       ...
       <form id="registration_form" action='index.php?m=register' method=post>
...
EOD;
   ...
   <form id="login_form" action='index.php?m=login' method=post>
      <p>username:<input type="text" name="username" default=$username ></p>
      <p>password:<input type="password" name="password"></p>
