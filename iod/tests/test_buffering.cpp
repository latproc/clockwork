...
TEST_F(BufferTest, getBufferValueAt) {
    ...
    EXPECT_EQ(1.0, getBufferValueAt(buf, 0))
        << "returns the earliest entry when the time is earlier than the earliest sample";
    EXPECT_EQ(1.5, getBufferValueAt(buf, 3))
        << "returns an estimate when the time is earlier than the earliest sample";
    EXPECT_EQ(3.0, getBufferValueAt(buf, 8))
        << "returns the latest entry when the time is later than the latest sample";
    destroyBuffer(buf);
}
