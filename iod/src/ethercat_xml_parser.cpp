...
void ConfigurationDetails::init() {
	...
	for (unsigned int i=0; i<estimated_max_syncs; ++i) c_syncs[i].index = 0xff;
...
SmMapping::~SmMapping() { if (pdos) delete pdos; }
...
DeviceInfo::DeviceInfo(uint64_t pc, uint64_t rn) : product_code(pc), revision_no(rn), selected_alt_sm(-1) { }
...
void EtherCATXMLParser::displayAttribute(xmlTextReaderPtr reader, const char *attr_name, const char *label) {
	char *attr = (char *)xmlTextReaderGetAttribute(reader, BAD_CAST attr_name);
	if (attr) {
		if (label) std::cout << label << attr;
		else std::cout << " " << attr_name << ": " << attr;
		free(attr);
	}
}

void EtherCATXMLParser::captureAttribute(xmlTextReaderPtr reader, const char *attr_name, std::map<std::string, std::string> &attributes) {
	char *attr = (char *)xmlTextReaderGetAttribute(reader, BAD_CAST attr_name);
	if (attr) attributes[attr_name] = attr;
}
...
void EtherCATXMLParser::displayPath() {
	...
	while (iter != current_path.end()) {
		...
		std::cout << s;
		if (iter != current_path.end()) std::cout << "/";
	}
	std::cout << "\n";
}
...
//		if (matched_device)std::cout << state << " " << kind << " " << name << "\n";
		if (kind == XML_READER_TYPE_END_ELEMENT) {
			if (current_path.size()) current_path.pop_back();
			--current_depth;
		...
		switch(state) {
			...
			case in_device_type:
			{
				...
				if (kind == XML_READER_TYPE_TEXT && matched_device) {
					if (matched_device) std::cout << "MATCHED Type: ";
			...
			case in_subnode:
			{
				if (kind == XML_READER_TYPE_ELEMENT ) {
					...
					if ( current_path.back() == deviceKey || current_path.back() == moduleKey) enter(in_device);
					else if (current_path.back() == rxPdoKey || current_path.back() == txPdoKey) {
						if (current_device) reenter(in_pdo);
					}
				}
			}
			break;
			case in_sm:
			{
				...
				if (matched_device && current_path.back() == smKey && (kind == XML_READER_TYPE_TEXT || kind == XML_READER_TYPE_CDATA) ) {
					if (xmlTextReaderHasValue(reader))
					...
					si->n_pdos = 0;
					const int c_pdos_size = sizeof(ec_pdo_info_t) * ConfigurationDetails::estimated_max_pdos;
			...
			case in_pdo:
			{
				...
				if (kind == XML_READER_TYPE_ELEMENT ) {
					...
					else enter(in_subnode);
					break;
				}
				else if (kind == XML_READER_TYPE_END_ELEMENT && (current_path.back() == deviceKey || current_path.back() == moduleKey) ) {
					...
					if (capturing_pdo) {
						if (current_sm_index >= 0) {
							...
							ec_pdo_info_t *pd = &current_device->config.c_syncs[current_sm_index].pdos[n];
							pd->index = current_pdo_index;
							pd->n_entries = current_device->config.num_entries - current_pdo_start_entry;
							pd->entries = &current_device->config.c_entries[current_pdo_start_entry];
	
							std::cout << "Added pdo entry "
								<< std::hex << "0x" << pd->index << std::dec
					...
					if (xmlTextReaderHasValue(reader)) {
						...
						if (selected_alt_sm) {
							for (unsigned int i=0; i<selected_alt_sm->mappings.size(); ++i) {
								for (unsigned int j=0; j<selected_alt_sm->mappings[i]->n_pdos; ++j) {
						...
						std::cout << "PDO Index: ";
						std::cout << "0x" << std::hex << current_pdo_index << std::dec
							<< " sm index " << current_alt_sm_mapping_index << " (" << attributes["Sm"] << ") "
							<< (capturing_pdo ? " (USE)" : "");
					}
					std::cout << "\n";
				}
			...
			case in_pdo_entry:
			{
				...
				else if (matched_device && (kind == XML_READER_TYPE_TEXT || kind == XML_READER_TYPE_CDATA) ) {
					if (xmlTextReaderHasValue(reader))  {
						entry_attributes[ current_path.back() ] = (const char *)xmlTextReaderConstValue(reader);
			...
			case in_alt_sm_mapping:
			{
				if (kind == XML_READER_TYPE_ELEMENT && smKey == name) {
					...
					if (count) {
						...
						while (count--) { std::cout << "0x" << std::hex << *i << std::dec << (count ? ", " : "\n"); ++i; }
					}
					reenter(in_subnode);
				}
			}
		...
		if (kind != XML_READER_TYPE_SIGNIFICANT_WHITESPACE) {
			...
			if (xmlTextReaderHasValue(reader)) std::cout << value;
			std::cout << "\n";
		}
#endif
