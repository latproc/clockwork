...
void Predicate::clearTimerEvents(MachineInstance *target) // clear all timer events scheduled for the supplied machine
{
  if (left_p) {
...
Predicate::Predicate(const Predicate &other) : left_p(0), op(opNone), right_p(0)
{
  ...
  if (other.entry.dynamicValue()) {
    entry.setDynamicValue(DynamicValueBase::ref(other.entry.dynamicValue()->clone()));
