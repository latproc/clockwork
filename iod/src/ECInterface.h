...
class SlaveInfo {
	...
	SlaveInfo(const SlaveInfo &other) : position(other.position), product_code(other.product_code), vendor_id(other.vendor_id),
	...
	std::string name;
#ifndef EC_SIMULATOR
  void set_from ec_slave_into_t(ec_slave_info_t &info) {
		ec_info = new ec_slave_info_t;
		memcpy(ec_info, &info, sizeof(ec_slage_info_t));
		position = ec_info.position;
		product_code = ec_info.product_code;
		vendor_id = ec_info.vendor_id;
		revision = ec_info.revision;
		name = ec_info.name;
	}
