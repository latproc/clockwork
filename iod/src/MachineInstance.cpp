...
State &MachineInstance::getCurrent() { return current_state; }
...
bool MachineInstance::stableStateValid(const std::string state_name) {
    ...
    for (unsigned int ss_idx = 0; ss_idx < stable_states.size(); ++ss_idx) {
        ...
        if (s.condition(this)) {
            if (s.state_name != state_name) {
                std::cerr << "Invalid transition to " << state_name
                          << " due to first matching state " << s.state_name << "\n";
            }
            found = s.state_name;
            break;
        }
...
const Value *MachineInstance::resolve(std::string property) {
    if (property.find('.') != std::string::npos) {
        ...
        if (!state_machine) {
            resetTemporaryStringStream();
            ss << __FILE__ << ":" << __LINE__ << " " << _name
               << " could not find a machine definition: " << _type;
...
Value *MachineInstance::getMutableValue(const char *property_name) {
    ...
    if (property.find('.') != std::string::npos) {
        ...
        if (!state_machine) {
            resetTemporaryStringStream();
            ss << __FILE__ << ":" << __LINE__ << " " << _name
               << " could not find a machine definition: " << _type << " getting property '"
               << property_name << "'";
            DBG_PROPERTIES << ss.str() << "\n";
            error_messages.push_back(ss.str());
            ++num_errors;
