...
struct PredicateActionTemplate : public ActionTemplate {
    ...
    std::ostream &operator<<(std::ostream &out) const { return out << *predicate; }
    Predicate *predicate;
};

struct PredicateAction : public Action {
    PredicateAction(MachineInstance *mi, PredicateActionTemplate &pat)
        : Action(mi), predicate(new Predicate(*pat.predicate)) {}
    ~PredicateAction();
    Status run();
