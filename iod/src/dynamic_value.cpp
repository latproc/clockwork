...
const Value &PropertyLookupValue::operator()() {
	...
	if (prop.kind == Value::t_string) { prop.toSymbol(); }
	if (prop.isNull()) {
		std::stringstream ss; ss << mi->getName() << " no property " << property_name << " for property lookup";
		MessageLog::instance()->add(ss.str().c_str());
		last_result = false; return last_result;
	}
    machine = mi->lookup(machine_name);
    if (!machine)  {
        std::stringstream ss; ss << mi->getName() << " no machine " << machine_name << " for property lookup";
        MessageLog::instance()->add(ss.str().c_str());
        last_result = false; return last_result;
    }
...
Value *PropertyLookupValue::getMutable(MachineInstance *current_scope) {
	...
	if (!machine)  {
		std::stringstream ss; ss << current_scope->getName() << " no machine " << machine_name << " for property lookup";
		MessageLog::instance()->add(ss.str().c_str());
		last_result = false;
		return nullptr;
	}
	...
	if (prop.kind == Value::t_symbol || prop.kind == Value::t_string) {
		...
		std::stringstream ss; ss << machine->getName() << " " << property_name << " does not resolve to a name";
		MessageLog::instance()->add(ss.str().c_str());
		last_result = false; return nullptr;
	}
	return nullptr;
}
...
std::ostream &operator<<(std::ostream &out, const PropertyLookupValue &val) { return val.operator<<(out); }
