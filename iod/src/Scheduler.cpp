...
std::string Scheduler::getStatus() {
    ...
    if (!anomaly_events.empty()) {
        ss << "recent_overdue:\n";
        for (std::deque<std::string>::const_iterator ai = anomaly_events.begin();
             ai != anomaly_events.end(); ++ai) {
            ss << "  " << *ai << "\n";
        }
    }
...
void Scheduler::idle() {
    ...
    while (state != e_aborted) {
        ...
        if (state == e_waiting && is_ready) {
            ...
            if (last_sched_signal_us != 0) {
                ...
                while (state != e_aborted && microsecs() < deadline) {
                    uint64_t rem = deadline - microsecs();
                    if (rem > 500) {
                        rem = 500; // chunk so we re-check deadline often
                    }
                    try {
                        boost::this_thread::sleep_for(
                            boost::chrono::microseconds(rem));
                    }
            ...
            last_sched_signal_us = microsecs();
        ...
        while (state == e_running && is_ready) {
            ...
            next_time = 0;
            const uint64_t fired_at = microsecs();
            const uint64_t lateness = fired_at > item->delivery_time
                                          ? fired_at - item->delivery_time : 0;
            if (lateness) {
                overdue_count.fetch_add(1, std::memory_order_relaxed);
                updateMax(max_lateness_us, lateness);
                if (lateness >= anomaly_threshold_us) {
                    recordAnomaly(item, fired_at, lateness);
                }
            }
