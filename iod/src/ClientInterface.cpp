...
class MyMonitor : public zmq::monitor_t {
  public:
    ...
    virtual void on_monitor_started() { DBG_MSG << "CI << command channel monitor started\n"; }
...
void IODCommandThread::operator()() {
    ...
    do { // wait to start
        ...
        if (!safeRecv(access_req, start_cli, 19, true, len, -1)) {
            assert(false); // TBD - can this happen?
            usleep(100000);
            continue;
        }
        if (len > 20) {
            start_cli[19] = 0;
            FileLogger fl(program_name);
            fl.f() << "client interface startup got unexpected: " << start_cli << "\n";
        }
        if (len < 20) {
            start_cli[len] = 0;
        };
