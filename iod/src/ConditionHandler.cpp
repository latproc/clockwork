...
ConditionHandler::ConditionHandler()
    : timer_val(0), timer_op(opGE), trigger(0), uses_timer(false), triggered(false) {}

ConditionHandler::ConditionHandler(const ConditionHandler &other)
    : condition(other.condition), timer_val(other.timer_val), trigger(other.trigger),
      uses_timer(other.uses_timer), triggered(other.triggered), _command_name(other._command_name),
...
bool ConditionHandler::check(MachineInstance *machine) {
    ...
    if (_command_name == "FLAG") {
            ...
            if (condition(machine)) {
                if (strcmp("on", flag->getCurrentStateString())) {
                    if (tracing() && machine->isTraceable()) {
                        ...
                        machine->ss << machine->getCurrentStateString() << " "
                                    << *condition.predicate;
        ...
        triggered = true;
        DBG_AUTOSTATES << machine->getName() << " subcondition triggered: " << _command_name << " "
                       << *(condition.predicate) << "\n";
        ...
        if (triggered) {
            DBG_AUTOSTATES << "		 condition " << (condition.predicate) << " already triggered\n";
        }
        if (condition(machine)) {
            DBG_AUTOSTATES << "		condition " << (condition.predicate) << " passes\n";
        }
    }
    if (triggered) {
        return true;
