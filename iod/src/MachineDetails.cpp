...
MachineDetails::MachineDetails(const char *nam, const char *cls, std::list<Parameter> &params,
                               const char *sf, int sl, SymbolTable &props,
                               MachineInstance::InstanceType kind)
    : machine_name(nam), machine_class(cls), source_file(sf), source_line(sl), instance_type(kind) {
...
MachineInstance *MachineDetails::instantiate() {
    MachineInstance *machine =
        MachineInstanceFactory::create(machine_name.c_str(), machine_class.c_str(), instance_type);
