...
ResumeActionTemplate::ResumeActionTemplate(const std::string &name, const std::string &state,
                                           const char *property, const Value *val)
...
Action *ResumeActionTemplate::factory(MachineInstance *mi) { return new ResumeAction(mi, this); }
    ...
    if (machine_name == "ALL") {
        ...
        if (machine) {
            if (!machine->getStateMachine()) {
                char buf[150];
                snprintf(buf, 150, "resuming a machine %s that has no state machine",
