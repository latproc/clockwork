...
void collectSlaves(std::vector<SlaveInfo> &slave_arr) {
		...
		std::transform(slaves.begin(), slaves.end(), std::back_inserter(slave_arr), );
...
class ModuleSpec {
	public:
		ModuleSpec(MachineInstance *m) :
		  position(m->getValue("position")),
			xml_filename(m->getValue("config_file"))
			selected_sm(m->getValue("alternate_sync_manager"))
			product_code(m->getValue("ProductCode"))
			revision_no(m->getValue("RevisionNo")) {
		}
	Value &position;
	Value &xml_filename;
	Value &selected_sm ;
...
void update_device_spec_from_bus(ModuleSpec &spec, unsigned int position, std::vector<SlaveInfo> &slave_arr) {
	if (position < slave_arr.size()) {
		if (spec.product_code == SymbolTable::Null)
			spec.product_code = (long)slave_arr[ position.iValue ].product_code;
		if (spec.revision_no == SymbolTable::Null)
			spec.revision_no = (long)slave_arr[ position.iValue ].revision_no;
	...
	while (iter != MachineInstance::end())
	{
		...
		if (m->_type == "MODULE") {
			...
			if (xml_filename != SymbolTable::Null) {
				...
				update_device_spec_from_bus(seek, seek.position, slave_arr);
