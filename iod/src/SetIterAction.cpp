...
SetIterActionTemplate::SetIterActionTemplate(const std::string &name, Value val, Value pos, bool insert_before, bool expand_items)
...
SetIterAction::SetIterAction(MachineInstance *m, const SetIterActionTemplate *dat, Value pos, bool insert_before, bool expand_list)
...
SetIterAction::SetIterAction() : list_machine(0), entry_machine(0), position(-1), before(false), expand(false) {
}

std::ostream &SetIterAction::operator<<(std::ostream &out) const {
  out << "SET " << entry << " TO ";
	if (position < 0) out << "LAST"; else out << " ITEM " << position;
...
Action::Status SetIterAction::run() {
  ...
  if (iterator_machine && iterator_machine->_type != "ITERATOR")
    ...
    if (!list_machine)
      ...
      snprintf(err_msg, 100, "%s no iterator found (check %s)",
               owner->getName().c_str(), entry.asString().c_str());
  ...
  if (list_machine->parameters.size() == 0) {
    char *err_msg = (char *)malloc(100);
    snprintf(err_msg, 100, "%s: cannot set iterator; list %s has no items", owner->getName().c_str(), list_machine_name.c_str());
    MessageLog::instance()->add(err_msg);
    error_str = err_msg;
    status = Failed;
    return status;
  }
  if (pos < 0) pos = list_machine->parameters.size()-1;

  // check and update the iterator parameter to refer to the list
  if (iterator_machine->parameters.size() && iterator_machine->parameters[0].machine != list_machine) {
    Value v(entry);
