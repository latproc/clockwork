...
Action *UnlockActionTemplate::factory(MachineInstance *mi) { return new UnlockAction(mi, this); }
...
Action::Status UnlockAction::run() {
    ...
    if (machine)
        if (machine->unlock(owner)) {
            ...
            status = Failed; // invalid to try to unlock a machine that you didn't lock
        }
    else {
        status = Failed;
    }
