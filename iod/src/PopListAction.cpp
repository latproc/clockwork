...
Action *PopBackActionTemplate::factory(MachineInstance *mi) { return new PopBackAction(mi, this); }

PopBackAction::PopBackAction(MachineInstance *m, const PopBackActionTemplate *dat)
        ...
        if (list_machine->parameters.size()) {
            ...
            if (list_machine->parameters[i].val.kind == Value::t_symbol) {
                ...
                if (mi == 0) {
                    ...
                    res = list_machine->parameters[i].machine->getValue("VALUE");
                }
            }
            else {
            }
