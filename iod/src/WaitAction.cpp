...
WaitActionTemplate::WaitActionTemplate(CStringHolder property) : wait_time(-1), property_name(property.get()) {
...
void WaitActionTemplate::toC(std::ostream &out, std::ostream &vars) const {
  ...
  << "\t  RTScheduler_add(scheduler, ScheduleItem_create("<< wait_time << " - (m->machine.TIMER - ctx->wait_start), &m->machine));\n"
  << "\t  RTScheduler_release();\n"
  << "\t  ccrReturn(0);\n"
  << "\t}\n";
}
