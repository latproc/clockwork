...
FileOperationActionTemplate::FileOperationActionTemplate(Value num, Value a, Value b,
                                                         Value destination, Value property,
                                                         SetOperation op, Predicate *pred,
                                                         bool remove, Value start, Value end)
    : count(num), src_a_name(a.asString()), src_b_name(b.asString()),
      ...
      condition(pred), remove_selected(remove), start_pos(start), end_pos(end) {}

FileOperationActionTemplate::~FileOperationActionTemplate() {}

Action *FileOperationActionTemplate::factory(MachineInstance *mi) {
    switch (operation) {
    case soIntersect:
...
FileOperationAction::FileOperationAction(MachineInstance *m, const FileOperationActionTemplate *dat)
    : Action(m), scope(m), count(dat->count), source_a(dat->src_a_name), source_b(dat->src_b_name),
      dest(dat->dest_name), condition(dat->condition), property_name(dat->property_name),
      source_a_machine(0), source_b_machine(0), dest_machine(0), operation(dat->operation),
...
FileOperationAction::FileOperationAction() : Action(), dest_machine(0) {}

std::ostream &FileOperationAction::operator<<(std::ostream &out) const {
...
Action::Status FileOperationAction::run() {
    owner->start(this);
    try {
...
Action::Status FileOperationAction::doOperation() { return Failed; }

Action::Status FileOperationAction::checkComplete() {
...
IntersectSetOperation::IntersectSetOperation(MachineInstance *m,
                                             const FileOperationActionTemplate *dat)
    : FileOperationAction(m, dat) {
    /*
                ...
                if (v1.kind == Value::t_symbol &&
                    ...
                    if (CompareSymbolAndValue(owner, v1, property_name, b)) {
                        ...
                        break;
                         ...
                         (v1.kind == Value::t_string || v1.kind == Value::t_integer)) {
...
UnionSetOperation::UnionSetOperation(MachineInstance *m, const FileOperationActionTemplate *dat)
    : FileOperationAction(m, dat) {}
...
DifferenceSetOperation::DifferenceSetOperation(MachineInstance *m,
                                               const FileOperationActionTemplate *dat)
    : FileOperationAction(m, dat) {}
...
SelectSetOperation::SelectSetOperation(MachineInstance *m, const FileOperationActionTemplate *dat)
    : FileOperationAction(m, dat) {
    /*
    */
}
