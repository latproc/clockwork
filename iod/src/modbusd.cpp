...
void insert(int group, int addr, int value, size_t len) {
    ...
    if (group == 1) {
        ...
        if (len == 1) {
            ...
            if (DEBUG_BASIC) {
                std::cout << "Updated Modbus memory for 32 bit reg " << addr << " to "
                          << (value & 0xffffffff) << "\n";
            }
...
struct ModbusServerThread {
    ...
    void operator()() {
        ...
        while (modbus_state != ms_finished) {
            ...
            if (modbus_state == ms_starting || modbus_state == ms_resuming) {
                ...
                if (debug) {
                    std::cout << "Modbus listen socket: " << socket << "\n" << std::flush;
                }
                FD_ZERO(&connections);
            ...
            for (int conn = 0; conn <= max_fd; ++conn) {
                ...
                if (conn == socket) {
                    ...
                    int panel_fd;
                    if ((panel_fd = accept(socket, (struct sockaddr *)&panel_in, &addr_size)) ==
                        -1) {
                        perror("accept");
                    }
                    else {
                        ...
                        int option = 1;
                        int res =
                            setsockopt(panel_fd, IPPROTO_TCP, TCP_NODELAY, &option, sizeof(int));
                        if (res == -1) {
                            perror("setsockopt");
                        }
                    ...
                    modbus_set_socket(modbus_context,
                                      conn); // tell modbus to use this current connection
                    n = modbus_receive(modbus_context, query);
                    char *previous_value =
                        0; // used to see if a string value is different between ram and connection
                    if (n != -1) {
                        ...
                        if (fc == 3) {
                                ...
                                for (int bit = 0; bit < 8; ++bit) {
                                    ...
                                    if (active_addresses.find(addr_str) != active_addresses.end()) {
                                        ...
                                        if (!initialised_address[addr_str] ||
                                            (val != 0 && modbus_mapping->tab_bits[addr] == 0) ||
                            ...
                            //int num_bytes = query_backup[function_code_offset+5];
                            unsigned char *data =
                                query_backup + function_code_offset + 6; // interpreted as binary
                            for (int reg = 0; reg < num_words; ++reg) {
                                ...
                                if (active_addresses.find(addr_str) != active_addresses.end()) {
                                    ...
                                    */
                                    if (!initialised_address[addr_str] ||
                                        val != modbus_mapping->tab_registers[addr]) {
                                        if (debug & DEBUG_LIB)
                                            std::cout << " Updating register " << addr << " to "
                                                      << val << " from connection " << conn << "\n";
                                        iod_sync_commands.push_back(
                                            getIODSyncCommand(4, addr + 1, val));
                                        initialised_address[addr_str] = true;
                                    }
                                }
                                ++addr;
                        ...
                        if (fc == 2) {
                            ...
                            if (active_addresses.find(addr_str) != active_addresses.end())
                                if (debug & DEBUG_LIB) {
                                    std::cout << timestamp << " connection " << conn << " read "
                                              << num_coils << " discrete " << addr << " (";
                                    for (int bitn = 0; bitn < num_coils; ++bitn) {
                                        std::cout
                                            << (int)(modbus_mapping->tab_input_bits[addr + bitn])
                                            << " ";
                                    }
                                    std::cout << ")\n";
                                }
                            ...
                            for (int i = 0; i < num_regs; ++i) {
                                if (((uint16_t *)(dest))[i] != modbus_mapping->tab_registers[addr + i])
                                    if (DEBUG_BASIC) {
                                        std::cout << "connection " << conn << " num regs: " << num_regs << " changed rw register " << (addr + i) << "\n";
                                    }
                            }
                            free(new_value);
                            free(previous_value);
                            ...
                            if (!ignore_coil_change) {
                                ...
                                if (DEBUG_BASIC)
                                    std::cout << timestamp << " Updating coil " << addr
                                              ...
                                              << ((query_backup[function_code_offset + 3]) ? " on"
                                                                                           : " off")
                                              << "\n";
                            }
                        }
                            ...
                            std::cout << timestamp << " function code: "
                                      << (int)query_backup[function_code_offset] << "\n";
                        }
                        if (n == -1) {
                            if (DEBUG_BASIC) {
    ...
    void resume() { modbus_state = ms_resuming; }
...
void loadData(const char *initial_settings) {
    ...
    if (!obj) {
        ...
        while (init.getline(buf, 200, '\n')) {
            ...
            if (debug) {
                std::cout << name << ": " << group << " " << kind << " " << addr << " " << len
                          << " " << value << "\n";
            }
            if (kind == 7) {
                len = 2; // float
    ...
    else {
        {
            ...
            while (item) {
                if (item->type == cJSON_Array) {
                    ...
                    if (DEBUG_BASIC) {
                        std::cout << name << ": " << group << " " << addr << " " << len << " "
                                  << value << "\n";
                    }
                    if (value.kind == Value::t_string) {
                        std::string valstr = value.asString();
                        insert((int)group.iValue, (int)addr.iValue - 1, valstr.c_str(),
                               valstr.length() + 1); // note copying null
                    }
                    else {
                        insert((int)group.iValue, (int)addr.iValue - 1, (int)value.iValue,
                               len.iValue);
                    }
                }
                else {
...
static void toggle_debug(int sig) {
    ...
    if (debug) {
        ...
        if (saved_debug == 0) {
            saved_debug = NOTLIB ^ VERBOSE_TOPLC;
        }
        debug = saved_debug;
    }
}
    ...
    try {
        ...
        while (program_state != s_finished) {
            ...
            if (cmd == "UPDATE" && count >= 6) {
                ...
                if (debug) {
                    std::cout << "IOD: " << group << " " << addr << " " << name << " " << len << " "
                              << params[5] << "\n";
                }
