...
SortListAction::SortListAction(MachineInstance *m, const SortListActionTemplate *dat, bool asc)
    : Action(m), list_machine_name(dat->list_machine_name), property_name(dat->property_name),
      list_machine(0), ascending(asc) {}

SortListAction::SortListAction() : list_machine(0), ascending(true) {}

std::ostream &SortListAction::operator<<(std::ostream &out) const {
    return out << "Sort List Action " << list_machine->getName() << " BY PROPERTY "
               << property_name;
}
    ...
    if (list_machine && list_machine->_type == "LIST") {
        ...
        if (list_machine->parameters.empty()) {
            ...
            if (use_integer_sort) {
                ...
                std::sort(list_machine->parameters.begin(), list_machine->parameters.end(),
                          property_sort);
            }
            else {
