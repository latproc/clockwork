...
class Options {
    ...
    Options()
        : is_server(true), port_(10240), host_(0), name_(0), machine_(0), property_(0), pattern_(0),
          iod_host_(0), serial_port_name_(0), serial_settings_(0), watch_(0), queue_(0),
          collect_duplicates(false), disconnect_on_timeout(true), cw_publisher(5556),
          got_host(false), got_port(true), got_property(false), got_pattern(false),
          structured_messaging(true), got_queue(false) {
  ...
  public:
    ...
    bool valid() const {
        ...
        bool result = ((got_port && got_host) || (got_serial && got_serial_settings)) &&
                      (got_property || (got_queue && structured_messaging)) && got_pattern &&
                      name_ != 0 && iod_host_ != 0;
        if (!result) {
...
struct ConnectionThread {
    ...
    void operator()() {
        ...
        try {
            ...
            if (Options::instance()->server()) {
                listener = anetTcpServer(msg_buffer, Options::instance()->port(),
                                         Options::instance()->host());
                if (listener == ANET_ERR) {
                    std::cerr << msg_buffer << " attempting to listen on port "
                              << Options::instance()->port() << "...aborting\n";
            ...
            while (!done) {
                ...
                if (DeviceStatus::instance()->current() == DeviceStatus::e_disconnected &&
                    Options::instance()->client()) {
                    if (Options::instance()->serialPort()) {
                        connection = setupSerialPort(Options::instance()->serialPort(),
                                                     Options::instance()->serialSettings());
                        if (connection == -1) {
                            usleep(retry_delay); // pause before trying again
                            if (retry_delay < 2000000) {
                                retry_delay *= 1.2;
                            }
                            continue;
                        }
                ...
                while (!done) {
                    ...
                    if (dev_state == DeviceStatus::e_connected || dev_state == DeviceStatus::e_up ||
                        dev_state == DeviceStatus::e_timeout) {
                ...
                if (Options::instance()->server() &&
                    ...
                    if (offset < buffer_size - 1) {
                        ...
                        offset = 0;
                        each_match(Options::instance()->regexpInfo(), buf, &offset,
                                   &MatchFunction::match_func, 0);
    ...
    void updateProperty() {
        struct timeval now;
        gettimeofday(&now, 0);
        if (last_status != DeviceStatus::instance()->current() ||
            now.tv_sec >= last_property_update.tv_sec + 5) {
            {
                boost::mutex::scoped_lock lock(connection_mutex);
            ...
            if (!sent) {
                std::cerr << "Failed to set status property " << Options::instance()->name()
                          << ".status\n"
                          << " response: " << response << "\n"
                          << std::flush;
            }
...
static void toggle_debug(int sig) {
    ...
    if (debug) {
        ...
        LogState::instance()->erase(DebugExtra::instance()->DEBUG_CHANNELS);
    }
}
...
class ProcessingThread {
  public:
    ...
    ProcessingThread(Options &opt, zmq::socket_t &command_sock, ConnectionManager *connection_mgr)
    ...
    void operator()() {
        ...
        if (options.watchProperty()) {
            ...
            assert(sm);
            items[idx].socket = (void *)sm->setup();
            items[idx].events = ZMQ_POLLERR | ZMQ_POLLIN;
            idx++;
            subs_index = idx;
            items[idx].fd = 0;
            items[idx].socket = (void *)sm->subscriber();
            items[idx].events = ZMQ_POLLERR | ZMQ_POLLIN;
            idx++;
        }
        else {
        ...
        while (!done) {
            ...
            try {
                ...
                if (zmq_errno()) {
                    ...
                    {
                        FileLogger fl(program_name);
                        fl.f() << "exception when checking connections: " << e.what() << "\n"
                               << std::flush;
                    }
                }
                if (error_count > 10) {
...
int main(int argc, const char *argv[]) {
    ...
    try {
        ...
        try {
            if (options.watchProperty()) {
                connection_manager =
                    new SubscriptionManager(Options::instance()->getChannelName(), eCLOCKWORK);
            }
            else {
