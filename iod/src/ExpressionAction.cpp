			...
			if (rhs.kind == Value::t_symbol) {
				...
				if (extra != SymbolTable::Null) {
					...
					if (machine) {
						...
						char buf[400];
						snprintf(buf, 400, "%s: no machine can be found for SET %s TO PROPERTY %s OF %s",
...
ExpressionActionTemplate::ExpressionActionTemplate(CStringHolder var, opType oper, const Value &v, const Value &e) : lhs(var), rhs(v), op(oper), extra(e) {
  if (op == opDec) rhs = -v;
}
