...
void usage(int argc, char const *argv[]) {
    ...
    if (cw) {
        ...
        std::cerr << "Plant Clockwork runtime (iod / iod-elc). MQTT starts automatically\n"
    ...
    if (cw) {
        ...
        std::cerr
            ...
            << "                             Plant iod-elc always starts MQTT.\n";
    }
    ...
    std::cerr
        ...
        << "\n"
        << "EtherCAT / setup (iod-elc; ignored or unused on cw):\n"
        << "  -e IFACE                   EtherCAT adapter name\n"
        << "  --setup-recipe FILE, -R    Ordered setup recipe (repeatable; FILE@pos)\n"
        << "  --setup-positions LIST     Positions for the last --setup-recipe\n"
        << "  --setup-domain ID          Domain for the last --setup-recipe\n"
        << "  --setup-product CODE       Product code for the last --setup-recipe\n"
        << "  --setup-vendor ID          Vendor id for the last --setup-recipe\n"
        << "\n"
    ...
    while (i < argc) {
        ...
        */
            ...
            */
            ...
            // cw: opt-in MQTT connect. iod-elc always starts MQTT and ignores this.
            set_mqtt_enabled(1);
        }
