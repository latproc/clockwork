...
void MachineClass::exportHandlers(std::ostream &ofs) {
    std::stringstream received_message_handlers;
    {
        ...
        while (recv_iter != receives.end()) {
            ...
            ofs << "int cw_" << name << "_" << method_name(item.first.getText()) << "(struct cw_"
                << name << " *m, ccrContParam) {\n";
        ...
        if (!init_seen) {
            ofs << "int cw_" << name << "_" << initial_state_message << "(struct cw_" << name
                << " *m, ccrContParam) {\n"
                << "\tm->machine.execute = 0;\n"
                << "\treturn 1;\n"
                << "};\n";
    ...
    ofs << "int cw_" << name
        << "_handle_message(struct MachineBase *obj, struct MachineBase *source, int state) {\n"
...
static std::string generate_name_lookup_decls(std::string var, std::string to_lookup,
...
bool MachineClass::cExport(const std::string &filename) {
...

    {
        ...
        while (p_iter != parameters.end()) {
            ...
            setup << "\tif (" << p.val << ") MachineDependencies_add(" << p.val << ", cw_" << name
                  << "_To_MachineBase(m));\n";
    ...
    }
    {
        ...
        ofs << "int cw_" << name
            << "_handle_message(struct MachineBase *ramp, struct MachineBase *machine, int state);\n";
        ...
        ofs << "\tm->machine.state = state_cw_" << initial_state << ";\n"
            ...
            << "\tm->vars = (struct cw_" << name << "_Vars *)malloc(sizeof(struct cw_" << name
            << "_Vars));\n"
            << "\tm->backup = (struct cw_" << name << "_Vars_backup *)malloc(sizeof(struct cw_"
            << name << "_Vars_backup));\n"
            << "\tinit_Vars(m, m->vars);\n"
            << "\tbackup_Vars(m);\n"
