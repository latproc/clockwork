...
void MachineClass::addState(const char *name) { states.push_back(new State(name)); }
...
if (timer_clauses.size()) {
    ofs << "uint64_t cw_" << name << "_next_trigger_time(struct cw_" << name << " *m, struct cw_"
        << name << "_Vars_backup *v);\n";
}
...
if (timer_clauses.size()) {
    ...
    while (iter != timer_clauses.end()) {
        ...
        Predicate *sub_p =
            (l && (l->entry.kind == Value::t_symbol || stringEndsWith(l->entry.sValue, ".TIMER")))
                ? r
                : l;
...
handlers << "\tMachineActions_add(cw_" << name << "_To_MachineBase(m), (enter_func)cw_" << name
         << "_" << initial_state.getName() << "_enter);\n";
...
ofs << "\tm->machine.state = state_cw_" << initial_state << ";\n"
    ...
    << "_Vars));\n"
    << "\tm->backup = (struct cw_" << name << "_Vars_backup *)malloc(sizeof(struct cw_" << name
    << "_Vars_backup));\n"
    << "\tinit_Vars(m, m->vars);\n"
    << "\tbackup_Vars(m);\n"
