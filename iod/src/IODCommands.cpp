...
bool IODCommandCycling::run(std::vector<Value> &params) {
    // Operator-only: walk machines once. No background sampling.
    // Optional third arg: short-hold threshold in milliseconds (default 50).
    ...
    ss << "FAST STATE THRASH (on-demand; no CW-loop cost)\n"
       << "Looks for continuous flip-flop: almost all of the last "
       << MachineInstance::STATE_HISTORY_SIZE << " holds short (<" << (short_hold_us / 1000)
       << "ms),\n"
       << "avg+max hold short, whole ring burned in <1s, >=10 transitions/s.\n"
       << "Ignores long-stable machines with occasional brief test/update hops\n"
       << "(e.g. idle→update every 5s, scales getWeight, boot INIT sequences).\n"
       << "\n"
       << "Offenders:\n";
    ...
    if (flagged == 0) {
        ss << "  (none — no continuous fast thrash in recent state history)\n";
    }
...
bool IODCommandHealth::run(std::vector<Value> &params) {
    // One clear line for iosh startup: load snapshot + continuous thrash count.
    const ProcessingThread::ProcSnap s = ProcessingThread::lastProcSnap();
    size_t thrash = 0;
    ...
    if (!s.valid) {
        ...
        const uint64_t age_ms =
            (microsecs() > s.at_us) ? (microsecs() - s.at_us) / 1000ULL : 0;
        // Use peak over last second (not post-drain "now") for busy judgment.
