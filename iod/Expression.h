...
struct Predicate {
    ...
    Predicate(Value &v) : left_p(0), op(opNone), right_p(0), entry(v), mi(0), cached_entry(0), priority(0), lookup_error(false) {
        if (entry.kind == Value::t_symbol && entry.sValue == "DEFAULT") priority = 1;
    }
    Predicate(const char *s) : left_p(0), op(opNone), right_p(0), entry(s), mi(0), cached_entry(0), priority(0), lookup_error(false) {
        if (entry.kind == Value::t_symbol && entry.sValue == "DEFAULT") priority = 1;
    }
