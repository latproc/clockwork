...
struct CopyPatternActionTemplate : public ActionTemplate {
	CopyPatternActionTemplate(CStringHolder d, CStringHolder p, CStringHolder prop) : dest(d), pattern(p), property(prop) { }
...
struct CopyPatternAction : public Action {
	CopyPatternAction(MachineInstance *mi, CopyPatternActionTemplate &pat)
        : Action(mi), dest(pat.dest.get()), pattern(pat.pattern.get()), property(pat.property.get()) { }
	Status run();
...
struct ExtractPatternActionTemplate : public ActionTemplate {
	ExtractPatternActionTemplate(CStringHolder d, CStringHolder p, CStringHolder prop) : dest(d), pattern(p), property(prop) { }
    virtual Action *factory(MachineInstance *mi);
    std::ostream &operator<<(std::ostream &out) const {
		return out << "copy " << pattern.get() << " from " << property.get();
    }
...
struct ExtractAllPatternActionTemplate : public ActionTemplate {
	ExtractAllPatternActionTemplate(CStringHolder d, CStringHolder p, CStringHolder prop) : dest(d), pattern(p), property(prop) { }
    virtual Action *factory(MachineInstance *mi);
    std::ostream &operator<<(std::ostream &out) const {
		return out << "copy " << pattern.get() << " from " << property.get();
    }
