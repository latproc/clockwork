...
StateBase::StateBase(bool priv, bool trans)
    : is_private(priv), is_transitional(trans) {}
...
State::State(const std::string &msg)
    : text(msg), val(0), name(msg, Value::t_string), enter_proc(0) {
    token_id = Tokeniser::instance()->getTokenId(text.c_str());
}

State::State(const char *msg)
    : text(msg), val(0), name(msg, Value::t_string), enter_proc(0) {
    token_id = Tokeniser::instance()->getTokenId(text.c_str());
}
