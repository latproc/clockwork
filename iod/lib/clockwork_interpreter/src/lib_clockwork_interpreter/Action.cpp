    ...
    std::stringstream ss;
    {
        ...
        while (iter != all_triggers.end()) {
            ...
            if (t->_internals->holders.size()) {
                ss << ":";
                std::list<Action *>::iterator h_iter =
                    t->_internals->holders.begin();
...
Trigger::Trigger(const std::string &n)
    : _internals(0), name(n), seen(false), owner(0), deleted(false), refs(1),
      is_active(true) {
...
Trigger::Trigger(TriggerOwner *own, const std::string &n)
    : _internals(0), name(n), seen(false), owner(own), deleted(false), refs(1),
      is_active(true) {
    _internals = new TriggerInternals;
    addTrigger(this);
}
...
Action::Action(MachineInstance *m)
    : refs(1), owner(m), error_str(""), result_str(""), status(New),
      ...
      timeout_msg(0), error_msg(0) {}

Action::Action(MachineInstance *m, Trigger *t)
    : refs(1), owner(m), error_str(""), result_str(""), status(New),
      saved_status(Running), blocked(0), trigger(t), started_(false),
      timeout_msg(0), error_msg(0) {}
...
void Action::setTrigger(Trigger *t) {
    ...
    if (trigger && trigger == t) {
        DBG_ACTIONS << "Attempt to set trigger " << t->getName()
...
Action::~Action() {
    ...
    if (trigger) {
        ...
        if (len > 5 && trigger->getName().substr(len - 5) == "_done" &&
            !trigger->fired()) {
            std::stringstream ss;
...
void Action::suspend() {
    ...
    if (status != Running) {
        DBG_M_ACTIONS << owner->getName()
                      << " suspend called when action is in state " << status
                      << "\n";
    }
    saved_status = status;
    status = Suspended;
...
Action::Status Action::operator()() {
    ...
    status = run();
    if (status == Failed && exceptions_enabled()) {
        reportError();
    }
    return status;
}
...
void Action::recover() {
    DBG_M_ACTIONS << "Action failed to remove itself after completing: "
                  << *this << "\n";
