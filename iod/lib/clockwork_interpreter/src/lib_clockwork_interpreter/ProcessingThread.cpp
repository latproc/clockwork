...
class ProcessingThreadInternals {
  public:
    ...
    ProcessingThreadInternals()
        : sequence(0), cycle_delay(1000),
          processing_wd("Processing Loop Watchdog", 2000) {}
};

ProcessingThread &ProcessingThread::create(ControlSystemMachine *m,
                                           HardwareActivation &activator,
...
CommandSocketInfo *ProcessingThread::addCommandChannel(CommandSocketInfo *csi) {
    std::list<CommandSocketInfo *>::iterator iter =
    ...
    while (iter != internals->channel_sockets.end()) {
        ...
        ++idx;
        if (info == csi) {
            NB_MSG << "Processing thread already has command socket info for "
                   << csi->address << " at index " << idx << "\n";
...
int ProcessingThread::pollZMQItems(int poll_wait, zmq::pollitem_t items[],
                                   int num_items, zmq::socket_t &ecat_sync,
            ...
            if (items[internals->ECAT_ITEM].revents & ZMQ_POLLIN) {
                ...
                while (true) {
                    ...
                    try {
                        ...
                        case 3: { // data
                            ecat_sync.getsockopt(ZMQ_RCVMORE, &more,
                                                 &more_size);
                            assert(more);
                            zmq::message_t message;
                            ecat_sync.recv(&message);
                            ...
                            memcpy(incoming_process_data, message.data(),
                                   msglen);
#if VERBOSE_DEBUG
                        ...
                        case 4: { // mask
                            ...
                            memcpy(incoming_process_mask, message.data(),
                                   msglen);
#if VERBOSE_DEBUG
                            std::cout << "got mask: ";
                        ...
                        if (zmq_errno() == EINTR) {
                            NB_MSG << "interrupted when sending update ("
                                   << (unsigned int)stage << ")\n";
                            continue;
                        } else {
...
void ProcessingThread::HandleIncomingEtherCatData(
    std::set<IOComponent *> &io_work_queue, uint64_t curr_t,
    ...
    if (n) { // io has indicated a change
        if (machine_is_ready) {
#if VERBOSE_DEBUG
            std::cout << "Processing got masked EtherCAT data at byte "
                      << (incoming_data_size - n) << "\n";
#endif
#ifdef KEEPSTATS
...
struct ProcessingStats {
  public:
    ProcessingStats()
        : avg_poll_time("AVG_POLL_TIME", 0),
          avg_iowork_time("AVG_IOWORK_TIME", 0),
          avg_plugin_time("AVG_PLUGIN_TIME", 0),
          avg_cmd_processing("AVG_PROCESSING_TIME", 0),
...
void ProcessingThread::operator()() {
    ...
    while (!program_done) {
        MEMCHECK();
        if (IOComponent::getHardwareState() ==
            IOComponent::s_hardware_preinit) {
            initialiseHardware();
        }
        pollMachine();
        ...
        while (!program_done) {
            ...
            // add the channel sockets to our poll info
            {
                ...
                num_channels =
                    idx -
                    dynamic_poll_start_idx; // the number channels we are actually monitoring
            }
            ...
            systems_waiting =
                pollZMQItems(poll_wait, items, 6 + num_channels, ecat_sync,
        ...
        */
        if (items[internals->ECAT_ITEM].revents & ZMQ_POLLIN) {
            HandleIncomingEtherCatData(io_work_queue, curr_t, last_sample_poll,
                                       avg_io_time);
        ...
        if (machine_is_ready && processing_state != eStableStates &&
            !io_work_queue.empty()) {
        ...
        if (status == e_waiting &&
            items[internals->DISPATCHER_ITEM].revents & ZMQ_POLLIN) {
            if (status == e_waiting) {
            ...
            if (have_command) {
                ...
                while (have_command &&
                    ...
                    while (i <= CommandSocketInfo::lastIndex() &&
                        ...
                        MessageHeader mh;
                        uint32_t default_id =
                            mh.getId(); // save the msgid to following check
                        if (safeRecv(*sock, &buf, &len, false, 0, mh)) {
                            ...
                            if (command) {
                                ...
                                if (mh.needsReply() ||
                                    ...
                                    snprintf(response, len + 40,
                                             "Unrecognised command: %s", buf);
                                    MessageHeader rh(mh);
                                    rh.source = mh.dest;
                                    rh.dest = mh.source;
                                    rh.start_time = microsecs();
        ...
        if (items[internals->SCHEDULER_ITEM].revents & ZMQ_POLLIN) {
#ifdef KEEPSTATS
            if (!stats.scheduler_delay.running()) {
                stats.scheduler_delay.start();
            }
#endif
            if (status == e_waiting && processing_state == eIdle) {
                ...
                if (len) {
                    ...
                    snprintf(
                        buf, 100,
                        "WARNING: scheduler sync returned zero length message");
                ...
                if (processing_state == ePollingMachines) {
                    ...
                    std::set<MachineInstance *> to_process;
                    {
                        ...
                        while (iter != runnable.end()) {
                            ...
                            if (!mi->enabled()) {
                                iter = runnable.erase(
                                    iter); // machine is disabled, it will get requeued when enabled again
                    ...
                    if (!to_process.empty()) {
                        DBG_SCHEDULER << "processing " << to_process.size()
                                      << " machines\n";
                        MachineInstance::processAll(
                            to_process, 150000, MachineInstance::NO_BUILTINS);
                    }
                    ...
                    std::set<MachineInstance *> to_process;
                    {
                        ...
                        while (iter != runnable.end()) {
                            ...
                            if (mi->executingCommand() ||
                                !mi->pendingEvents().empty() || mi->hasMail()) {
                                iter++;
                                continue;
                            }
                            if (mi->queuedForStableStateTest()) {
        ...
        } else if (status == e_waiting && machine_is_ready &&
                   !IOComponent::devices.empty() &&
                   (IOComponent::updatesWaiting() ||
                    IOComponent::getHardwareState() !=
                        IOComponent::s_operational)) {
#ifdef KEEPSTATS
        ...
        machine
            .idle(); // in case any of the above triggered a change to the machine state
        last_machine_change = machine.lastUpdated();
        if (program_done) {
            break;
        }
