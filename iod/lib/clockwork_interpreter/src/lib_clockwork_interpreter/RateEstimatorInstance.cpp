...
void RateEstimatorInstance::idle() {
    ...
    if (is_enabled) { // && hasWork()) {
        double delta = 0;
        uint64_t curr_t = (pos_m->io_interface) ? pos_m->io_interface->read_time
                                                : rate_calc_process_time;
...
RateEstimatorInstance::RateEstimatorInstance(CStringHolder name,
                                             const char *type,
...
bool RateEstimatorInstance::setValue(const std::string &property,
                                     const Value &update, uint64_t authority) {
