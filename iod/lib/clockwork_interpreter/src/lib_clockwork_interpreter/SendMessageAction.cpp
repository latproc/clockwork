...
SendMessageActionTemplate::SendMessageActionTemplate(Value msg,
                                                     MachineInstance *dest,
                                                     bool should_throw)
    : message(msg), target(dest->fullName()), target_machine(dest),
      abort_after(should_throw) {}
...
std::ostream &SendMessageActionTemplate::operator<<(std::ostream &out) const {
    return out << ((abort_after) ? "ThrowMessageActionTemplate"
                                 : "SendMessageActionTemplate ")
               << message << " " << ((target != 0) ? target : "");
}

void SendMessageActionTemplate::toC(std::ostream &out,
                                    std::ostream &vars) const {
...
SendMessageAction::SendMessageAction(MachineInstance *mi,
                                     SendMessageActionTemplate &eat)
    : Action(mi), message(eat.message), target(eat.target),
      target_machine(eat.target_machine), abort_after(eat.abort_after) {}
...
Action::Status SendMessageAction::run() {
    ...
    if (target != 0) {
        ...
        if (target_machine) {
            ...
            owner->sendMessageToReceiver(new Message(msg_str.c_str()),
                                         target_machine);
            if (target_machine->_type == "LIST" && target_machine->enabled()) {
                for (unsigned int i = 0; i < target_machine->parameters.size();
                     ++i) {
                    MachineInstance *entry =
                        target_machine->parameters[i].machine;
                    if (entry) {
                        owner->sendMessageToReceiver(
                            new Message(msg_str.c_str()), entry);
                    }
                }
            } else if (target_machine->_type == "REFERENCE" &&
                       target_machine->enabled() &&
                       target_machine->locals.size()) {
                for (unsigned int i = 0; i < target_machine->locals.size();
                     ++i) {
