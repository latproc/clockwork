...
MachineDetails::MachineDetails(const char *nam, const char *cls,
                               std::list<Parameter> &params, const char *sf,
                               int sl, SymbolTable &props,
                               MachineInstance::InstanceType kind)
...
MachineInstance *MachineDetails::instantiate() {
    MachineInstance *machine = MachineInstanceFactory::create(
        machine_name.c_str(), machine_class.c_str(), instance_type);
    machine->setDefinitionLocation(source_file.c_str(), source_line);
    std::copy(parameters.begin(), parameters.end(),
              back_inserter(machine->parameters));
