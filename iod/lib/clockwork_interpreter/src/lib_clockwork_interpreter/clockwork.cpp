...
ClockworkInterpreter::ClockworkInterpreter()
    : cycle_delay(0), default_poll_delay(0) {
...
void ClockworkInterpreter::setup(MachineInstance *new_settings) {
    ...
    cycle_delay =
        &ClockworkInterpreter::instance()->settings()->getValue("CYCLE_DELAY");
    default_poll_delay =
        ClockworkInterpreter::instance()->settings()->getMutableValue(
            "POLLING_DELAY");
...
void usage(int argc, char const *argv[]) {
    std::cerr << "Usage: " << argv[0]
              << " [-v] [-t] [-l logfilename] [-i persistent_store]\n"
...
static void listDirectory(const std::string pathToCheck,
...
int load_preset_modbus_mappings() {
    ...
    if (!modbus_mappings_file) {
        ...
        int max_input = 0;
        int max_holding = 0;
        bool generate_length =
            false; // backward compatibility for old mappings files
        while (modbus_mappings_file.getline(buf, 200, '\n')) {
            ...
            if (pos != std::string::npos) {
                ...
                if (ModbusAddress::preset_modbus_mapping.count(lookup_name) ==
                    0) {
                    int group_num, addr_num;
                    char *end = 0;
                    group_num = (int)strtol(group.c_str(), &end, 10);
                    if (*end) {
                        std::cerr << "error loading modbus mappings from "
                                  << modbus_map() << " at line " << lineno
                                  << " " << *end << "not expected\n";
                        ++errors;
                        continue;
                    }
                    ...
                    DBG_MODBUS << "Loaded modbus mapping " << group_num << " "
                               << addr << " " << length << "\n";
                    ModbusAddressDetails details(group_num, addr_num,
                                                 exportType, length);
...
void semantic_analysis() {
    ...
    while (iter != machines.end()) {
        ...
        if (!m) {
            std::stringstream ss;
            ss << "## - Warning: machine table entry " << (*iter).first
               << " has no machine";
            ++num_errors;
    ...
    while (m_iter != MachineInstance::end()) {
        ...
        std::map<std::string, MachineClass *>::iterator c_iter =
            machine_classes.find(m->_type);
        if (c_iter == machine_classes.end()) {
            std::stringstream ss;
            ss << "## - Error: class " << m->_type << " not found for machine "
               << m->getName() << "\n";
            error_messages.push_back(ss.str());
            ++num_errors;
            ...
            std::stringstream ss;
            ss << "Error: no state machine defined for instance "
               << (*c_iter).first;
            error_messages.push_back(ss.str());
            ++num_errors;
            NB_MSG << ss.str() << "\n";
    ...
    while (m_iter != MachineInstance::end()) {
        ...
        DBG_PARSER << "Machine " << mi->getName() << " (" << mi->_type
                   << ") has " << mi->parameters.size() << " parameters\n";
        ...
        if (mi->getStateMachine() &&
            ...
            if (mi->getStateMachine()->name == "LIST") {
                DBG_PARSER << "List has " << mi->parameters.size()
                           << " parameters\n";
                ...
                std::stringstream ss;
                ss << "## - Error: Machine " << mi->getStateMachine()->name
                   << " requires " << mi->getStateMachine()->parameters.size()
        ...
        for (unsigned int i = 0; i < mi->parameters.size(); i++) {
            ...
            if (p_i.kind == Value::t_symbol) {
                ...
                MachineInstance *found = mi->lookup(
                    mi->parameters
                        [i]); // uses the real_name field and caches the result
                if (found) {
                    // special check of parameter types for points
                    if (mi->_type == "POINT" && i == 0 &&
                        found->_type != "MODULE" &&
                        found->_type != "MQTTBROKER") {
                        std::stringstream ss;
                        ss << "Error: in the definition of " << mi->getName()
                           << ", " << p_i.sValue << " has type " << found->_type
                    ...
                    const Value &prop =
                        mi->getValue(mi->parameters[i].real_name);
        ...
        // parameters
        DBG_PARSER << "fixing parameter references for locals in "
                   << mi->getName() << "\n";
            ...
            for (unsigned int j = 0; j < m->parameters.size(); ++j) {
                ...
                if (p.val.kind == Value::t_symbol) {
                    ...
                    if (p.machine) {
                        ...
                        DBG_PARSER << " linked parameter " << j << " of local "
                                   << m->getName() << " ("
                                   << m->parameters[j].val << ") to "
                                   << p.machine->getName() << "\n";
                    } else {
                        std::stringstream ss;
                        ss << "## - Error: local " << m->getName()
                           << " for machine " << mi->getName()
                           << " sends an unknown parameter " << p.val
                           << " at position: " << j << "\n";
                        error_messages.push_back(ss.str());
                        ++num_errors;
                    }
        ...
        if (mi->getStateMachine()) {
            BOOST_FOREACH (StableState &ss, mi->stable_states) {
                ...
                if (ss.subcondition_handlers) {
                    BOOST_FOREACH (ConditionHandler &ch,
                                   *ss.subcondition_handlers) {
        ...
        if (mi->getStateMachine()) {
            ...
            BOOST_FOREACH (node, mi->getStateMachine()->global_references) {
                if (node.second) {
                    ...
                    snprintf(
                        buf, 100,
                        "machine %s cannot find a global machine called: %s\n",
    ...
    while (m_iter != MachineInstance::end()) {
        ...
        BOOST_FOREACH (rcv, to_add) {
            ...
            if (!source) {
                DBG_MSG << "Unknown machine when checking to duplicate "
                        << rcv.first.getText() << "\n";
            }
            ...
            if (source && source->getName() != machine) {
                ...
                mi->receives_functions.insert(
                    std::make_pair(Message(event.c_str()), rcv.second));
            }
        }
    }
...
int loadOptions(int argc, const char *argv[], std::list<std::string> &files) {
    ...
    while (i < argc) {
        ...
        */
                 ...
                 i < argc - 1) { // initialise from persistent store
            char buf[200];
            ...
            */
                   ...
                   i < argc - 1) { // debug config file
            set_debug_config(argv[++i]);
        } else if (strcmp(argv[i], "-m") == 0 &&
                   i < argc - 1) { // modbus mapping file
            set_modbus_map(argv[++i]);
        } else if (strcmp(argv[i], "-g") == 0 &&
                   i < argc - 1) { // dependency graph file
            set_dependency_graph(argv[++i]);
        } else if (strcmp(argv[i], "-r") == 0 &&
                   i < argc - 1) { // dependency graph root
            set_graph_root(argv[++i]);
        } else if (strcmp(argv[i], "-p") == 0 &&
                   i < argc - 1) { // publisher port
            set_publisher_port((int)strtol(argv[++i], 0, 10), true);
        } else if (strcmp(argv[i], "-ps") == 0 &&
                   i < argc - 1) { // persistent store port
        ...
        } else if (strcmp(argv[i], "--exceptions") ==
                   0) { // enable exceptions on action failues
...
void initialise_machines() {
    ...
    if (persistent_store()) {
        ...
        while (m_iter != MachineInstance::end()) {
            MachineInstance *m = *m_iter++;
            if (m && (m->_type == "CONSTANT" ||
                      m->getValue("PERSISTENT") == "true")) {
                ...
                std::map<std::string, std::map<std::string, Value>>::iterator
                    found = store.init_values.find(name);
                if (found != store.init_values.end()) {
                    ...
                    BOOST_FOREACH (node, list) {
                        long v;
                        double d;
                        DBG_INITIALISATION << name << " initialising "
                                           << node.first << " to "
                                           << node.second << "\n";
