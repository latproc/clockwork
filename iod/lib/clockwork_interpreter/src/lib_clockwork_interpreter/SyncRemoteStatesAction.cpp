...
class SyncRemoteStatesActionInternals {
  public:
    std::list<char *>
        messages; // encoded messages (free() required on completion)
    ...
    SyncRemoteStatesActionInternals()
        : header(MessageHeader::SOCK_CW, MessageHeader::SOCK_CHAN, false),
          iter(0), chn(0), sock(0), message_state(e_sending),
          process_state(ps_init) {
        //(ChannelInternals::SOCK_CHAN, ChannelInternals::SOCK_CHAN, true);
    }
};

SyncRemoteStatesActionTemplate::SyncRemoteStatesActionTemplate(Channel *channel,
                                                               zmq::socket_t *s)
    : chn(channel), sock(s) {}

SyncRemoteStatesAction::SyncRemoteStatesAction(
    MachineInstance *mi, SyncRemoteStatesActionTemplate &t)
    : Action(mi) {
...
Action::Status SyncRemoteStatesAction::execute() {
    ...
    if (internals->process_state == SyncRemoteStatesActionInternals::ps_init) {
        ...
        internals->process_state =
            SyncRemoteStatesActionInternals::ps_sending_messages;
        if (chn->syncRemoteStates(internals->messages)) {
            internals->iter =
                new std::list<char *>::iterator(internals->messages.begin());
            if (*internals->iter != internals->messages.end()) {
                internals->message_state =
                    SyncRemoteStatesActionInternals::e_sending;
            } else {
                internals->message_state =
                    SyncRemoteStatesActionInternals::e_done;
            }
        } else {
            status = Failed;
    ...
    if (internals->process_state ==
        ...
        if (internals->message_state ==
            ...
            safeSend(*internals->sock, current_message, strlen(current_message),
                     internals->header);
            free(current_message);
            *internals->iter = internals->messages.erase(*internals->iter);
            internals->message_state =
                SyncRemoteStatesActionInternals::e_receiving;
            // skip receiving temporarily
            internals->message_state = SyncRemoteStatesActionInternals::e_done;
        } else if (internals->message_state ==
                   SyncRemoteStatesActionInternals::e_receiving) {
            char *repl;
            size_t len;
            if (safeRecv(*internals->sock, &repl, &len, false, 0,
                         internals->header)) {
        ...
        if (internals->message_state ==
            ...
            if (!internals->iter) { // finished sending messages
                if (internals->process_state ==
                    ...
                    internals->process_state =
                        SyncRemoteStatesActionInternals::ps_waiting_ack;
                }
            }
        }
    } else if (internals->process_state ==
               SyncRemoteStatesActionInternals::ps_waiting_ack) {
        ...
        char *ack;
        size_t len;
        if (safeRecv(*internals->sock, &ack, &len, false, 0,
                     internals->header)) {
            DBG_CHANNELS << "channel " << chn->name << " got " << ack
                         << " from server\n";
            status = Complete;
