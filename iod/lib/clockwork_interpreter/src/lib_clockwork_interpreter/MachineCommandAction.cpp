...
void MachineCommandTemplate::setActionTemplates(
    std::list<ActionTemplate *> &new_actions) {
...
std::ostream &MachineCommand::operator<<(std::ostream &out) const {
    out << "Command " << owner->getName() << "." << command_name << " (at step "
        << current_step << ")";
...
Action::Status MachineCommand::runActions() {
    while (current_step < actions.size()) {
        ...
        if (!a->complete()) {
            DBG_M_ACTIONS << "leaving action: " << *a
                          << " to run for a while\n";
    ...
    DBG_M_ACTIONS << owner->getName() << " " << *this
