...
struct MQTTTopic {
    ...
    MQTTTopic(const std::string &t, const std::string &m)
        : topic(t), message(m) {}
};
...
class IOComponent : public Transmitter {
  public:
    ...
    static IOAddress add_io_entry(const char *name, unsigned int module_pos,
                                  ...
                                  unsigned int entry_offs,
                                  unsigned int bit_len = 1,
                                  bool is_signed = false);
    static void add_publisher(const char *name, const char *topic,
                              const char *message);
    static void add_subscriber(const char *name, const char *topic);
    static void processAll(uint64_t clock, size_t data_size, uint8_t *mask,
                           uint8_t *data,
  ...
  public:
    ...
    virtual void setupProperties(
        MachineInstance *
            m); // link properties in the component to the MachineInstance properties
