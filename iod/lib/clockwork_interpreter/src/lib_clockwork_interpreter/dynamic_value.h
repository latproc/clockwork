...
class DynamicValue : public DynamicValueBase {
  public:
    ...
    virtual const Value &
    operator()(MachineInstance *scope); // uses the provided machine's scope
    ...
    static DynamicValue *ref(DynamicValue *dv) {
        if (!dv)
            return 0;
        else
            dv->refs++;
  ...
  protected:
    ...
    DynamicValue(const DynamicValue &other)
        : last_result(other.last_result), scope(0), refs(1) {}
...
class ItemAtPosValue : public DynamicValue {
  public:
    ItemAtPosValue(const char *list, Value *val, bool remove = true)
        : index(*val), machine_list_name(list), machine_list(0),
...
class CastValue : public DynamicValue {
  public:
    CastValue(const char *name, const char *type_name)
        : property(name), kind(type_name) {}
    virtual ~CastValue() {}
    virtual Value &operator()(MachineInstance *);
    virtual DynamicValue *clone() const;
    virtual std::ostream &operator<<(std::ostream &) const;
