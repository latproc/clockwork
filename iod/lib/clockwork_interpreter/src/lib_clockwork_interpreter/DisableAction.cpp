...
DisableActionTemplate::DisableActionTemplate(const std::string &name,
                                             const char *property,
                                             const Value *val)
...
DisableAction::DisableAction(MachineInstance *m,
                             const DisableActionTemplate *dat)
    : Action(m), machine_name(dat->machine_name), machine(0), property_name(0),
      property_value(0) {
...
Action::Status DisableAction::run() {
    ...
    if (machine) {
        ...
        char buf[100];
        snprintf(buf, 100, "DisableAction Error: cannot find %s from %s",
                 machine_name.c_str(), owner->getName().c_str());
