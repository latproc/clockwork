...
#define toU16(m, v)                                                            \
    *((uint16_t *)m) =                                                         \
        ((((uint16_t)v) & 0xff00) >> 8) | ((((uint16_t)v) & 0x00ff) << 8)
#define fromU16(m)                                                             \
    (((*(uint16_t *)m) & 0xff00) >> 8) | ((*((uint16_t *)m) & 0x00ff) << 8)
#define toU32(m, v)                                                            \
...
boost::unique_lock<boost::recursive_mutex> io_lock(processing_queue_mutex,
                                                   boost::defer_lock);
...
IOComponent::HardwareState IOComponent::getHardwareState() {
...
IOComponent::IOComponent()
    : last_event(e_none), io_index(-1), raw_value(0),
      direction_(DirBidirectional) {
    boost::recursive_mutex::scoped_lock lock(processing_queue_mutex);
    processing_queue.push_back(this);
...
IOComponent *IOComponent::lookup_device(const std::string name) {
    IOComponent::DeviceList::iterator device_iter =
        IOComponent::devices.find(name);
...
void IOComponent::processAll(uint64_t clock, size_t data_size, uint8_t *mask,
                             uint8_t *data,
...
IOAddress IOComponent::add_io_entry(const char *name, unsigned int module_pos,
                                    ...
                                    unsigned int bit_len, bool signed_value) {
...

    {
        ...
        if (io_names.find(std::string(buf)) != io_names.end()) {
            std::cerr
                << "IOComponent::add_io_entry: warning - an IO component named "
...
void IOComponent::add_publisher(const char *name, const char *topic,
                                const char *message) {
...
std::ostream &IOComponent::operator<<(std::ostream &out) const {
    out << "readtime: " << (io_clock - read_time) << " [" << address.description
        << ", " << address.module_position << " " << address.io_offset << ':'
        << address.io_bitpos << "." << address.bitlen << "]=" << address.value;
    if (address.bitlen == 1 && io_process_data) {
        out << " ("
            << (bool)(io_process_data[address.io_offset] &
                      (1 << address.io_bitpos))
            << ")";
    }
    return out;
...
class InputFilterSettings {
  public:
    ...
    double
        last_sent; // this is the value to send unless the read value moves away from the mean
    ...
    const long *
        filter_type; // the user can select the filter using a "filter" property
    const long *calc_dt;
    const long *calc_d2t;
    ...
    unsigned int
        butterworth_len; // the number of coefficients in the Butterworth filter
    ...
    static long
        default_speed_filter_len; // a default value for speed_filter_len
    static long default_calc_dt;
    static long default_calc_d2t;
    static long default_calc_stddev;
    static long
        default_position_history;        // a default value for position_history
    ...
    InputFilterSettings()
        : property_changed(true), positions(0), last_sent(0.0), prev_sent(0.0),
          last_time(0), buffer_len(200), tolerance(&default_tolerance),
          filter_c_coeff(0), filter_d_coeff(0), filter_len(&default_filter_len),
          filter_type(0), calc_dt(&default_calc_dt),
          calc_d2t(&default_calc_d2t), calc_stddev(&default_calc_stddev),
          position_history(&default_position_history),
          speed_tolerance(&default_speed_tolerance), speed(0.0),
          speed_scale(1.0), accel(0.0), accel_scale(1.0), speeds(4),
          rate_len(4), input_bwf(0), vel_bwf(0), accel_bwf(0), throttle(0) {
        ...
        double bw_c[] = {0.002898194633721, 0.008694583901164,
                         0.008694583901164, 0.002898194633721};
        double bw_d[] = {1.000000000000, -2.374094743709, 1.929355669091,
                         -0.532075368312};
        ...
        input_bwf =
            new ButterworthFilter(butterworth_len, bw_c, butterworth_len, bw_d);
        vel_bwf =
            new ButterworthFilter(butterworth_len, bw_c, butterworth_len, bw_d);
        accel_bwf =
            new ButterworthFilter(butterworth_len, bw_c, butterworth_len, bw_d);
...
#define SECOND_DERIV_NORM 42.0
        ...
        if (last_time == 0) {
            ...
            speed = 0.0;
            speeds.append(speed);
        } else if ((long)(read_time - last_time) >=
            ...
            if (*calc_dt) {
                speed = savitsky_golay_filter(positions, smoothing_len,
            ...
            if (*calc_d2t) {
                ...
                accel = savitsky_golay_filter(positions, smoothing_len,
    ...
    double filter() {
        ...
        double c[] = {0.081, 0.215, 0.541, 0.865, 1,
                      0.865, 0.541, 0.215, 0.081};
        double res = 0;
...
long InputFilterSettings::default_tolerance =
    8; // a default value for filter_len
long InputFilterSettings::default_filter_len =
    12; // a default value for filter_len
long InputFilterSettings::default_speed_filter_len =
    4; // a default value for speed_filter_len
long InputFilterSettings::default_position_history =
    20; // a default value for position_history
long InputFilterSettings::default_speed_tolerance =
    20; // a default value for speed_tolerance
...
static bool getFloatValue(MachineInstance *scope, const char *name,
                          double &result) {
...
void AnalogueInput::setupProperties(MachineInstance *m) {
    ...
    if (settings) {
        ...
        if (getFloatValue(settings, "velocity_scale", speed_scale)) {
            ...
            std::cout << m->getName()
                      << " set velocity scale to: " << config->speed_scale
                      << "\n";
        }
        ...
        if (c_coeff && d_coeff) {
            ...
            if (num_c > 0 && num_c == num_d) {
                ...
                for (unsigned int i = 0; i < num_c; ++i) {
                    double val;
                    config->filter_c_coeff[i] =
                        c_coeff->parameters[i].val.asFloat(val) ? val : 1.0;
                    std::cout << " " << config->filter_c_coeff[i];
                }
                std::cout << "\nD: " << (int)num_d;
                for (unsigned int i = 0; i < num_d; ++i) {
                    double val;
                    config->filter_d_coeff[i] =
                        d_coeff->parameters[i].val.asFloat(val) ? val : 1.0;
                    std::cout << " " << config->filter_d_coeff[i];
                }
                std::cout << "\n";
                // swap the filter, TBD copy current values from old filter?
                ButterworthFilter *input_bwf =
                    new ButterworthFilter(num_c, config->filter_c_coeff, num_d,
                                          config->filter_d_coeff);
        ...
        if (v2.kind == Value::t_integer) {
            ...
            if (*config->filter_type == 2) {
                std::cout << "butterworth filter order "
                          << config->butterworth_len << "\n";
            }
        }
        ...
        std::cout << "Warning: analog input " << m->getName()
                  << " has no filter settings\n";
    }
}
...
int32_t AnalogueInput::filter(int32_t raw) {
    ...
    if (config->filter_type && *config->filter_type == 0) { //TBD wrong?
        config->last_sent = raw;
               ...
               (config->filter_type && *config->filter_type == 1)) {
        int32_t mean =
            (bufferAverage(config->positions, *config->filter_len) + 0.5f);
    ...
    */
    ...
    while (owners_iter != owners.end()) {
        ...
        o->properties.add("DurationTolerance", config->rate_len,
                          SymbolTable::ST_REPLACE);
        o->properties.add("raw", (long)raw, SymbolTable::ST_REPLACE);
        o->properties.add("VALUE", (long)config->last_sent,
                          SymbolTable::ST_REPLACE);
        ...
        if (*config->calc_d2t) {
            o->properties.add("Acceleration",
                              config->accel * config->accel_scale,
                              SymbolTable::ST_REPLACE);
        }
    }
...
class CounterInternals {
  public:
    ...
    int32_t
        last_sent; // this is the value to send unless the read value moves away from the mean
    int32_t
        prev_sent; // this is the value to send unless the read value moves away from the mean
    uint64_t last_time; // the last time we calculated speed;_
    ...
    CounterInternals()
        : positions(0), tolerance(&default_tolerance),
          filter_len(&default_filter_len),
          position_history(&default_position_history),
          speed_tolerance(&default_speed_tolerance),
          input_scale(&default_input_scale), last_sent(0), prev_sent(0),
          last_time(0), speed(0), buffer_len(200), speeds(4), rate_len(4) {
        positions = createBuffer(buffer_len);
    }
    ...
    void update(uint64_t read_time) {
        ...
        if (last_time == 0) {
            ...
            */
            ...
            if (rate_len < *position_history) {
                speed =
                    1000000.0 * rate(positions, (rate_len < 4) ? 4 : rate_len);
            } else {
                speed = 0.0;
            }
            speeds.append(speed);
...
int32_t Counter::filter(int32_t val) {
    ...
    if (*internals->tolerance > 1) {
        int32_t mean =
            (bufferAverage(internals->positions, *internals->filter_len) +
             0.5f);
        ...
        internals->last_sent =
            (*internals->input_scale == 1) ? val : (uint32_t)(scaled_val + 0.5);
    }
    ...
    */
    ...
    while (owners_iter != owners.end()) {
        MachineInstance *o = *owners_iter++;
        o->properties.add("IOTIME", (long)read_time, SymbolTable::ST_REPLACE);
        o->properties.add("DurationTolerance", internals->rate_len,
                          SymbolTable::ST_REPLACE);
        o->properties.add("VALUE", (long)scaled_val, SymbolTable::ST_REPLACE);
        o->properties.add("Position", (long)internals->last_sent,
                          SymbolTable::ST_REPLACE);
        o->properties.add(
            "Velocity",
            (long)internals->speeds.average(internals->speeds.length()),
            SymbolTable::ST_REPLACE);
    }
#endif
...
CounterRate::CounterRate(IOAddress addr)
    : IOComponent(addr), times(16), positions(0) {
    start_t = microsecs();
}
...
class PID_Settings {
  public:
    ...
    PID_Settings()
        : property_changed(true), max_forward(16383), max_reverse(-16383),
          Kp(20.0), set_point(0), estimated_speed(0.0f), Pe(0.0f), current(0),
          last_pos(0.0f), position(0), measure_time(0), last_time(0),
          last_power(0), slow_speed(200), full_speed(1000), stopping_time(300),
          stopping_distance(2000), tolerance(10), min_update_time(20),
          deceleration_allowance(20), deceleration_rate(0.8f) {}
};
...
bool IOComponent::ownersEnabled() const {
    if (owners.empty()) {
        ...
        std::list<MachineInstance *>::const_iterator owners_iter =
            owners.begin();
        while (owners_iter != owners.end()) {
            MachineInstance *o = *owners_iter++;
            if (o->enabled()) {
...
IOUpdate *IOComponent::getUpdates() {
    ...
    //MEMCHECK();
#if VERBOSE_DEBUG
    std::cout << std::flush << "IOComponent::getUpdates preparing to send "
              << res->size() << " d:";
...
IOUpdate *IOComponent::getDefaults() {
    ...
    assert(default_data);
    assert(default_mask);
    copyMaskedBits(io_process_data, default_data, default_mask,
                   process_data_size);
...
void IOComponent::setupIOMap() {
    ...
    std::cout << ((max_offset + 1) * sizeof(IOComponent *))
              << " bytes reserved for index io\n";
    ...
    if (address.bitlen == 1) {
        ...
        if (address.value != value) { // TBD is this test necessary?
            ...
            while (owner_iter != owners.end()) {
                ...
                work_queue.push_back(new Package(
                    this, *owner_iter++, new Message(evt, Message::LEAVEMSG)));
            }
#endif
            if (value) {
                evt = "on_enter";
            ...
            while (owner_iter != owners.end()) {
                work_queue.push_back(new Package(
                    this, *owner_iter++, new Message(evt, Message::ENTERMSG)));
            }
        }
        address.value = value;
    } else {
        ...
        if (regular_polls.count(this)) {
            ...
            address.value = val;
                   ...
                   (hardware_state == s_operational &&
                    raw_value != (uint32_t)val)) {
            //std::cerr << "raw io value changed from " << raw_value << " to " << val << "\n";
            raw_value = val;
            int32_t new_val = filter(val);
            if (hardware_state ==
                s_operational) { //&& address.value != new_val) {
