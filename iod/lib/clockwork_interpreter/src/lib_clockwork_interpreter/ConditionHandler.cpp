...
ConditionHandler::ConditionHandler(const ConditionHandler &other)
    : condition(other.condition), command_name(other.command_name),
      flag_name(other.flag_name), timer_val(other.timer_val),
      action(other.action), trigger(other.trigger),
      uses_timer(other.uses_timer), triggered(other.triggered) {
    if (trigger) {
        trigger->retain();
    }
}
...
bool ConditionHandler::check(MachineInstance *machine) {
    ...
    if (command_name == "FLAG") {
        ...
        if (!flag) {
            std::cerr << machine->getName() << " error: flag " << flag_name
                      << " not found\n";
        } else {
            if (condition(machine)) {
                if (strcmp("on", flag->getCurrentStateString(machine))) {
                    if (tracing() && machine->isTraceable()) {
                        ...
                        machine->ss << machine->getCurrentStateString(machine)
                                    << " " << *condition.predicate;
                        machine->setValue(
                            "TRACE", Value(machine->ss.str(), Value::t_string));
                    }
                ...
                if (!flag->isActive()) {
                    ...
                    SetStateAction *ssa =
                        dynamic_cast<SetStateAction *>(ssat.factory(flag));
                    flag->enqueueAction(ssa);
                }
            }
        }
    } else if (!triggered && ((trigger && trigger->fired()) || !trigger) &&
               condition(machine)) {
        triggered = true;
        DBG_AUTOSTATES << machine->getName()
                       << " subcondition triggered: " << command_name << " "
                       << *(condition.predicate) << "\n";
        ...
        if (!trigger) {
            DBG_AUTOSTATES << "    condition does not have a timer\n";
        }
        if (triggered) {
            DBG_AUTOSTATES << "     condition " << (condition.predicate)
                           << " already triggered\n";
