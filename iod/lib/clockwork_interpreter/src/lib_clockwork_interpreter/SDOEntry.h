...
class SDOEntry {
  public:
    ...
    SDOEntry(std::string nam, uint16_t index, uint8_t subindex,
             const uint8_t *data, size_t size, uint8_t offset = 0)
#ifndef EC_SIMULATOR
        ; // implemented in ECInterface.cpp
#else
        : name(nam), module_(0), index_(0), subindex_(0), offset_(0), data_(0),
          size_(0), realtime_request(0), sync_done(false), error_count(0) {
    }
#endif
    ~SDOEntry();
  ...
  private:
    ...
    bool
        sync_done; // set value will not change the io until the entre has been synced
