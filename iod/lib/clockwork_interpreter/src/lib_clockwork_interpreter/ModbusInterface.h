...
class ModbusAddressDetails {
  public:
    ...
    ModbusAddressDetails(short grp, short addr, ModbusExport::Type exportType,
                         short n)
...
class ModbusAddress {
  public:
    ...
    ModbusAddress()
        : group(none), address(0), allocated(0), exportType(ModbusExport::none),
          offset(0), source(unknown) {}
    ...
    static ModbusAddress alloc(Group g, unsigned int n,
                               ModbusAddressable *owner, Source src,
                               const std::string &full_name,
                               ModbusExport::Type exType);

    void update(MachineInstance *owner, Group g, int addr, const char *value,
                int len);
    void update(MachineInstance *owner, Group g, int addr, double new_value,
                int len);
    void update(MachineInstance *owner, Group g, int addr, int new_value,
                int len);
...
class ModbusAddressable {
  public:
    ...
    virtual void modbusUpdated(ModbusAddress &addr, unsigned int offset,
                               int new_value) {
    ...
    virtual int getModbusValue(ModbusAddress &addr, unsigned int offset,
                               int len) = 0;
};
