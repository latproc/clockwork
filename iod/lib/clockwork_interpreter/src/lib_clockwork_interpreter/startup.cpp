...
void predefine_special_machines() {
    ...
    settings_class->properties.add("INFO", "Clockwork host",
    ...
    settings_class->properties.add("CYCLE_DELAY", 2000,
                                   SymbolTable::ST_REPLACE);
    settings_class->properties.add("POLLING_DELAY", 2000,
                                   SymbolTable::ST_REPLACE);
    ...
    ain_class->properties.add("Acceleration", Value(0.0),
                              SymbolTable::ST_REPLACE);
    ...
    module_class->transitions.push_back(
        Transition(State("INIT"), State("OP"), Message("turnOn")));
    module_class->transitions.push_back(
        Transition(State("INIT"), State("PREOP"), Message("powerUp")));
    module_class->transitions.push_back(
        Transition(State("PREOP"), State("OP"), Message("turnOn")));
    module_class->transitions.push_back(
    ...
    flag->transitions.push_back(
        Transition(State("off"), State("on"), Message("turnOn")));
    flag->transitions.push_back(
        Transition(State("on"), State("off"), Message("turnOff")));
    ...
    MachineInstance *miwc = MachineInstanceFactory::create(
        "ETHERCAT_WC", "ETHERCAT_WORKINGCOUNTER");
    ...
    MachineInstance *mils =
        MachineInstanceFactory::create("ETHERCAT_LS", "ETHERCAT_LINKSTATUS");
    ...
    MachineInstance *miec =
        MachineInstanceFactory::create("ETHERCAT", "ETHERCAT_BUS");
    ...
    MachineInstance *settings =
        MachineInstanceFactory::create("SYSTEM", "SYSTEMSETTINGS");
    ...
    MachineInstance *channels =
        MachineInstanceFactory::create("CHANNELS", "LIST");
