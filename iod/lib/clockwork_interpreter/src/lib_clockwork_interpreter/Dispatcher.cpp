...
Dispatcher::Dispatcher()
    : socket(0), started(false), dispatch_thread(0), thread_ref(0),
        ...
        if (status == e_waiting_cw) {
            ...
            while (!to_deliver.empty()) {
                ...
                to_deliver.pop_front();
                {
                    ...
                    if (to) {
                        MachineInstance *mi =
                            dynamic_cast<MachineInstance *>(to);
                        ...
                        if (!chn && mi && mi->getStateMachine() &&
                            mi->getStateMachine()->token_id ==
                                ClockworkToken::EXTERNAL) {
                            DBG_DISPATCHER
                                << "Dispatcher sending external message " << *p
                                << " to " << to->getName() << "\n";
                            {
                                ...
                                Value port_val =
                                    remote->properties.lookup("PORT");
                                Value protocol =
                                    mi->properties.lookup("PROTOCOL");
                                long port;
                                if (port_val.asInteger(port)) {
                                    if (protocol == "RAW") {
                                        MessagingInterface *mif =
                                            MessagingInterface::create(
                                                host.asString(), (int)port,
                                                eRAW);
                                        if (!mif->started()) {
                                            mif->start();
                                        }
                            ...
                            DBG_DISPATCHER << "Dispatcher queued " << *p
                                           << " to " << to->getName() << "\n";
                            to->enqueue(*p);
