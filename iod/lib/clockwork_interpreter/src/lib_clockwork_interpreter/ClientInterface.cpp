...
IODCommandThread *IODCommandThread::instance() {
    if (!instance_) {
        instance_ = new IODCommandThread;
    }
    return instance_;
}
...
struct IODCommandPersistentStateFactory : public IODCommandFactory {
    IODCommandPersistentState *create() {
        return new IODCommandPersistentState();
    }
};
struct IODCommandChannelRefreshFactory : public IODCommandFactory {
    IODCommandChannelRefresh *create() {
        return new IODCommandChannelRefresh();
    }
};
struct IODCommandSchedulerStateFactory : public IODCommandFactory {
    IODCommandSchedulerState *create() {
        return new IODCommandSchedulerState();
    }
};
...
struct CommandThreadInternals : public ClientInterfaceInternals {
  public:
    ...
    CommandThreadInternals()
        : socket(*MessagingInterface::getContext(), ZMQ_REP) {
...
void IODCommandThread::registerCommand(std::string name,
                                       IODCommandFactory *cmd) {
    CommandThreadInternals *cti = dynamic_cast<CommandThreadInternals *>(
        IODCommandThread::instance()->internals);
...
class MyMonitor : public zmq::monitor_t {
  public:
    ...
    virtual void on_event_connect_retried(const zmq_event_t &event_,
                                          const char *addr_) {
        DBG_MSG
            << "CI command channel command channel on_event_connect_retried "
            << addr_ << "\n";
    }
    ...
    virtual void on_event_accepted(const zmq_event_t &event_,
                                   const char *addr_) {
        DBG_MSG << "CI command channel on_event_accepted " << event_.value
                << " " << addr_ << "\n";
    }
    ...
    virtual void on_event_disconnected(const zmq_event_t &event_,
                                       const char *addr_) {
        DBG_MSG << "CI command channel on_event_disconnected " << event_.value
                << " " << addr_ << "\n";
    }
...
IODCommand *parseCommandString(const char *data) {
    ...
    if (count == 1 && ds == "LIST") {
    ...
    } else if (ds == "MODBUS" && (count == 2 || count == 3) &&
               params[1] == "EXPORT") {
        ...
        command = new IODCommandHelp;
               ...
               (ds == "CLEAR" && count == 2 && params[1] == "MESSAGES")) {
...
void IODCommandThread::operator()() {
    ...
    CommandThreadInternals *cti =
        dynamic_cast<CommandThreadInternals *>(internals);
    ...
    char url_buf[30];
    int retries =
        2; // attempt to use the default command port and auto allocate another if necessary
    ...
    while (retries > 0) {
        ...
        try {
            ...
            {
                NB_MSG << "Error: trying port " << port << ": "
                       << zmq_strerror(zmq_errno()) << "\n";
                FileLogger fl(program_name);
                usleep(10);
                fl.f() << "Error: trying port " << port << ": "
                       << zmq_strerror(zmq_errno()) << "\n";
                if (!command_port_fixed() && --retries > 0) {
                    port = Channel::uniquePort();
                    usleep(
                        100); // give time for the new port to become available
                    continue;
                }
                exit(1);
            }
        ...
        try {
            ...
            for (int i = 0; i < 3; ++i) {
                if (items[i].revents & ZMQ_POLLERR) {
                    char buf[150];
                    snprintf(buf, 150,
                             "Processing thread saw zmq error on socket %d", i);
            ...
            */
            ...
            if (items[0].revents & ZMQ_POLLIN) {
                ...
                char *data = (char *)malloc(
                    size + 1); // note: leaks if an exception is thrown
            ...
            if (!done) {
                {
                    FileLogger fl(program_name);
                    fl.f() << "zmq error: " << zmq_strerror(zmq_errno()) << "\n"
                           << std::flush;
                }
                if (errno) {
                    NB_MSG << "error during client communication: "
                           << strerror(errno) << "\n"
                           << std::flush;
                }
                if (zmq_errno()) {
