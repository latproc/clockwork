...
class MachineInstance : public Receiver,
                        public ModbusAddressable,
                        public TriggerOwner {
  ...
  public:
    ...
    void addParameter(const Parameter &param, MachineInstance *machine = 0,
                      int position = -1, bool before = false);
    ...
    static void add_io_entry(const char *name, unsigned int io_offset,
    ...
    virtual void sendMessageToReceiver(Message *m, Receiver *r = NULL,
    ...
    void resetStableStateFlags(
        bool changed_state,
        StableState
            *active_state);  // clear subcondition flags ready for state change
    virtual bool isShadow(); // is this machine a shadow instance?
    virtual Channel *ownerChannel();

    std::string &fullName() const;
    std::string modbusName(const std::string &property,
    ...
    Action *findHandler(Message &msg, Transmitter *t,
    ...
    static bool processAll(std::set<MachineInstance *> &to_process,
                           uint32_t max_time, PollType which);
    //static void updateAllTimers(PollType which);
    //void updateTimer(long dt);
    static bool checkStableStates(std::set<MachineInstance *> &to_process,
                                  uint32_t max_time);
    ...
    static std::list<MachineInstance *>::iterator begin() {
        return all_machines.begin();
    }
    static std::list<MachineInstance *>::iterator end() {
        return all_machines.end();
    }

    static std::list<MachineInstance *>::iterator io_modules_begin() {
        return io_modules.begin();
    }
    static std::list<MachineInstance *>::iterator io_modules_end() {
        return io_modules.end();
    }

    static std::list<MachineInstance *>::iterator begin_active() {
        return active_machines.begin();
    }
    static std::list<MachineInstance *>::iterator end_active() {
        return active_machines.end();
    }
    static void addActiveMachine(MachineInstance *m) {
    ...
    Trigger *setupTrigger(const std::string &machine_name,
                          const std::string &message, const char *suffix);
    ...
    void sendModbusUpdate(const std::string &property_name,
                          const Value &new_value);
    ModbusAddress addModbusExport(std::string name, ModbusAddress::Group g,
    ...
    void setupModbusPropertyExports(std::string property_name,
    ...
    void modbusUpdated(ModbusAddress &addr, unsigned int offset,
                       float new_value);
    void modbusUpdated(ModbusAddress &addr, unsigned int offset,
  ...
  protected:
    ...
    State current_state;
    State visible_state;
    uint64_t setupSubconditionTriggers(const StableState &s,
                                       uint64_t earliestTimer);
    Action *findReceiveHandler(Transmitter *from, const Message &m,
    ...
    std::vector<std::string>
        command_names; // used for mapping modbus offsets to states
    ...
    uint64_t
        last_state_evaluation_time; // dynamic value check against this before recalculating
    void *data;                     // plugin data
    uint64_t
        idle_time; // amount of time to be idle between state polls (microsec)
    uint64_t next_poll;
  ...
  protected:
    ...
    static std::list<MachineInstance *>
        active_machines; // machines that require idle() processing
    static std::list<MachineInstance *>
        shadow_machines; // machines that shadow remote machines
    static std::set<MachineInstance *>
        pending_state_change; // machines that need to check their stable states
    static std::set<MachineInstance *>
        plugin_machines; // machines that have plugins
    static std::list<MachineInstance *> io_modules; // machines of type MODULE
    static std::list<Package *>
        pending_events; // machines that shadow remote machines
...
class MachineValue : public DynamicValue {
  public:
    MachineValue(MachineInstance *mi, std::string name)
        : machine_instance(mi), local_name(name) {}
    ...
    Value &operator()(MachineInstance *m) {
        ...
        if (machine_instance->_type == "VARIABLE" ||
            machine_instance->_type == "CONSTANT") {
...
class MachineInstanceFactory {
  public:
    static MachineInstance *create(MachineInstance::InstanceType instance_type =
                                       MachineInstance::MACHINE_INSTANCE);
    static MachineInstance *create(CStringHolder name, const char *type,
