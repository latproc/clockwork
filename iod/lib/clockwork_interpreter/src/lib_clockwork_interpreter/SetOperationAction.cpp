...
static void debugParameterChange(MachineInstance *dest_machine) {
    ...
    for (unsigned int i = 0;
         n < bufsize - 2 && i < dest_machine->parameters.size(); ++i) {
        if (n + strlen(delim) +
                dest_machine->parameters[i].val.asString().length() >=
            bufsize - 2) {
            n = bufsize - 2;
            break;
        }
        ...
        n +=
            strlen(delim) + dest_machine->parameters[i].val.asString().length();
        delim = ",";
    }
...
SetOperationActionTemplate::SetOperationActionTemplate(
    Value num, Value a, Value b, Value destination, Value property,
    SetOperation op, Predicate *pred, bool remove, Value start, Value end)
    : count(num), src_a_name(a.asString()), src_b_name(b.asString()),
      dest_name(destination.asString()), property_name(property.asString()),
      operation(op), condition(pred), remove_selected(remove), start_pos(start),
      end_pos(end) {}
...
Action *SetOperationActionTemplate::factory(MachineInstance *mi) {
    switch (operation) {
    case soIntersect:
        return new IntersectSetOperation(mi, this);
    case soUnion:
        return new UnionSetOperation(mi, this);
    case soDifference:
        return new DifferenceSetOperation(mi, this);
    case soSelect:
        return new SelectSetOperation(mi, this);
    }
    return 0;
}

SetOperationAction::SetOperationAction(MachineInstance *m,
                                       const SetOperationActionTemplate *dat)
    : Action(m), scope(m), count(dat->count), source_a(dat->src_a_name),
      source_b(dat->src_b_name), dest(dat->dest_name),
      condition(dat->condition), property_name(dat->property_name),
      source_a_machine(0), source_b_machine(0), dest_machine(0),
      operation(dat->operation), remove_selected(dat->remove_selected),
      start_pos(dat->start_pos), end_pos(dat->end_pos) {
...
std::ostream &SetOperationAction::operator<<(std::ostream &out) const {
    return out << "Set Operation " << source_a << ", " << source_b << " to "
               << dest << "\n";
}
...
bool CompareValues(Value a, Value &b) {
    if (a == SymbolTable::Null || b == SymbolTable::Null) {
        throw SetOperationException();
    }
    return a == b;
}

bool CompareSymbolAndValue(MachineInstance *scope, Value &sym,
                           std::string &prop, Value &b) {
...
Action::Status SetOperationAction::doOperation() { return Failed; }
...
IntersectSetOperation::IntersectSetOperation(
    MachineInstance *m, const SetOperationActionTemplate *dat)
...
void setListItem(MachineInstance *list, const Value &item, int index,
                 bool &add_item) {
    if (add_item) {
    ...
    while (i < source_a_machine->parameters.size()) {
        ...
        for (unsigned int j = 0; j < source_b_machine->parameters.size(); ++j) {
            ...
            if (condition.predicate) {
                ...
                if (num_copied >= to_copy) {
                    goto doneIntersectOperation;
                }
                if (matched && remove_selected) {
                    goto intersectSkipToNextSourceItem; // skip the increment to next parameter
                }
                if (matched) {
                    break; // already matched this item, no need to keep looking
                }
            } else {
                ...
                } else if (b.kind == Value::t_symbol &&
                           (v1.kind == Value::t_string ||
                            v1.kind == Value::t_integer)) {
...
doneIntersectOperation:
    if (!track_a.add_item && !track_a.keep_item &&
        source_a_machine->locals.size()) {
        std::vector<Parameter>::iterator iter =
            source_a_machine->locals.begin();
        for (unsigned int i = 0; i < track_a.index; ++i, iter++) {
            ;
        }
        source_a_machine->locals.erase(iter);
    }
    if (!track_b.add_item && !track_b.keep_item &&
        source_b_machine->locals.size()) {
        std::vector<Parameter>::iterator iter =
            source_b_machine->locals.begin();
...
UnionSetOperation::UnionSetOperation(MachineInstance *m,
                                     const SetOperationActionTemplate *dat)
    : SetOperationAction(m, dat) {}
    ...
    if (source_a_machine != dest_machine)
        for (unsigned int i = 0; i < source_a_machine->parameters.size(); ++i) {
            ...
            if (a.kind == Value::t_symbol) {
                ...
                if (!MachineIncludesParameter(dest_machine, val)) {
                    dest_machine->addParameter(a);
                }
            } else {
                if (!MachineIncludesParameter(dest_machine, a)) {
...
DifferenceSetOperation::DifferenceSetOperation(
    MachineInstance *m, const SetOperationActionTemplate *dat)
    : SetOperationAction(m, dat) {}
...
SelectSetOperation::SelectSetOperation(MachineInstance *m,
                                       const SetOperationActionTemplate *dat)
    : SetOperationAction(m, dat) {
    /*
    */
...
Action::Status SelectSetOperation::doOperation() {
    ...
    {
        if (start_pos.kind == Value::t_symbol ||
            start_pos.kind == Value::t_string) {
            Value v = scope->properties.lookup(start_pos.sValue.c_str());
            if (!v.asInteger(sp)) {
                sp = -1;
            }
        } else {
            if (!start_pos.asInteger(sp)) {
                sp = -1;
            }
        }
        if (sp == -1) {
        ...
        if (end_pos.kind == Value::t_symbol ||
            end_pos.kind == Value::t_string) {
            Value v = scope->properties.lookup(end_pos.sValue.c_str());
            if (!v.asInteger(ep)) {
                ep = -1;
            }
        } else {
            if (!end_pos.asInteger(ep)) {
                ep = -1;
            }
        }
    ...
    if (source_a_machine) {
        ...
        } else if ((count.kind == Value::t_integer && count < 0) ||
                   !count.asInteger(to_copy)) {
            to_copy = source_a_machine->parameters.size();
        }
#ifdef DEPENDENCYFIX
        Value last;
        ...
        while (i < source_a_machine->parameters.size()) {
            ...
            if (a.kind == Value::t_symbol) {
                ...
                if ((!condition.predicate || condition(owner))) {
                    ...
                    if (remove_selected) {
                        continue; // skip the increment to next parameter
                    }
