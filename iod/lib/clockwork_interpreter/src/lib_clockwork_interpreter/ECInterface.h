...
#ifndef __ECInterface_h
#define __ECInterface_h
...
/* This is a generic interface to remote EtherCAT(tm) hardware. */

/*
    the entry details structure is used to gather data about an entry in a module
*/
class IOModule {
  public:
    virtual ~IOModule();
...
class EntryDetails_internal;
class EntryDetails {
  public:
    enum EntryAccess { RO, RW, WO };
    std::string name;
    uint16_t entry_index;
    uint8_t entry_subindex;
    EntryAccess access;
    uint16_t bit_length;
    EntryDetails_internal *internals;
};
...
class ECInterface {
  public:
    ...
    static uint64_t
        master_last_checked; // time the master status was last checked
    static uint64_t
        master_state_changed; // time a last state change was detected in the master
    ...
    void
    receiveState(); // get state from EtherCAT, use collectState() to process it
    int
    collectState(); // returns non-zero if there are machines that are affected by the new state
    void sendUpdates();
    ...
    static void setup(
        void *
            data); // call init and link to the clockwork machine instance for ethercat
    void add_io_entry(const char *name, unsigned int io_offset,
    ...
    void setMaxIOIndex(
        unsigned int
            new_max); // min index into user required process data (must be zero)
    void setMinIOIndex(
        unsigned int new_min); // max index into user required process data
    uint32_t
    getProcessDataSize(); // returns process data size of user selected data set
    ...
    static SDOEntry *createSDORequest(std::string name, ECModule *module,
  ...
  private:
    ...
    unsigned int
        max_io_index; // last byte of the process data needed by the user
