...
ControlSystemMachine::ControlSystemMachine()
    : state(s_unknown), ethercat_machine(0), activate_requested(false),
      deactivate_requested(false), ecat_last_synced(0) {
    std::cout << "ControlSystem version " << Latproc_VERSION_MAJOR << "."
              << Latproc_VERSION_MINOR << "\n";
...
bool ControlSystemMachine::connected() const {
    return state == s_connected || state == s_operational;
}
...
bool ControlSystemMachine::deactivationRequested() {
    return deactivate_requested;
}

uint64_t ControlSystemMachine::lastUpdated() { return ecat_last_synced; }
...
bool ControlSystemMachine::c_disconnected() const { return !c_connected(); }
...
void ControlSystemMachine::idle() {
    ...
    */
    ...
    case s_connected:
        if (c_online()) {
            enter_slaves_online();
        } else if (c_operational()) {
            enter_operational();
        } else if (c_disconnected()) {
            enter_disconnected();
        } else if (!c_connected()) {
            enter_unknown();
        }
...
void ControlSystemMachine::sync_slaves() {
    ...
    std::list<MachineInstance *>::iterator iter =
        MachineInstance::io_modules_begin();
    while (iter != MachineInstance::io_modules_end()) {
        ...
        if (m->enabled()) {
            ...
            if (pos != SymbolTable::Null) {
                ...
                ECModule *module =
                    dynamic_cast<ECModule *>(ECInterface::findModule(position));
                if (module) {
                    const char *module_state = 0;
                    int state = module->slave_config_state.al_state &
                                0x0f; // mask off error bit
                    if (state == 1) {
                        module_state = "INIT";
                    } else if (state == 2) {
                        module_state = "PREOP";
                    ...
                    if (module_state &&
                        m->getCurrent().getName() != module_state) {
                        SetStateActionTemplate ssat =
                            SetStateActionTemplate("SELF", module_state);
                        SetStateAction *ssa =
                            dynamic_cast<SetStateAction *>(ssat.factory(m));
                        m->enqueueAction(ssa);
                    }
                }
            }
            ...
            ecat_last_synced =
                0; // haven't synced all modules yet because this one is disabled
        }
    }
...
void ControlSystemMachine::enter_connected() {
    ...
    if (ethercat_machine) {
        {
            SetStateActionTemplate ssat =
                SetStateActionTemplate("ETHERCAT_LS", "UP");
            SetStateAction *ssa =
                dynamic_cast<SetStateAction *>(ssat.factory(ethercat_machine));
            ethercat_machine->enqueueAction(ssa);
        }
        SetStateActionTemplate ssat =
            SetStateActionTemplate("SELF", "CONNECTED");
        SetStateAction *ssa =
            dynamic_cast<SetStateAction *>(ssat.factory(ethercat_machine));
        ethercat_machine->enqueueAction(ssa);
    }
...
void ControlSystemMachine::enter_disconnected() {
...
#ifndef EC_SIMULATOR
              << " wc_state: 0x" << std::hex
              << (int)ECInterface::domain1_state.wc_state << std::dec
    ...
    if (ethercat_machine) {
        {
            SetStateActionTemplate ssat =
                SetStateActionTemplate("ETHERCAT_LS", "DOWN");
            SetStateAction *ssa =
                dynamic_cast<SetStateAction *>(ssat.factory(ethercat_machine));
            ethercat_machine->enqueueAction(ssa);
        }
        SetStateActionTemplate ssat =
            SetStateActionTemplate("SELF", "DISCONNECTED");
        SetStateAction *ssa =
            dynamic_cast<SetStateAction *>(ssat.factory(ethercat_machine));
        ethercat_machine->enqueueAction(ssa);
    }
