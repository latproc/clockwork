...
SharedWorkSet::erase(std::set<MachineInstance *>::iterator &iter) {
    boost::recursive_mutex::scoped_lock scoped_lock(mutex);
...
std::set<MachineInstance *>::iterator SharedWorkSet::begin() {
    return busy_machines.begin();
}
std::set<MachineInstance *>::iterator SharedWorkSet::end() {
    return busy_machines.end();
}
