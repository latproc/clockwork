...
class MachineClass {
  public:
    ...
    virtual void addProperty(
        const char *name); // used in interfaces to list synced properties
    virtual void addPrivateProperty(
        const char *name); // used in interfaces to list synced properties
    virtual void addPrivateProperty(
        const std::string
            &name); // used in interfaces to list synced properties
    virtual void addCommand(
        const char *name); // used in interfaces to list permitted commands
    MachineCommandTemplate *findMatchingCommand(std::string cmd_name,
                                                const char *state);
