...
bool Predicate::usesTimer(Value &timer_val) const {
    ...
    if (entry.kind == Value::t_symbol &&
        entry.token_id == ClockworkToken::TIMER) {
        return true;
    } else {
        return false;
    }
...
void Predicate::clearTimerEvents(
    MachineInstance
        *target) // clear all timer events scheduled for the supplid machine
{
    if (left_p) {
...
Predicate::Predicate(const Predicate &other)
    ...
    if (other.dyn_value) {
        dyn_value =
            new Value(DynamicValueBase::ref(other.dyn_value->dyn_value));
    } else {
        dyn_value = 0;
    }
...
Predicate &Predicate::operator=(const Predicate &other) {
    ...
    entry.cached_machine =
        0; // do not preserve any cached machine pointers in this clone
    priority = other.priority;
    mi = 0;
...
PredicateSymbolDetails::PredicateSymbolDetails(std::string n, std::string t,
                                               std::string e)
    : name(n), type(t), export_name(e) {}

PredicateSymbolDetails::PredicateSymbolDetails() {}

bool PredicateSymbolDetails::operator<(
    const PredicateSymbolDetails &other) const {
...
Predicate::PredicateSymbolDetailsFromValue(const Value &entry) {
...
void Predicate::findSymbols(std::set<PredicateSymbolDetails> &symbols,
    ...
    if (entry.kind == Value::t_symbol) {
        ...
        if (psd.type == "unknown" &&
            parent) { // look deeper into the current context
        ...
        if (entry.kind == Value::t_symbol) {
            if (entry.sValue == "TIMER") {
                ...
                if (item != symbols.end()) {
                    ...
                    if (psd.type == "state") {
                        out << ExportState::instance()->prefix()
                            << psd.export_name;
                    } else {
                        out << ExportState::instance()->prefix()
                            << psd.export_name;
    ...
    if (left_p) {
        ...
        if (entry.kind == Value::t_symbol) {
            ...
            if (item != symbols.end()) {
                ...
                vars << ",(long)" << ExportState::instance()->prefix()
                     << psd.export_name;
            } else {
                out << entry.sValue << " (%ld)";
                vars << ",(long)" << ExportState::instance()->prefix()
...
std::ostream &Stack::traverse(std::ostream &out,
                              std::list<ExprNode>::const_iterator &iter,
...
const Value *resolveCacheMiss(Predicate *p, MachineInstance *m, bool left,
                              bool reevaluate) {
    Value *v = &p->entry;
    p->clearError();
...
ExprNode eval_stack(MachineInstance *m,
                    std::list<ExprNode>::const_iterator &stack_iter) {
    ...
    case opMatch: {
        assert(a.val);
        assert(b.val);
        return (bool)matches(a.val->asString().c_str(),
                             b.val->asString().c_str());
    }
    case opAny:
    case opCount:
    case opAll:
...
bool prep(Stack &stack, Predicate *p, MachineInstance *m, bool left,
          bool reevaluate) {
    // check for state comparison
    if (p->left_p && p->left_p->op == opNone && p->right_p &&
        p->right_p->op == opNone && (p->op == opEQ || p->op == opNE)) {
        if (p->left_p->entry.kind == Value::t_symbol &&
            p->right_p->entry.kind == Value::t_symbol) {
            ...
            if (lhm && !rhm) {
                if (lhm->hasState(p->right_p->entry.sValue)) {
                    ...
                    p->left_p->dyn_value = new Value(
                        new MachineValue(lhm, p->left_p->entry.sValue));
                }
            } else if (rhm && !lhm) {
                if (rhm->hasState(p->left_p->entry.sValue)) {
                    p->left_p->entry.kind = Value::t_string;
                    p->right_p->dyn_value = new Value(
                        new MachineValue(rhm, p->right_p->entry.sValue));
                }
            }
        }
    }
...
ExprNode::ExprNode(const ExprNode &other)
    : tmpval(other.tmpval), val(other.val), node(other.node), op(other.op),
      kind(other.kind) {
...
bool Condition::operator()(MachineInstance *m) {
    if (predicate) {
        ...
        if (predicate->stack.stack.size() == 0) {
            if (!prep(predicate->stack, predicate, m, true,
                      predicate->needs_reevaluation)) {
                std::ostream &out = MessageLog::instance()->get_stream();
                out << m->getName()
                    << " condition failed: predicate failed to resolve: "
                    << *this->predicate << "\n";
                MessageLog::instance()->release_stream();
                return false;
            }
        }
        std::list<ExprNode>::const_iterator work =
            predicate->stack.stack.begin();
        ...
        if (last_result.kind == Value::t_bool) {
            ...
            std::ostream &out = MessageLog::instance()->get_stream();
            out << "warning:  last result of " << *predicate
                << " is not boolean: " << last_result << "\n";
            MessageLog::instance()->release_stream();
        }
    }
    return false;
