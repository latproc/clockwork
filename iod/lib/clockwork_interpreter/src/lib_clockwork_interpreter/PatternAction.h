...
struct CopyPatternActionTemplate : public ActionTemplate {
    CopyPatternActionTemplate(CStringHolder d, CStringHolder p,
                              CStringHolder prop)
...
struct CopyAllPatternActionTemplate : public ActionTemplate {
    CopyAllPatternActionTemplate(CStringHolder d, CStringHolder p,
                                 CStringHolder prop)
        : dest(d), pattern(p), property(prop) {}
    virtual Action *factory(MachineInstance *mi);
    std::ostream &operator<<(std::ostream &out) const {
...
struct ExtractPatternActionTemplate : public ActionTemplate {
    ExtractPatternActionTemplate(CStringHolder d, CStringHolder p,
                                 CStringHolder prop)
        : dest(d), pattern(p), property(prop) {}
    virtual Action *factory(MachineInstance *mi);
    std::ostream &operator<<(std::ostream &out) const {
...
struct ExtractAllPatternActionTemplate : public ActionTemplate {
    ExtractAllPatternActionTemplate(CStringHolder d, CStringHolder p,
                                    CStringHolder prop)
        : dest(d), pattern(p), property(prop) {}
    virtual Action *factory(MachineInstance *mi);
    std::ostream &operator<<(std::ostream &out) const {
...
struct ExtractAllPatternAction : public Action {
    ExtractAllPatternAction(MachineInstance *mi,
                            ExtractAllPatternActionTemplate &pat)
        : Action(mi), dest(pat.dest.get()), pattern(pat.pattern.get()),
          property(pat.property.get()) {}
    Status run();
    Status checkComplete();
...
struct ReplacePatternActionTemplate : public ActionTemplate {
    ReplacePatternActionTemplate(CStringHolder p, CStringHolder prop,
                                 CStringHolder repl)
        : pattern(p), property(prop), replacement(repl) {}
    virtual Action *factory(MachineInstance *mi);
    std::ostream &operator<<(std::ostream &out) const {
        return out << "replace " << pattern.get() << " in " << property.get()
...
struct ReplaceAllPatternActionTemplate : public ActionTemplate {
    ReplaceAllPatternActionTemplate(CStringHolder p, CStringHolder prop,
                                    CStringHolder repl)
        : pattern(p), property(prop), replacement(repl) {}
    virtual Action *factory(MachineInstance *mi);
    std::ostream &operator<<(std::ostream &out) const {
        return out << "replace all" << pattern.get() << " in " << property.get()
                   << " with " << replacement.get();
    }
    CStringHolder pattern;
    CStringHolder property;
...
struct ReplaceAllPatternAction : public Action {
    ReplaceAllPatternAction(MachineInstance *mi,
                            ReplaceAllPatternActionTemplate &pat)
        : Action(mi), pattern(pat.pattern.get()), property(pat.property.get()),
          replacement(pat.replacement.get()) {}
    Status run();
    Status checkComplete();
