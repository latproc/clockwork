...
bool CounterRateInstance::setValue(const std::string &property,
    ...
    if (property == "VALUE") {
        ...
        int32_t mean =
            (settings->readings.average(settings->readings.length()) + 0.5f);
        if ((uint32_t)abs(mean - settings->last_sent) >
        ...
        if (settings->update_t + 2000 < now_t) {
            long new_val =
                (long)((float)settings->position +
                       settings->velocity * 2 /
                           1000.0f); //* (now_t-update_t) / 1000000.0f );
