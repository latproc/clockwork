...
struct IncludeActionTemplate : public ActionTemplate {
    ...
    IncludeActionTemplate(const std::string &name, Value val, Value pos = -1,
                          bool before = false, bool expand = false);
    ...
    std::string
        list_machine_name; // name of the list to include the machine into
    Value entry;
    Value position;
    bool before;
    bool expand;
...
struct IncludeAction : public Action {
    IncludeAction(MachineInstance *m, const IncludeActionTemplate *dat,
                  const Value pos, bool before, bool expand = false);
    IncludeAction();
    Status run();
