...
WaitActionTemplate::WaitActionTemplate(Value &val)
    : wait_time(-1), property_name("") {
...
WaitActionTemplate::WaitActionTemplate(long delta)
    : wait_time(delta), property_name("") {}

std::ostream &WaitActionTemplate::operator<<(std::ostream &out) const {
...
Action::Status WaitAction::run() {
    ...
    if (wait_time == -1) {
        ...
        if (v.kind == Value::t_float) {
            DBG_M_PROPERTIES
                << "Error: expected an integer value for wait_time, got: " << v
                << "\n";
            wait_time = v.trunc();
    ...
    if (wait_time == 0) {
        ...
        status = Running;
        if (trigger) {
            DBG_MESSAGING << owner->getName() << " " << *this
                          << " removing old trigger\n";
            cleanupTrigger();
        }
        char buf[100];
...
Action::Status WaitAction::checkComplete() {
    DBG_M_MESSAGING << owner->getName() << " " << *this
