...
class SampleBuffer : public Buffer {
  public:
    ...
    SampleBuffer(int buf_size) : Buffer(buf_size) {
...
class ButterworthFilter {
  public:
    ButterworthFilter(int num_c, double *c_coeff, int num_d, double *d_coeff)
        : num_c_coefficients(num_c), c_coefficients(c_coeff),
          signal_buf(num_c + 1), num_d_coefficients(num_d),
          d_coefficients(d_coeff), filtered_buf(num_d + 1) {}
