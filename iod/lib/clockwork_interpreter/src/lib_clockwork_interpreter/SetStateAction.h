...
struct SetStateAction : public Action {
    ...
    Value saved_state;
    Value
        new_state; // new state as given by the program (may be a property name)
    State value;
...
struct MoveStateAction : public SetStateAction {
    MoveStateAction(MachineInstance *mi, MoveStateActionTemplate &t,
                    uint64_t auth = 0)
...
struct SetIOStateAction : public Action {
    SetIOStateAction(MachineInstance *mi, IOComponent *io,
                     const State &new_state, uint64_t auth = 0)
