...
IncludeAction::IncludeAction(MachineInstance *m,
                             const IncludeActionTemplate *dat, Value pos,
                             bool insert_before, bool expand_list)
    : Action(m), list_machine_name(dat->list_machine_name), entry(dat->entry),
      list_machine(0), entry_machine(0), position(pos), before(insert_before),
      expand(expand_list) {}

IncludeAction::IncludeAction()
    : list_machine(0), entry_machine(0), position(-1), before(false),
      expand(false) {}
...
Action::Status IncludeAction::run() {
    ...
    if (list_machine) {
            ...
            */
            if (entry.kind == Value::t_symbol) {
                ...
                if (!done) {
                    char buf[400];
                    snprintf(buf, 400,
                             "%s  failed to lookup machine: %s  for "
                             "assignment. Treating it as a string",
                             owner->fullName().c_str(),
                ...
                Parameter &p = list_machine->locals.at(
                    0); //TBD take more care of this index
            ...
            bool found = false;
            long pos = -1;
            if (position.kind == Value::t_string ||
                position.kind == Value::t_symbol) {
                ...
                if (pos_v == SymbolTable::Null || !pos_v.asInteger(pos)) {
                    ...
                    snprintf(
                        err_msg, 100, "%s Cannot find an integer value for %s",
                ...
                snprintf(err_msg, 100,
                         "%s Unexpected value %s when inserting item",
                         owner->getName().c_str(), position.asString().c_str());
                MessageLog::instance()->add(err_msg);
                error_str = err_msg;
                status = Failed;
                    ...
                    if (!machine) {
                        ...
                        if (v.kind == Value::t_symbol) {
                            ...
                            if (machine) {
                                ...
                                list_machine->addParameter(v, machine, pos,
                                                           before);
                            } else {
                        ...
                        if (machine->_type == "LIST" && expand) {
                            for (unsigned int i = 0;
                                 i < machine->parameters.size(); i++) {
                                MachineInstance *item =
                                    machine->parameters[i].machine;
                                list_machine->addParameter(
                                    machine->parameters[i].val, item, pos,
                                    before);
                                if (pos >= 0) {
                            ...
                            list_machine->addParameter(entry, machine, pos,
                                                       before);
                        }
                    }
                } else {
        ...
        snprintf(err_msg, 400,
                 "%s Cannot find a machine named %s for list insert of %s",
