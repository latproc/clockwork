...
int getIntValue(cwpi_Scope s, const char *property_name, const long **res) {
    ...
    if (!scope) {
        MessageLog::instance()->add(
            "getIntValue was passed a null instance from a plugin");
        return 0;
    }
...
void setStringValue(cwpi_Scope s, const char *property_name,
...
int changeState(cwpi_Scope s, const char *new_state) {
    ...
    if (!scope) {
        MessageLog::instance()->add(
            "changeState was passed a null instance from a plugin");
        return 0;
    }
    SetStateActionTemplate ssat("SELF", new_state);
    if (scope->isActive()) {
        scope->enqueueAction(ssat.factory(
            scope)); // execute this state change once all other actions are complete
    } else {
...
char *getState(cwpi_Scope s) {
    ...
    if (!scope) {
        MessageLog::instance()->add(
            "getState was passed a null instance from a plugin");
        return 0;
    }
...
void *getNamedScope(cwpi_Scope s, const char *name) {
    ...
    if (!scope) {
        MessageLog::instance()->add(
            "getInstanceData was passed a null instance from a plugin");
        return 0;
    }
...
void setInstanceData(cwpi_Scope s, void *block) {
    ...
    if (!scope) {
        MessageLog::instance()->add(
            "setInstanceData was passed a null instance from a plugin");
        return;
    }
    scope->setData(block);
}
