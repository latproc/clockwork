...
MachineClass::MachineClass(const char *class_name)
    : default_state("unknown"), initial_state("INIT"), name(class_name),
      allow_auto_states(true), token_id(0), plugin(0), polling_delay(0),
      parent(0) {
    addState("INIT");
...
void MachineClass::addState(const State &state) {
    ...
    if (found == states.end()) {
        ...
        *((*found).second) = state;
        MessageLog::instance()->add("Machine ", name, "already has state ",
                                    state.getName());
    }
}
...
void MachineClass::addProperty(const char *p) { property_names.insert(p); }
...
void MachineClass::addCommand(const char *p) { command_names.insert(p); }
...
MachineCommandTemplate *MachineClass::findMatchingCommand(std::string cmd_name,
                                                          const char *state) {
    // find the correct command to use based on the current state
    std::multimap<std::string, MachineCommandTemplate *>::iterator cmd_it =
        commands.find(cmd_name);
...
const Value &ExportState::symbol(const char *name) {
...
void MachineClass::exportHandlers(std::ostream &ofs) {
    std::stringstream received_message_handlers;
    {
        ...
        std::multimap<Message, MachineCommandTemplate *>::const_iterator
            recv_iter = receives.begin();
        while (recv_iter != receives.end()) {
            const std::pair<Message, MachineCommandTemplate *> item =
                *recv_iter++;
            ...
            for (unsigned int i = 0; i < (item.second)->action_templates.size();
                 ++i) {
            ...
            ofs << "int cw_" << name << "_" << method_name(item.first.getText())
                << "(struct cw_" << name << " *m, ccrContParam) {\n";
            std::string saved_prefix = ExportState::instance()->prefix();
            if (can_block) {
                ofs << "\tccrBeginContext;\n";
                ExportState::instance()->set_prefix(std::string("ctx->") +
                                                    saved_prefix);
            }
            ...
            if (item.first.isSimple()) {
                ...
                if (name_pos != std::string::npos) {
                    ...
                    std::string msg_name(
                        item.first.getText().substr(name_pos + 1));
                    size_t enter_pos = msg_name.find("_enter");
                    if (enter_pos != std::string::npos &&
                        enter_pos == msg_name.length() - 6) {
        ...
        if (!init_seen) {
            ofs << "int cw_" << name << "_" << initial_state_message
                << "(struct cw_" << name << " *m, ccrContParam) {\n"
                << "\tm->machine.execute = 0;\n"
                << "\treturn 1;\n"
                << "};\n";
    ...
    ofs << "int cw_" << name
        << "_handle_message(struct MachineBase *obj, struct MachineBase "
           "*source, int state) {\n"
        << "\tstruct cw_" << name << " *m = (struct cw_" << name << " *)obj;\n"
        << external_handlers << "\tmarkPending(obj);\n"
        << "\treturn 1;\n"
        << "}\n";
}
...
#else
    std::multimap<Message, MachineCommandTemplate *>::const_iterator recv_iter =
        receives.begin();
    while (recv_iter != receives.end()) {
...
static std::string generate_name_lookup_decls(std::string var,
                                              std::string to_lookup,
    ...
    if (type == "machine") {
        ...
        if (to_lookup.find('.') != std::string::npos) {
            ...
            ExportState::instance()->remotes().insert(
                var); // used to access the machine owning the variable
        }
        res << "\tValue *l_" << var << ";\n";
    }
    return res.str();
...
static std::string generate_name_backups(std::string var, std::string to_lookup,
                                         const std::string type) {
    std::stringstream res;
...
static std::string generate_name_lookup(std::string var, std::string to_lookup,
                                        std::string &type) {
    std::stringstream res;
    ...
    if (pos == std::string::npos) { // local variable
        ...
        if (is_machine) {
            res << "\tv->l_" << var << " = mm->lookup_machine(mm, sym_"
                << short_name << ");\n";
...
bool MachineClass::cExport(const std::string &filename) {
    ...
    std::map<std::string, PredicateSymbolDetails> &symbols(
        ExportState::all_symbol_names());

    {
        ...
        while (p_iter != parameters.end()) {
            ...
            values << ", " << p.val;
            MachineInstance *p_machine =
                MachineInstance::find(p.val.asString().c_str());
            refs << "\tMachineBase *_" << p.val << ";\n";
            symbols[p.val.sValue] =
                PredicateSymbolDetails(p.val.sValue, "machine", p.val.sValue);
            setup << "\tm->_" << p.val << " = " << p.val << ";\n";
            setup << "\tif (" << p.val << ") MachineDependencies_add(" << p.val
                  << ", cw_" << name << "_To_MachineBase(m));\n";
            lookup_machine << "\tif (symbol == sym_" << p.val << ") return m->_"
                           << p.val << ";\n";
...

    {
        ...
        ofh << "#ifndef __cw_" << name << "_h__\n"
            << "#define __cw_" << name << "_h__\n"
            << "\n#include \"runtime.h\"\n";

        ofh << "#define Value int\n"
        ...
        while (pnames_iter != properties.end()) {
            ...
            symbols[prop.first] =
                PredicateSymbolDetails(prop.first, "property", prop.first);
            lookup << "\tif (symbol == sym_" << prop.first << ") return &m->"
                   << prop.first << ";\n";
        ...
        while (opts_iter != options.end()) {
            ...
            symbols[opt.first] =
                PredicateSymbolDetails(opt.first, "property", opt.first);
            lookup << "\tif (symbol == sym_" << opt.first << ") return &m->"
                   << opt.first << ";\n";
        ...
        ofh << "};\n"
            << "struct IOAddress *cw_" << name << "_getAddress(struct cw_"
            << name << " *p);\n"
            << "struct cw_" << name << " *create_cw_" << name
            << "(const char *name";
        ...
        ofh << params.str() << ");\n"
            << "MachineBase *cw_" << name << "_To_MachineBase(struct cw_"
            << name << " *);\n"
            << "#endif\n";
    }
    {
        ...
        while (iter != states.end()) {
            ...
            State *s = item.second;
            symbols[s->getName()] =
                PredicateSymbolDetails(s->getName(), "state", s->getName());
        }
        for (unsigned int i = 0; i < stable_states.size(); ++i) {
            ...
            std::set<PredicateSymbolDetails>::iterator iter =
                state_symbols.begin();
            while (iter != state_symbols.end()) {
                const PredicateSymbolDetails &psd = (*iter++);
                std::map<std::string, PredicateSymbolDetails>::iterator found =
                    symbols.find(psd.name);
                if (found != symbols.end()) {
        ...
        while (recv_iter != receives.end()) {
            ...
            for (unsigned int i = 0; i < (item.second)->action_templates.size();
                 ++i) {
                ActionTemplate *at = (item.second)->action_templates.at(i);
                PredicateActionTemplate *pat =
                    dynamic_cast<PredicateActionTemplate *>(at);
                if (pat) {
                    ...
                    while (iter != pat_symbols.end()) {
                        const PredicateSymbolDetails &psd = (*iter++);
                        std::map<std::string, PredicateSymbolDetails>::iterator
                            found = symbols.find(psd.name);
                        if (found != symbols.end()) {
                            const PredicateSymbolDetails &other =
                                (*found).second;
                            if (psd.type == "unknown" &&
                                other.type != "unknown") {
                                PredicateSymbolDetails update(psd);
        ...
        {{std::map<std::string, PredicateSymbolDetails>::iterator iter =
              symbols.begin();
            ...
            if (psd.type != "ignored") {
                ...
                if (type == "unknown") {
                    if (psd.name.find('.') != std::string::npos) {
                        ...
                        type = "state";
                    } else if (this->options.find(psd.name) !=
                               this->options.end()) {
                        type = "property";
                    } else {
                        for (size_t i = 0; i < parameters.size(); ++i) {
                            if (parameters[i].val.kind == Value::t_symbol &&
                                parameters[i].val.sValue == psd.name) {
                                type = "machine";
                            }
                        }
                        if (type == "unknown") {
                ...
                if (type == "unknown") {
                    std::cout << "Unknown type for " << psd.name << " in "
                              << name << "; assuming state\n";
                    type = "state";
                    used_states.insert(psd.name);
                    symbols[psd.name] =
                        PredicateSymbolDetails(psd.name, type, psd.export_name);
                }
                lookup_vars_init +=
                    generate_name_lookup(psd.export_name, psd.name, type);
                lookup_vars +=
                    generate_name_lookup_decls(psd.export_name, psd.name, type);
                backup_vars +=
                    generate_name_backups(psd.export_name, psd.name, type);
            }
            iter++;
        }
    }
...
ofs << "static void init_Vars(struct cw_" << name << " *m, struct cw_" << name
    << "_Vars *v) {\n"
    << "\tv->m = m;\n"
    << lookup_vars_init << "}\n";
...
{
    ofs << "Value *cw_" << name << "_lookup(struct cw_" << name
        << " *m, int symbol) {\n"
        << lookup.str() << "\treturn 0;\n}\n";
    ofs << "MachineBase *cw_" << name << "_lookup_machine(struct cw_" << name
        << " *m, int symbol) {\n"
        << lookup_machine.str() << "\treturn 0;\n}\n";
}
ofs << "void cw_" << name << "_describe(struct cw_" << name << " *m);\n";
ofs << "int cw_" << name
    << "_handle_message(struct MachineBase *ramp, struct MachineBase *machine, "
       "int state);\n";
ofs << "int cw_" << name << "_check_state(struct cw_" << name << " *m);\n";
if (timer_clauses.size()) {
    ofs << "uint64_t cw_" << name << "_next_trigger_time(struct cw_" << name
        << " *m, struct cw_" << name << "_Vars_backup *v);\n";
}
...
ofs << params.str() << ") {\n"
    << "\tstruct cw_" << name << " *p = (struct cw_" << name
    << " *)malloc(sizeof(struct cw_" << name << "));\n"
...
if (timer_clauses.size()) {
    ...
    while (iter != timer_clauses.end()) {
        ...
        Predicate *sub_p = (l && (l->entry.kind == Value::t_symbol ||
        ...
        std::map<std::string, PredicateSymbolDetails>::iterator found =
            ExportState::all_symbol_names().find(source_name);
...
handlers << "\tMachineActions_add(cw_" << name
         << "_To_MachineBase(m), (enter_func)cw_" << name << "_"
         << initial_state.getName() << "_enter);\n";

ofs << "void Init_cw_" << name << "(struct cw_" << name
    << " *m, const char *name";
if (name == "ANALOGINPUT" || name == "ANALOGOUTPUT") {
    ofs << ", int pin";
}
...
ofs << "\tm->machine.state = state_cw_" << initial_state << ";\n"
    << "\tm->machine.check_state = ( int(*)(MachineBase*) )cw_" << name
    << "_check_state;\n"
    << "\tm->machine.handle = (message_func)cw_" << name
    << "_handle_message; // handle message from other machines\n"
    << "\tm->machine.lookup = (lookup_func)cw_" << name
    << "_lookup; // lookup symbols within this machine\n"
    << "\tm->machine.lookup_machine = (lookup_machine_func)cw_" << name
    << "_lookup_machine; // lookup symbols within this machine\n"
    << "\tm->machine.describe = (describe_func)cw_" << name << "_describe;\n"
    << "\tm->vars = (struct cw_" << name << "_Vars *)malloc(sizeof(struct cw_"
    << name << "_Vars));\n"
    << "\tm->backup = (struct cw_" << name
    << "_Vars_backup *)malloc(sizeof(struct cw_" << name << "_Vars_backup));\n"
    << "\tinit_Vars(m, m->vars);\n"
    << "\tbackup_Vars(m);\n"
    << handlers.str() << "\tmarkPending(&m->machine);\n"
    << "}\n";

ofs << "struct IOAddress *cw_" << name << "_getAddress(struct cw_" << name
    << " *p) {\n"
...
for (unsigned int i = 0; i < stable_states.size(); ++i) {
    ...
    if (s.condition.predicate->priority != 1) { // not the DEFAULT state
        ...
        when_clauses << "\tsnprintf(buf, 200, \"" << s.state_name
                     << " [%d]: " << desc.str() << "\""
    ...
    if (receives.find(Message(enter_msg_name.c_str())) != receives.end()) {
        ofs << "\t\tnew_state_enter = (enter_func)cw_" << name << "_"
            << enter_msg_name << ";\n";
    }
    ofs << "\t}\n";
}
...
ofs << "\tif (new_state && new_state != m->machine.state) {\n"
    ...
    << "_To_MachineBase(m), new_state, new_state_enter); // TODO: fix me\n"
    << "\t\tmarkPending(&m->machine);\n"
    << "\t\tres = 1;\n"
    << "\t}\n";

if (timer_clauses.size()) {
    ofs << "\tuint64_t delay = cw_" << name << "_next_trigger_time(m, v);\n"
        << "\tif (delay > 0) {\n"
        ...
        << "\t\t}\n"
        << "\t\tRTScheduler_add(scheduler, ScheduleItem_create(delay, "
           "&m->machine));\n"
        << "\t\tRTScheduler_release();\n"
        << "\t}\n";
}
...
ofs << "void cw_" << name << "_describe(struct cw_" << name << " *m) {\n"
    << "\tstruct cw_" << name << "_Vars_backup *v = m->backup;\n"
    << "{\n\tchar buf[100];\n"
    << "\tsnprintf(buf, 100, \"%s: %s  Class: " << name
    << "\", m->machine.name, name_from_id(m->machine.state));\n"
