...
struct PredicateTimerDetails {
    int64_t delay;
    std::string label;
    PredicateTimerDetails(int64_t dly, const std::string &lbl)
        : delay(dly), label(lbl) {}
...
class Predicate {
  public:
    ...
    const Value *
        last_calculation; // for dynamic values, retains the last value for display only
    ...
    Predicate(Value *v)
        : left_p(0), op(opNone), right_p(0), entry(*v), mi(0), dyn_value(0),
          cached_entry(0), last_calculation(0), priority(0),
          lookup_error(false), needs_reevaluation(true),
    ...
    Predicate(const char *s)
        : left_p(0), op(opNone), right_p(0), entry(s), mi(0), dyn_value(0),
          cached_entry(0), last_calculation(0), priority(0),
          lookup_error(false), needs_reevaluation(true),
          last_evaluation_time(0) {
        if (entry.kind == Value::t_symbol && entry.sValue == "DEFAULT") {
    ...
    Predicate(Predicate *l, PredicateOperator o, Predicate *r)
        : left_p(l), op(o), right_p(r), mi(0), dyn_value(0), cached_entry(0),
          last_calculation(0), priority(0), lookup_error(false),
          needs_reevaluation(true), last_evaluation_time(0) {}
    ~Predicate();
    ...
    */
    ...
    PredicateTimerDetails *scheduleTimerEvents(
        ...
        MachineInstance
            *target); // setup timer events that trigger the supplied machine
    void clearTimerEvents(
        MachineInstance *
            target); // clear all timer events scheduled for the supplid machine

    void findSymbols(std::set<PredicateSymbolDetails> &,
                     const Predicate *parent) const;
    static PredicateSymbolDetails
    PredicateSymbolDetailsFromValue(const Value &entry);
