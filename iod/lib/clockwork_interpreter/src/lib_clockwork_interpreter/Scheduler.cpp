...
std::string Scheduler::getStatus() {
    ...
    if (state == e_running) {
        ...
        snprintf(buf, 300,
                 ...
                 state, ready(now), items.size(),
                 (now - ProcessingThread::programStartTime()),
                 (float)notification_sent / 1000000.0f,
...
ScheduledItem::ScheduledItem(long delay, Trigger *t)
...
Scheduler::Scheduler()
    : state(e_waiting),
      update_sync(*MessagingInterface::getContext(), ZMQ_PAIR),
...
bool CompareSheduledItems::operator()(const ScheduledItem *a,
                                      const ScheduledItem *b) const {
    bool result = (*a) >= (*b);
    return result;
}
...
void Scheduler::add(ScheduledItem *item) {
    ...
    if (delay <= next_delay_time ||
        next_delay_time <= 0) { // && delay>=0 && next_delay_time>=0) {
...
void Scheduler::idle() {
    ...
    while (state != e_aborted) {
        ...
        while (state == e_running && is_ready) {
            ...
            ScheduledItem *item = 0;
            {
                boost::recursive_mutex::scoped_lock scoped_lock(
                    Scheduler::instance()->internals->q_mutex);
                item = next();
                DBG_SCHEDULER << "Scheduler activating scheduled item "
                              << (*item) << " ready. " << items.size()
                              << " items remain\n";
                pop();
            }
            next_time = 0;
            if (item->trigger) {
                if (item->trigger->enabled()) {
                    DBG_SCHEDULER << "Scheduler firing trigger "
                                  << item->trigger->getName() << "\n";
                    item->trigger->fire();
                }
                delete item;
                ++items_found;
        ...
        if (state == e_running) {
            ...
            if (next() == 0) {
                DBG_SCHEDULER
                    << "no more scheduled items, waiting for cw ack\n";
            }
