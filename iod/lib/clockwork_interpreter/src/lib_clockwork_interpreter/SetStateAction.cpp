...
void SetStateActionTemplate::toC(std::ostream &out, std::ostream &vars) const {
    ...
    out << "\tif (executing(m->_" << machine_name << ")) {"
        << "\t\tccrReturn(0);\n"
        << "\t}\n"
        << "changeMachineState(m->_" << machine_name << ", state_cw_"
        << new_state << ", 0);\n";
}

SetStateAction::SetStateAction(MachineInstance *mi, SetStateActionTemplate &t,
                               uint64_t auth)
    : Action(mi), target(t.target), saved_state(t.new_state),
      new_state(t.new_state), value(t.new_state.sValue.c_str()), machine(0),
      authority(auth) {}
...
Action::Status SetStateAction::executeStateChange(bool use_transitions) {
    ...
    if (owner->getStateMachine()->default_state.getName() !=
            new_state.asString() &&
        owner->isStableState(new_state.asString()) &&
        !owner->stableStateValid(new_state.asString())) {
        std::stringstream ss;
        ss << owner->getName() << " aborting invalid stable state change"
           << std::flush;
    ...
    if (machine) {
        if (machine->getName() != target.get()) {
            DBG_M_ACTIONS << owner->getName() << " lookup for " << target.get()
                          << " returned " << machine->getName() << "\n";
        }
        ...
        if (!machine->hasState(new_state.sValue)) {
            ...
            if (deref != SymbolTable::Null) {
                DBG_M_ACTIONS << *this << " dereferenced " << new_state
                              << " to " << deref << "\n";
                if (deref.kind != Value::t_symbol &&
                    deref.kind != Value::t_string) {
                    std::stringstream ss;
                    ss << owner->fullName() << " state " << deref << " ("
                       << deref.kind << ")"
                       << " must be a symbol or string" << std::flush;
                    char *msg = strdup(ss.str().c_str());
                    error_str = msg;
                    ...
                    ss << " does not have a state " << deref.sValue
                       << std::flush;
                    char *msg = strdup(ss.str().c_str());
                    MessageLog::instance()->add(msg);
                    error_str = msg;
                ...
                DBG_M_ACTIONS << owner->fullName() << " setting state of "
                              << machine->fullName()
                ...
                snprintf(
                    buf, 150,
                    "Error: Machine %s asked to change to unknown state (%s)",
        ...
        if (machine->io_interface) {
            ...
            if (value == "on") {
                ...
                status = Running;
                setTrigger(owner->setupTrigger(machine->getName(),
                                               value.getName(), ""));
                return status;
            } else if (value == "off") {
        ...
        //          )
        {
            if (machine->getCurrent(owner) == value) {
                DBG_M_ACTIONS << machine->getName() << " is already " << value
                              << " skipping " << *this << "\n";
                status = Complete;
                cleanupTrigger();
                owner->stop(this);
            ...
            if (use_transitions) {
                ...
                if (!machine->transitions.empty()) {
                    std::list<Transition>::iterator iter =
                        machine->transitions.begin();
                    while (iter != machine->transitions.end()) {
                        const Transition &t = *iter++;
                        if ((t.source == machine->getCurrent(owner) ||
                             t.source.getName() == "ANY") &&
                            ...
                            if (!t.condition || (*t.condition)(owner)) {
                                ...
                                std::stringstream ss;
                                ss << owner->getName() << " "
                                   << "Transition from " << t.source << " to "
                                   << value << " denied due to condition "
            ...
            if (status == Complete || status == Failed) {
                ...
                trigger = owner->setupTrigger(machine->getName(),
                                              value.getName(), "");
            }
            if (status == Action::Failed) {
                char buf[100];
                snprintf(buf, 100, "%s::setState(%s) Failed\n",
                         machine->getName().c_str(), value.getName().c_str());
        ...
        status = Running;
        setTrigger(
            owner->setupTrigger(machine->getName(), value.getName(), ""));
        return status;
    } else {
        std::stringstream ss;
...

    {
        if (machine->io_interface) {
            ...
            if (value.getName() == pt->getStateString()) {
                ...
                DBG_M_ACTIONS << machine->getName() << " still in "
                              << pt->getStateString() << " waiting for "
                              << value << "\n";
                return status;
            }
        } else {
            if (machine->getCurrent(owner).getName() == value.getName()) {
                ...
                DBG_M_ACTIONS << owner->getName() << " still waiting for "
                              << machine->getName() << " to move to state "
                              << value << "\n";
            }
        }
    }

    DBG_M_ACTIONS << machine->getName() << " (" << machine->getCurrent(owner)
                  << ") waiting for " << value << "\n";
    return status;
