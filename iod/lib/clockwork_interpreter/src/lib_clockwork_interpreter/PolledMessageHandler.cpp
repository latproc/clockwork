...
bool PolledMessageHandler::changed =
    false; // indicates whether the cached poll_items is valid

PolledMessageHandler::PolledMessageHandler(zmq::socket_t &socket,
                                           bool is_enabled)
    : sock(socket), enabled(is_enabled) {}
...
zmq::pollitem_t *PolledMessageHandler::getZMQPollItems(zmq::pollitem_t *items) {
    ...
    if (res == NULL) {
        res = new zmq::pollitem_t[handlers.size()];
    }
