...
std::ostream &operator<<(std::ostream &out, const DynamicValue &val) {
    return val.operator<<(out);
}

DynamicValue *AssignmentValue::clone() const {
    return new AssignmentValue(*this);
}
...
DynamicValue *ItemAtPosValue::clone() const {
    return new ItemAtPosValue(*this);
}
...
std::ostream &AnyInValue::operator<<(std::ostream &out) const {
    return out << "ANY " << machine_list_name << " ARE " << state << "("
               << last_result << ")";
}
std::ostream &operator<<(std::ostream &out, const AnyInValue &val) {
    return val.operator<<(out);
}
...
std::ostream &operator<<(std::ostream &out, const AllInValue &val) {
    return val.operator<<(out);
}
...
std::ostream &AnyEnabledDisabledValue::operator<<(std::ostream &out) const {
    return out << "ANY " << machine_list_name
               << ((check_enabled) ? " ENABLED" : " DISABLED") << "("
               << last_result << ")";
}
std::ostream &operator<<(std::ostream &out,
                         const AnyEnabledDisabledValue &val) {
    return val.operator<<(out);
}

DynamicValue *AllEnabledDisabledValue::clone() const {
    return new AllEnabledDisabledValue(*this);
}
...
std::ostream &operator<<(std::ostream &out,
                         const AllEnabledDisabledValue &val) {
    return val.operator<<(out);
}
...
std::ostream &operator<<(std::ostream &out, const CountValue &val) {
    return val.operator<<(out);
}

DynamicValue *FindValue::clone() const { return new FindValue(*this); }
std::ostream &FindValue::operator<<(std::ostream &out) const {
    return out << "INDEX OF ITEM IN " << machine_list_name << " WHERE "
               << *condition.predicate << " (" << last_result << ")";
}
std::ostream &operator<<(std::ostream &out, const FindValue &val) {
    return val.operator<<(out);
}

DynamicValue *SumValue::clone() const { return new SumValue(*this); }
std::ostream &SumValue::operator<<(std::ostream &out) const {
    return out << "SUM " << property << " FROM " << machine_list_name << " ("
               << last_result << ")";
}
std::ostream &operator<<(std::ostream &out, const SumValue &val) {
    return val.operator<<(out);
}

DynamicValue *MinValue::clone() const { return new MinValue(*this); }
std::ostream &MinValue::operator<<(std::ostream &out) const {
    return out << "MIN " << property << " FROM " << machine_list_name << " ("
               << last_result << ")";
}
std::ostream &operator<<(std::ostream &out, const MinValue &val) {
    return val.operator<<(out);
}

DynamicValue *MaxValue::clone() const { return new MaxValue(*this); }
std::ostream &MaxValue::operator<<(std::ostream &out) const {
    return out << "MAX " << property << " FROM " << machine_list_name << " ("
               << last_result << ")";
}
std::ostream &operator<<(std::ostream &out, const MaxValue &val) {
    return val.operator<<(out);
}

DynamicValue *MeanValue::clone() const { return new MeanValue(*this); }
std::ostream &MeanValue::operator<<(std::ostream &out) const {
    return out << "MEAN " << property << " FROM " << machine_list_name << " ("
               << last_result << ")";
}
std::ostream &operator<<(std::ostream &out, const MeanValue &val) {
    return val.operator<<(out);
}
...
DynamicValue *ExpressionValue::clone() const {
...
std::ostream &operator<<(std::ostream &out, const SizeValue &val) {
    return val.operator<<(out);
}
...
std::ostream &operator<<(std::ostream &out, const BitsetValue &val) {
    return val.operator<<(out);
}
...
Value &AssignmentValue::operator()() {
    ...
    if (src.kind == Value::t_symbol) {
        ...
        if (last_result == SymbolTable::Null) {
            ...
            std::cerr << "assigning: " << last_result << " ("
                      << last_result.kind << ") to " << dest_name << "\n";
        }
    } else {
        last_result = src;
    }
...
const Value &AnyInValue::operator()() {
    ...
    if (state_property == 0) {
        char buf[150];
        snprintf(buf, 150, "%s cannot find state %s", mi->getName().c_str(),
                 state.c_str());
    ...
    for (unsigned int i = 0; i < machine_list->parameters.size(); ++i) {
        ...
        if (state_val ==
            machine_list->parameters[i].machine->getCurrent(scope).getName()) {
            last_result = true;
            return last_result;
        }
    }
...
AnyEnabledDisabledValue::AnyEnabledDisabledValue(
    const AnyEnabledDisabledValue &other) {
...
AllEnabledDisabledValue::AllEnabledDisabledValue(
    const AllEnabledDisabledValue &other) {
    check_enabled = other.check_enabled;
    machine_list_name = other.machine_list_name;
    machine_list = 0;
}
...
MinValue::MinValue(const MinValue &other) { property = other.property; }
...
MaxValue::MaxValue(const MaxValue &other) { property = other.property; }
...
DynamicValue *PopListBackValue::clone() const {
    return new PopListBackValue(*this);
}
...
Value &PopListBackValue::operator()(MachineInstance *mi) {
    ...
    if (!machine_list) {
        std::stringstream ss;
        ss << mi->getName() << " no machine " << machine_list_name
           << " for LIST operation\n";
...
DynamicValue *PopListFrontValue::clone() const {
    return new PopListFrontValue(*this);
}
...
Value &PopListFrontValue::operator()(MachineInstance *mi) {
    ...
    if (machine_list->_type == "REFERENCE") {
        if (machine_list->locals.size()) {
            ...
            if (machine_list->locals[0].machine &&
                !last_result.cached_machine) {
        ...
        if (machine_list->parameters.size()) {
            ...
            if (machine_list->parameters[0].machine &&
                ...
                last_result.cached_machine =
                    machine_list->parameters[0].machine;
            }
            if (remove_from_list) {
                machine_list->removeDependancy(
                    machine_list->parameters[0].machine);
                machine_list->stopListening(
                    machine_list->parameters[0].machine);
                machine_list->parameters.erase(
                    machine_list->parameters.begin());
...
Value &ItemAtPosValue::operator()(MachineInstance *mi) {
    ...
    if (machine_list->parameters.size()) {
        ...
        if (index.kind == Value::t_symbol) {
            if (!mi->getValue(index.sValue).asInteger(idx)) {
                MessageLog::instance()->add(
                    "non-numeric index when evaluating ITEM AT pos");
                last_result = false;
                return last_result;
            }
        ...
        if (idx >= 0 && idx < (long)machine_list->parameters.size()) {
            ...
            if (remove_from_list) {
                machine_list->parameters.erase(
                    machine_list->parameters.begin() + idx);
                if (machine_list->_type == "LIST") {
                    machine_list->setNeedsCheck();
                    displayList(machine_list);
                }
...
const Value &EnabledValue::operator()() {
    ...
    if (!machine) {
        std::stringstream ss;
        ss << mi->getName() << " no machine " << machine_name
           << " for ENABLED test\n";
        MessageLog::instance()->add(ss.str().c_str());
        last_result = false;
        return last_result;
...
std::ostream &operator<<(std::ostream &out, const EnabledValue &val) {
    return val.operator<<(out);
}
...
std::ostream &operator<<(std::ostream &out, const CastValue &val) {
    return val.operator<<(out);
}
...
std::ostream &operator<<(std::ostream &out, const ExistsValue &val) {
    return val.operator<<(out);
}

DynamicValue *ClassNameValue::clone() const {
    return new ClassNameValue(*this);
}
...
DynamicValue *ChangingStateValue::clone() const {
    return new ChangingStateValue(*this);
}
...
AsFormattedStringValue::AsFormattedStringValue(
    const AsFormattedStringValue &other) {
    property_name = other.property_name;
    format = other.format;
}
DynamicValue *AsFormattedStringValue::clone() const {
    return new AsFormattedStringValue(*this);
}
const Value &AsFormattedStringValue::operator()() {
    ...
    if (v == SymbolTable::Null) {
        std::stringstream ss;
        ss << mi->getName() << " cannot find  " << property_name
           << " for AS STRING\n";
        MessageLog::instance()->add(ss.str().c_str());
        last_result = 0;
        return last_result;
