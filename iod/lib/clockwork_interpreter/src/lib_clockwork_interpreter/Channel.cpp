...
class CommandLogFilter : public MessageFilter {
  public:
    CommandLogFilter(Channel *chn, const char *hdr)
        : channel(chn), header(hdr) {}
...
bool RemoteClockworkCommandFilter::filter(char **buf, size_t &len) {
    char *data = *buf;
    {
        ...
        fl.f() << "Channel " << channel->getName()
               << " subscriber received: " << data << " len:" << len << "\n";
    }
    if (len == 0) {
        return true;
    }
    ...
    if (command) {
        if (command->param(0) == "STATE") {
            MachineInstance *m =
                MachineInstance::find(command->param(1).asString().c_str());
            if (!m->isShadow()) {
                if (command->numParams() == 4 &&
                    ...
                    char *msg = MessageEncoding::encodeState(
                        m->getName(), command->param(2).sValue.c_str(),
...
class ChannelInternals {
  public:
    ...
    bool changed_state;
    ChannelInternals()
        : command_sock(0), cmd_sock_info(0), router_thread(0),
          thread_started(false), changed_state(false) {}
...
Channel::Channel(const std::string ch_name, const std::string type)
    : MachineInstance(ch_name.c_str(), type.c_str()), ChannelImplementation(),
      ...
      monit_subs(0), monit_pubs(0), connect_responder(0),
      disconnect_responder(0), monitor_thread(0), throttle_time(0),
      connections(0), aborted(false), subscriber_thread(0), started_(false),
      cmd_client(0), cmd_server(0), last_throttled_send(0), does_monitor(false),
      does_share(false), does_update(false) {
...
void Channel::syncInterfaceProperties(MachineInstance *m,
    ...
    if (definition()->updates_names.count(m->getName()) ||
        ...
        if (!mc) {
            std::string msg = MessageLog::instance()->add(
                "Warning: Interface ", interface_name, " is not defined");
            DBG_CHANNELS << msg << "\n";
            return;
        }
        MachineInterface *mi = dynamic_cast<MachineInterface *>(mc);
        if (mi) {
            DBG_CHANNELS << "collecting properties for machine " << m->getName()
                         << ":" << mi->name << "\n";
            ...
            while (props != mi->property_names.end()) {
                ...
                if (v != SymbolTable::Null) {
                    ...
                    DBG_CHANNELS << "Note: machine " << m->getName()
                                 << " does not have a property " << s
                                 << " corresponding to interface "
                                 << interface_name << "\n";
                }
            }
        } else {
            DBG_CHANNELS << "could not find the interface definition for "
                         << m->getName() << "\n";
        }
    }
}
...
bool Channel::syncRemoteStates(std::list<char *> &messages) {
    ...
    if (isClient()) {
        if (definition()->hasFeature(ChannelDefinition::ReportStateChanges)) {
            std::set<MachineInstance *>::iterator iter =
                this->channel_machines.begin();
            while (iter != channel_machines.end()) {
                MachineInstance *m = *iter++;
                if (!m->isShadow()) {
                    std::string state(m->getCurrentStateString(this));
                    DBG_CHANNELS << "Machine " << m->getName()
                                 << " current state: " << state << "\n";
        ...
        if (definition()->hasFeature(
                ChannelDefinition::ReportPropertyChanges)) {
            std::set<MachineInstance *>::iterator iter =
                this->channel_machines.begin();
            while (iter != channel_machines.end()) {
                MachineInstance *m = *iter++;
        ...
        if (definition()->hasFeature(ChannelDefinition::ReportStateChanges)) {
            ...
            while (iter != channel_machines.end()) {
                ...
                if (!m->isShadow()) {
                    ...
                    if (cmd_client) {
                        char *msg = MessageEncoding::encodeState(
                            m->getName(), state, definition()->authority);
                        messages.push_back(msg);
                        //std::string response;
...
Action::Status Channel::setState(const State &new_state, uint64_t authority,
                                 bool resume) {
    ...
    if (new_state != ChannelImplementation::DISCONNECTED && connections == 0) {
        ...
        if (!communications_manager ||
            (isClient() && !communications_manager->monit_setup)) {
            {
                FileLogger fl(program_name);
    ...
    if (res != Action::Complete) {
        ...
        snprintf(buf, 100, "Channel %s SetState to %s not complete",
                 name.c_str(), new_state.getName().c_str());
    ...
    if (new_state == ChannelImplementation::CONNECTED) {
        ...
        if (isClient()) {
            snprintf(buf, 100,
                     "Channel %s (client); setting state to DOWNLOADING",
                     name.c_str());
            MessageLog::instance()->add(buf);
            DBG_CHANNELS << buf << "\n";
            ...
            enqueueAction(ssat.factory(
                this)); // execute this state change once all other actions are done
        }
    } else if (new_state == ChannelImplementation::WAITSTART) {
        snprintf(buf, 100, "Channel %s: (%s) setting state to WAITSTART",
                 name.c_str(), (isClient()) ? "client" : "server");
        assert(!communications_manager->monit_subs.disconnected());
        MessageLog::instance()->add(buf);
        DBG_CHANNELS << buf << "\n";
        enableShadows();
    } else if (new_state == ChannelImplementation::UPLOADING) {
        snprintf(buf, 100, "Channel %s: (%s) setting state to UPLOADING",
                 name.c_str(), (isClient()) ? "client" : "server");
        MessageLog::instance()->add(buf);
        DBG_CHANNELS << buf << "\n";
        ...
        snprintf(buf, 100, "Channel %s: (%s) setting state to DOWNLOADING",
                 name.c_str(), (isClient()) ? "client" : "server");
        DBG_CHANNELS << name << " DOWNLOADING\n";
        assert(communications_manager->setup().connected());
        MessageHeader mh(MessageHeader::SOCK_CTRL, MessageHeader::SOCK_CTRL,
                         false);
        mh.start_time = microsecs();
        std::string ack;
        if (isClient()) {
            snprintf(buf, 100,
                     "Channel %s (client); sending 'status' to partner",
                     name.c_str());
            MessageLog::instance()->add(buf);
            DBG_CHANNELS << buf << "\n";
            ...
            snprintf(buf, 100,
                     "Channel %s is server; sending 'done' to partner",
                     name.c_str());
            MessageLog::instance()->add(buf);
            DBG_CHANNELS << buf << "\n";
            mh.needReply(true);
...
Action::Status Channel::setState(const char *new_state_cstr, uint64_t authority,
                                 bool resume) {
...
void Channel::addConnection() {
    ...
    if (connections == 1) {
        if (isClient()) {
            if (definition()->isPublisher()) {
                ...
                SetStateActionTemplate *ssat_connected =
                    new SetStateActionTemplate(CStringHolder("SELF"),
                                               "CONNECTED");
        ...
        if (current_state != ChannelImplementation::WAITSTART) {
            snprintf(buf, 100,
                     "Channel %s added connection %d; now waiting for start",
...
int Channel::uniquePort(unsigned int start, unsigned int end) {
    ...
    while (true) {
        try {
            zmq::socket_t test_bind(*MessagingInterface::getContext(),
                                    ZMQ_PULL);
...
MachineClass *
ChannelDefinition::interfaceFor(const char *monitored_machine) const {
    std::map<std::string, Value>::const_iterator found =
        updates_names.find(monitored_machine);
...
ChannelDefinition *ChannelDefinition::find(const char *name) {
    ...
    std::map<std::string, ChannelDefinition *>::iterator found =
        all->find(name);
    if (found == all->end()) {
        return 0;
    }
...
bool ChannelImplementation::monitors() const {
    return monitors_exports || !monitors_names.empty() ||
           !monitors_patterns.empty() || !monitors_properties.empty();
}
...
void Channel::startServer(ProtocolType proto) {
    if (mif) {
        DBG_CHANNELS
            << "Channel::startServer() called when mif is already allocated\n";
        return;
    }
    if (monitor_thread) {
...
void Channel::startClient() {
    ...
    if (mif) {
        DBG_CHANNELS
            << "Channel::startClient() called when mif is already allocated\n";
        return;
    }
...
void Channel::stopServer() {
    ...
    if (monit_subs) {
        ...
        if (disconnect_responder) {
            monit_subs->removeResponder(ZMQ_EVENT_DISCONNECTED,
...
void Channel::checkStateChange(std::string event) {
    char buf[100];
    DBG_CHANNELS << "Channel " << name << " received " << event << " in "
                 << current_state << " on " << name << "\n";

    internals->changed_state = false;
    if ((event == "ack" || event == "noevent") &&
        (current_state == ChannelImplementation::DOWNLOADING ||
         current_state == ChannelImplementation::UPLOADING)) {
        return;
    }
    ...
    if (isClient()) {
        if (current_state == ChannelImplementation::DOWNLOADING) {
            ...
            snprintf(buf, 100, "Channel %s (client) becomming ACTIVE",
                     name.c_str());
            MessageLog::instance()->add(buf);
            DBG_CHANNELS << name << " -> ACTIVE\n";
            setState(ChannelImplementation::ACTIVE);
        } else {
            DBG_CHANNELS << "Unexpected channel state " << current_state
                         << " on " << name << "\n";
            //assert(false);
        }
    } else {
        ...
        if (current_state == ChannelImplementation::CONNECTED &&
            event == "status") {
            Value wait_time(1000);
            WaitActionTemplate wat(wait_time);
            enqueueAction(wat.factory(
                this)); // execute this state change once all other actions are done
            ...
            if (event == "status") {
                ...
                DBG_CHANNELS << name << ": unexpected event " << event << " in "
                             << current_state << "\n";
            }
        } else if (current_state == ChannelImplementation::ACTIVE &&
                   event == "status") {
            {
                FileLogger fl(program_name);
...
void Channel::prepareCommandSocketAndSubscriber() {
    ...
    */
    if (isClient()) {
        ...
        if (host == SymbolTable::Null) {
            if (definition_->options.find("host") !=
                definition_->options.end()) {
        ...
        if (!port_val.asInteger(port)) {
            if (definition_->options.find("port") !=
                definition_->options.end()) {
                const Value item = definition_->options.at("port");
                port = item.iValue;
            }
            if (port == 0) {
                NB_MSG << "client Channel " << name
                       << " failed to start (port == 0)";
                exit(1);
            }
        }
        DBG_CHANNELS << " channel " << _name << " starting subscription to "
                     << host << ":" << port << "\n";
        communications_manager =
            new SubscriptionManager(definition()->name.c_str(), eCHANNEL,
                                    host.asString().c_str(), 0, (int)port);
    } else {
        communications_manager = new SubscriptionManager(
            definition()->name.c_str(), eCHANNEL, "*", port);
    }
#if 1
    {
        int retry = 1;
...
void Channel::operator()() {
    ...
    SubscriptionManager *sm =
        dynamic_cast<SubscriptionManager *>(communications_manager);
    int cmd_server_idx = 0;

    NB_MSG << name << " connecting to remote socket "
           << internals->cmd_sock_info->address << "\n";
    internals->command_sock =
        new zmq::socket_t(*MessagingInterface::getContext(), ZMQ_PAIR);
    ...
    internals->router.addRoute(MessageHeader::SOCK_CTRL, ZMQ_PAIR,
                               sock_crtl_name);
    ...
    while (!aborted && communications_manager) {
        try {
            ...
            if (items[subscriber_idx].revents & ZMQ_POLLERR) {
                {
                    ...
                    fl.f()
                        << name
                        << " thread detected error on subscriber connection\n";
                }
                continue;
            }
            ...
            {
                FileLogger fl(program_name);
                fl.f() << "zmq exception in Channel " << name << " "
                       << zmq_strerror(zmq_errno()) << "\n";
            }
        } catch (const std::exception &ex) {
            char buf[100];
            snprintf(buf, 100, "Channel %s saw exception %s", name.c_str(),
                     ex.what());
            MessageLog::instance()->add(buf);
            NB_MSG << buf << "\n";
        }
...
bool Channel::receivedStateChangeMessage(zmq::socket_t &remote_sock,
    ...
    if (safeRecv(remote_sock, &data, &len, false, 0, mh)) {
        ...
        if (strncmp(data, "done", len) == 0 ||
            ...
            if (isClient() && !mh.needsReply()) {
                snprintf(buf, 100,
                         "Error: Channel %s: need-reply flag on %s message "
                         "header missing",
            ...
            snprintf(buf, 100, "Channel %s received unexpected: %s",
                     name.c_str(), data);
            MessageLog::instance()->add(buf);
            NB_MSG << buf << "\n";
            understood = false;
        ...
        if (mh.needsReply()) {
            ...
            snprintf(buf, 100, "Channel %s sending %s to '%s' message",
                     name.c_str(), response, (data) ? data : "<empty>");
            MessageLog::instance()->add(buf);
...
bool Channel::sendMessage(const char *msg, zmq::socket_t &sock,
                          std::string &response, MessageHeader &header) {
    char tnam[100];
    int pgn_rc = pthread_getname_np(pthread_self(), tnam, 100);
    assert(pgn_rc == 0);

    if (!subscriber_thread) {
        DBG_CHANNELS << tnam << " Channel " << name << " sendMessage() sending "
                     << msg << " directly\n";
        ...
        DBG_CHANNELS << "Channel " << name
                     << " sendMessage() waiting for response\n";
        safeRecv(*cmd_client, &response_buf, &rlen, true, 0, header);
        response = response_buf;
        DBG_CHANNELS << "Channel " << name << " sendMessage() got response "
                     << response << " for " << msg
                     << " from a channel thread\n";
    }
    return true;
}
...
zmq::socket_t *Channel::createCommandSocket(bool client_endpoint) {
    std::string cmd_socket_name =
        internals->getCommandSocketName(client_endpoint);
    if (client_endpoint) {
        zmq::socket_t *sock =
            new zmq::socket_t(*MessagingInterface::getContext(), ZMQ_PAIR);
...
void Channel::startSubscriber() {
    ...
    int count = 0;
    while (++count < 10 && !internals->thread_started) {
        usleep(100);
    }
    ...
    disconnect_responder = new ChannelDisconnectMonitor(this);
    if (isClient()) {
        communications_manager->monit_subs.addResponder(ZMQ_EVENT_CONNECTED,
                                                        connect_responder);
    } else {
        communications_manager->monit_subs.addResponder(ZMQ_EVENT_ACCEPTED,
                                                        connect_responder);
    }
    communications_manager->monit_subs.addResponder(ZMQ_EVENT_DISCONNECTED,
                                                    disconnect_responder);
...
void ChannelDefinition::instantiateInterfaces() {
    ...
    while (iter != all->end()) {
        ...
        while (updated_machines != item.second->updates_names.end()) {
            const std::pair<std::string, Value> &instance_name =
                *updated_machines++;
            MachineInstance *found =
                MachineInstance::find(instance_name.first.c_str());
            if (!found) { // instantiate a shadow to represent this machine
                if (item.second) {
                    DBG_CHANNELS << "Instantiating SHADOW "
                                 << instance_name.first << " for Channel "
                                 << item.second->name << "\n";
                    MachineInstance *m = MachineInstanceFactory::create(
                        instance_name.first.c_str(),
                    ...
                    MachineClass *mc = MachineClass::find(
                        instance_name.second.asString().c_str());
                    ...
                    char buf[150];
                    snprintf(buf, 150, "Error: no interface named %s",
                             item.first.c_str());
                    MessageLog::instance()->add(buf);
                    DBG_CHANNELS << buf << "\n";
                }
        ...
        std::map<std::string, Value>::iterator i_shared =
            item.second->shares_names.begin();
        while (i_shared != item.second->shares_names.end()) {
            ...
            if (!found) { // instantiate a shadow to represent this machine
                if (item.second) {
                    DBG_CHANNELS << "instantiating shadow machine "
...
Channel *Channel::create(unsigned int port, ChannelDefinition *defn) {
    ...
    if (defn->getThrottleTime()) {
        DBG_CHANNELS << " channel " << channel_name << " is throttled ("
                     << defn->getThrottleTime() << ")\n";
...
static void copyJSONArrayToSet(cJSON *obj, const char *key,
...
static void copyJSONArrayToMap(cJSON *obj, const char *key,
                               std::map<std::string, Value> &res,
                               const char *key_name = "property",
...
static cJSON *MapToJSONArray(std::map<std::string, Value> &items,
    ...
    while (iter != items.end()) {
        ...
        cJSON_AddItemToObject(js_item, key_name,
                              cJSON_CreateString(item.first.c_str()));
...
char *ChannelDefinition::toJSON() {
    ...
    cJSON_AddItemToObject(obj, "monitors",
                          StringSetToJSONArray(this->monitors_names));
    cJSON_AddItemToObject(obj, "monitors_patterns",
                          StringSetToJSONArray(this->monitors_patterns));
    cJSON_AddItemToObject(obj, "monitors_properties",
                          MapToJSONArray(this->monitors_properties));
    ...
    cJSON_AddItemToObject(obj, "updates",
                          MapToJSONArray(this->updates_names, "name", "type"));
    cJSON_AddItemToObject(obj, "sends",
                          StringSetToJSONArray(this->send_messages));
    cJSON_AddItemToObject(obj, "receives",
                          StringSetToJSONArray(this->recv_messages));
...
void Channel::sendPropertyChangeMessage(MachineInstance *m,
                                        const std::string &name,
                                        const Value &key, const Value &val,
                                        uint64_t auth) {
    if (definition()->hasFeature(
            ChannelDefinition::ReportLocalPropertyChanges) ||
        (m->getStateMachine() &&
         m->getStateMachine()->local_properties.count(key.asString()))) {
        return;
    }
    if (communications_manager) {
        std::string response;
        ...
        if (!definition()->isPublisher()) {
            ...
            if (m->isShadow() && m->ownerChannel() == this) {
                ...
                if (!m->getStateMachine()->property_names.count(
                        key.asString())) {
                ...
                MachineClass *if_defn =
                    definition()->interfaceFor(m->getName().c_str());
                assert(if_defn);
                if (if_defn && !if_defn->property_names.count(key.asString())) {
                    return; //ignore properties no in the interface definition
                }

                cmd = MessageEncoding::encodeCommand(
        ...
        MessageHeader mh(MessageHeader::SOCK_CW, MessageHeader::SOCK_CHAN,
                         false);
        mh.start_time = microsecs();
        if (isClient()) {
...
void Channel::sendPropertyChange(MachineInstance *machine, const Value &key,
                                 const Value &val, uint64_t authority) {
    if (!all) {
        return;
    }
    ...
    while (iter != all->end()) {
        ...
        if (chn->filtersAllow(machine)) {
            if (chn->throttle_time && machine->needsThrottle()) {
                DBG_CHANNELS << chn->getName() << " throttling "
                             << machine->getName() << " " << key << "\n";
                ...
                if (chn->definition()->hasFeature(
                    ...
                    (machine->getStateMachine() &&
                     !machine->getStateMachine()->propertyIsLocal(key))) {
                    chn->sendPropertyChangeMessage(machine, machine->getName(),
                                                   key, val, authority);
                }
            }
        } else {
...
void Channel::sendThrottledUpdates() {
    ...
    bool do_modbus =
        definition()->hasFeature(ChannelDefinition::ReportModbusUpdates);
    bool do_properties =
        definition()->hasFeature(ChannelDefinition::ReportPropertyChanges);
    last_throttled_send = microsecs();
    std::map<MachineInstance *, MachineRecord *>::iterator iter =
        throttled_items.begin();
    while (iter != throttled_items.end()) {
        ...
        if (do_properties || do_modbus) {
            if (do_properties) {
                ...
                std::map<std::string, Value>::iterator props_iter =
                    props->begin();
                while (props_iter != props->end()) {
                    ...
                    if (definition()->hasFeature(
                            ChannelDefinition::ReportLocalPropertyChanges) ||
                        (mi->getStateMachine() &&
                         !mi->getStateMachine()->propertyIsLocal(key)))
                        sendPropertyChangeMessage(
                            item.second->machine,
                            item.second->machine->getName(), key, prop.second);
                    if (!do_modbus) {
            ...
            assert(
                false); // should not be here if we aren't doing properties or modbus messages
        }
        delete item.second;
...
void Channel::sendPropertyChanges(MachineInstance *machine) {
    ...
    while (iter != all->end()) {
        Channel *chn = (*iter).second;
        iter++;
        bool do_modbus = chn->definition()->hasFeature(
            ChannelDefinition::ReportModbusUpdates);
        bool do_properties = chn->definition()->hasFeature(
            ChannelDefinition::ReportPropertyChanges);
        bool do_local_properties = chn->definition()->hasFeature(
            ChannelDefinition::ReportLocalPropertyChanges);
...
bool Channel::patternMatches(const std::string &machine_name) {
    ...
    while (iter != monitors_patterns.end()) {
        ...
        if (!rexp->compilation_error) {
            ...
            DBG_CHANNELS << "Channel error: " << name << " "
...
bool Channel::doesMonitor() {
    if (!does_monitor) {
        ...
        DBG_CHANNELS << name << " monitoring " << channel_machines.size()
                     << " machines\n";
    }
    return does_monitor;
}
...
bool Channel::throttledItemsReady(uint64_t now_usecs) const {
    return !throttled_items.empty() &&
           now_usecs - last_throttled_send >= throttle_time;
}

void Channel::sendModbusUpdate(MachineInstance *machine,
    ...
    while (iter != all->end()) {
        ...
        if (chn->throttle_time && machine->needsThrottle()) {
            DBG_CHANNELS << chn->getName() << " throttling "
                         << machine->getName() << " " << property_name << "\n";
            if (!chn->throttled_items[machine]) {
                chn->throttled_items[machine] = new MachineRecord(machine);
            }
            chn->throttled_items[machine]->properties[property_name] =
                new_value;
        } else {
...
void Channel::sendStateChange(MachineInstance *machine, std::string new_state,
    ...
    while (iter != all->end()) {
        ...
        if (!chn->definition()->hasFeature(
                ChannelDefinition::ReportStateChanges)) {
            continue;
        }
        ...
        if (chn->filtersAllow(machine)) {
            ...
            if (!chn->definition()->isPublisher()) {
                if (machine->isShadow()) {
                    ...
                    cmdstr = MessageEncoding::encodeState(machine_name,
                                                          new_state, auth);
                } else {
                    //NB_MSG << "using authority " << chn->getAuthority()
                    //<< " to set " << machine_name << " to " << new_state << "\n";
                    cmdstr = MessageEncoding::encodeState(
                        machine_name, new_state, chn->getAuthority());
                }
            ...
            if (!chn->isClient() && chn->communications_manager) {
                std::string response;
                MessageHeader mh(MessageHeader::SOCK_CW,
                                 MessageHeader::SOCK_CHAN, false);
                mh.start_time = microsecs();
                ...
                //chn->sendMessage(cmdstr, *chn->cmd_client, response);
                       ...
                       chn->communications_manager->setupStatus() ==
                           SubscriptionManager::e_done) {
                MessageHeader mh(MessageHeader::SOCK_CW,
                                 MessageHeader::SOCK_CHAN, false);
                mh.start_time = microsecs();
...
void Channel::requestStateChange(MachineInstance *machine,
                                 std::string new_state, uint64_t auth) {
    ...
    if (filtersAllow(machine)) {
        ...
        if (!definition()->isPublisher()) {
            if (machine->isShadow()) {
                cmdstr =
                    MessageEncoding::encodeState(machine_name, new_state, auth);
            } else {
                //NB_MSG << "using authority " << chn->getAuthority()
                //<< " to set " << machine_name << " to " << new_state << "\n";
                cmdstr = MessageEncoding::encodeState(machine_name, new_state,
        ...
        if (!isClient() && communications_manager) {
            ...
            safeSend(*cmd_client, cmdstr, strlen(cmdstr), mh);
                   ...
                   communications_manager->setupStatus() ==
                       SubscriptionManager::e_done) {
...
void Channel::sendCommand(MachineInstance *machine, std::string command,
                          std::list<Value> *params, MessageHeader mh) {
    char tnam[100];
    int pgn_rc = pthread_getname_np(pthread_self(), tnam, 100);
    assert(pgn_rc == 0);
    ...
    if (machine->isShadow()) {
        ...
        DBG_CHANNELS << " redirecting " << command << " to "
                     << machine->getName() << " owner channel\n";
        if (command == "UPDATE") {
            //assert(false);
            return; // there should be no way for modbus updates to go to shadows
        }
        char *cmd =
            MessageEncoding::encodeCommand(command, params); // send command
        ...
        while (iter != all->end()) {
            ...
            if (command == "UPDATE" &&
                !chn->definition()->hasFeature(
                    ChannelDefinition::ReportModbusUpdates)) {
                continue;
            }
            DBG_CHANNELS << " sending " << command << " of "
                         << machine->getName() << " to " << chn->getName()
                         << "\n";
            ...
            if (chn->filtersAllow(machine)) {
                if ((!chn->isClient() && chn->communications_manager) ||
                    ...
                    std::string response;
                    char *cmd = MessageEncoding::encodeCommand(
                        command, params); // send command
                    DBG_CHANNELS << "Channel " << chn->name << " sending "
                                 << cmd << " on cmd_server\n";
                    MessageHeader mh(MessageHeader::SOCK_CW,
                                     MessageHeader::SOCK_CHAN, false);
                    mh.start_time = microsecs();
...
void ChannelImplementation::removeMonitor(const char *s) {
    ...
    if (monitors_names.count(s)) {
        ...
        DBG_CHANNELS << "remove monitor for " << s
                     << "...not found adding ignore pattern\n";
        std::string pattern = "^";
        pattern += s;
        pattern += "$";
...
void ChannelImplementation::addMonitorProperty(const char *key,
                                               const Value &val) {
    monitors_properties[key] = val;
    modified();
}
void ChannelImplementation::removeMonitorProperty(const char *key,
                                                  const Value &val) {
...
void Channel::enableShadows() {
    ...
    std::map<std::string, Value>::const_iterator iter =
        definition()->updates_names.begin();
    while (iter != definition()->updates_names.end()) {
        ...
        if (ms) {
            ...
            if ((isClient() && authority == machine_auth) ||
                ...
                DBG_CHANNELS << "Channel " << name
                             << " enabling shadow machine " << ms->getName()
                             << "\n";
    ...
    while (iter != definition()->updates_names.end()) {
        ...
        if (ms) {
            ...
            if ((isClient() && authority == machine_auth) ||
                (definition()->authority == machine_auth)) {
                DBG_CHANNELS << "Channel " << name
                             << " disabling shadow machine " << ms->getName()
                             << "\n";
                ms->disable();
            }
        }
...
void Channel::setupShadows() {
    ...
    if (!definition_) {
        char buf[150];
        snprintf(buf, 150,
                 "Error: channel definition %s for %s could not be found",
                 name.c_str(), _type.c_str());
        MessageLog::instance()->add(buf);
        return;
    }
    ...
    while (iter != definition()->updates_names.end()) {
        ...
        if (m && !ms) { // this machine is not a shadow.
            ...
            DBG_CHANNELS << "Channel " << name << " adding shadow machine "
                         << m->getName() << "\n";
            channel_machines.insert(m);
            modified();
...
CommandSocketInfo::CommandSocketInfo(Channel *chn) : sock(0), index(0) {
    ...
    DBG_CHANNELS << "Bound command channel socket (processing side) of "
                 << chn->getName() << " to " << buf << " at index " << index
                 << "\n";
}
...
void Channel::setupCommandSockets() {
    ...
    while (iter != all->end()) {
        ...
        if (chn->internals->cmd_sock_info) {
            ProcessingThread::instance()->addCommandChannel(
                chn->internals->cmd_sock_info);
            NB_MSG << "channel " << chn->getName()
                   << " already has a command socket.. using it\n";
            continue;
        }
        if (!chn->definition()) {
            NB_MSG << "channel " << chn->getName()
        ...
        if (chn->definition() && !chn->definition()->isPublisher()) {
            try {
                ...
                NB_MSG << tnam << " " << chn->name
                       << " remote end bound to socket "
            ...
            NB_MSG << chn->name
                   << " s a publisher. not using a command socket\n";
        }
    }
}
...
void Channel::setupFilters() {
    ...
    if (definition()->monitor_linked.size()) {
        std::set<std::string>::const_iterator iter =
            definition()->monitor_linked.begin();
        while (iter != definition()->monitor_linked.end()) {
            const std::string &machine_name = *iter++;
            MachineInstance *master =
                MachineInstance::find(machine_name.c_str());
            if (!master) {
                char buf[150];
                snprintf(buf, 150, "Channel %s cannot find master machine %s",
                         name.c_str(), machine_name.c_str());
                MessageLog::instance()->add(buf);
                DBG_CHANNELS << buf << "\n";
            }
            std::list<MachineInstance *>::iterator m_iter =
    ...
    std::set<std::string>::iterator iter =
        definition()->monitors_patterns.begin();
    while (iter != definition()->monitors_patterns.end()) {
        ...
        if (!rexp->compilation_error) {
            ...
            while (m_iter != MachineInstance::end()) {
                MachineInstance *machine = *m_iter++;
                if (machine &&
                    execute_pattern(rexp, machine->getName().c_str()) == 0) {
    ...
    definition()->processIgnoresPatternList(
        definition()->ignores_patterns.begin(),
    ...
    definition()->processIgnoresPatternList(ignores_patterns.begin(),
                                            ignores_patterns.end(), this);
    ...
    does_monitor = monitors_exports || definition()->monitors_exports ||
                   !(definition()->monitors_names.empty() &&
                     definition()->monitors_patterns.empty() &&
                     definition()->monitors_properties.empty() &&
                     definition()->ignores_patterns.empty() &&
                     ignores_patterns.empty() && monitors_names.empty() &&
                     monitors_patterns.empty() && monitors_properties.empty());
...
void ChannelDefinition::processIgnoresPatternList(
    std::set<std::string>::const_iterator iter,
    ...
    while (iter != last) {
        ...
        rexp_info *rexp = create_pattern(pattern.c_str());
        if (!rexp->compilation_error) {
            std::list<MachineInstance *>::iterator machines =
                MachineInstance::begin();
            while (machines != MachineInstance::end()) {
                ...
                if (machine &&
                    ...
                    if (chn->channel_machines.count(machine)) {
                        ...
                        DBG_CHANNELS << "ignore pattern " << pattern
                                     << " matches " << machine->fullName()
                                     << " but it is not monitored\n";
                    }
                } else {
                    DBG_CHANNELS << machine->getName() << " does not match "
                                 << pattern << "\n";
                }
            }
        } else {
