...
double FloatBuffer::inner_product(double *coefficients,
                                  unsigned int num_coeff) const {
    float sum = 0;
...
float ButterworthFilter::filter(float x) {
    ...
    double s1 = filtered_buf.inner_product(
        d_coefficients + 1, num_d_coefficients - 1); // convolved output
    ...
    if (n > (unsigned int)BUFSIZE / 2) {
        ...
        if (missing >= 3) {
            ...
            for (int i = missing; i > 0; --i) {
                quickAppend(val - i * mean_change,
                            time - i * (time - last_time) / missing);
            }
        }
    }
    front = (front + 1) % BUFSIZE;
...
double
SampleBuffer::rate() const // returns dv/dt between the two sample positions
{
    if (front == back || back == -1) {
        return 0.0f;
    }
