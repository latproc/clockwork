    ...
    if (params.size() == 2) {
        ...
        if (device) {
            ...
            MachineInstance *machine =
                MachineInstance::find(params[1].asString().c_str());
            if (machine) {
                ok = true;
                result_str = machine->getCurrentStateString(
                    0); // iosh shows the public state
            } else {
                error_str = "Not Found";
            }
        }
...
bool IODCommandSetStatus::run(std::vector<Value> &params) {
    ...
    uint64_t auth = 0;
    if (params.size() == 5 ||
        (params.size() == 4 && params[2] == "TO")) { // SET machine TO state
        ...
        if (params.size() == 5) {
            ...
            if (params[4].asInteger(res)) {
               ...
               params.size() == 4) { // STATE machine state
        ds = params[1].asString();
    ...
    if (device) {
        ...
        if (mi) {
            if (!mi->getStateMachine()) {
                char buf[150];
                snprintf(buf, 150, "Error: machine %s has no state machine",
                         mi->getName().c_str());
            ...
            if (!s) {
                if (mi->isShadow()) {
                    ...
                    char buf[150];
                    snprintf(buf, 150,
                             "Error: machine %s has no state called '%s'",
...
bool IODCommandToggle::run(std::vector<Value> &params) {
    if (params.size() == 2) {
        ...
        Output *device = dynamic_cast<Output *>(
            IOComponent::lookup_device(params[1].asString()));
        if (device) {
            if (device->isOn()) {
                device->turnOff();
            ...
            // MQTT
            if (m->getCurrent(0).getName() == "on" ||
                m->getCurrent(0).getName() == "off") {
                ...
                m->execute(new Message(msg_str.c_str(), Message::ENTERMSG),
                           m->mq_interface);
                result_str = "OK";
                return true;
            } else {
...
bool IODCommandProperty::run(std::vector<Value> &params) {
    ...
    if (m) {
        if (params.size() > 5) {
            ...
            if (m->debug()) {
                DBG_PROPERTIES << "setting property " << params[1] << "."
                               << params[2] << " to " << params[3] << "\n";
            }
            long authority = 0;
            ...
            if (use_authority && authority && !m->isShadow()) {
                error_str =
                    "Refusing to change property due to authorisation conflict";
                NB_MSG << error() << "\n";
                return false;
            }
            if (params[3].kind == Value::t_string ||
                params[3].kind == Value::t_symbol) {
                long x;
                char *p;
                ...
                else if (params[3].asString().length() > 0 && *p == 0) {
                ...
                if (use_authority) {
                    changed = m->setValue(params[2].asString(),
                                          params[3].asString(), authority);
                } else {
                    changed =
                        m->setValue(params[2].asString(), params[3].asString());
                }
            }
        }
        if (changed) {
...
bool IODCommandList::run(std::vector<Value> &params) {
    ...
    std::list<MachineInstance *>::const_iterator iter =
...
bool IODCommandFind::run(std::vector<Value> &params) {
    std::ostringstream ss;
    std::map<std::string, MachineInstance *>::const_iterator iter =
        machines.begin();
    while (iter != machines.end()) {
        MachineInstance *m = (*iter).second;
        const Value &val = m->properties.lookup("VALUE");
        if (params.size() == 1 ||
            m->getName().find(params[1].asString()) != std::string::npos) {
            if (val != SymbolTable::Null) {
                ss << m->getName() << " " << m->_type << " " << val
                   << ((m->enabled()) ? "" : " [DISABLED]") << "\n";
            } else {
                ss << m->getName() << " " << m->_type << " "
                   ...
                   (m->getStateMachine() &&
                    m->getStateMachine()->name.find(params[1].asString()) !=
                        std::string::npos)) {
            ss << (m->getName()) << " " << m->_type << " "
               << m->getCurrentStateString(0)
               << ((m->enabled()) ? "" : " [DISABLED]") << "\n";
...
cJSON *printMachineInstanceToJSON(MachineInstance *m, std::string prefix = "") {
    ...
    if (m->io_interface) {
        ...
        if (mod) {
            cJSON_AddStringToObject(node, "module_name",
                                    mod->getName().c_str());
        }
    }
    // #endif
    ...
    while (st_iter != m->properties.end()) {
        ...
        if (item.second.kind == Value::t_integer) {
            cJSON_AddNumberToObject(node, item.first.c_str(),
                                    item.second.iValue);
        } else if (item.second.kind == Value::t_float) {
            cJSON_AddItemToObject(node, item.first.c_str(),
                                  cJSON_CreateDouble(item.second.fValue));
        } else {
            cJSON_AddStringToObject(node, item.first.c_str(),
                                    item.second.asString().c_str());
        }
    }
    Action *action;
        ...
        if (!limited || (limited && m->properties.lookup("tab") == tab)) {
            ...
            for (unsigned int idx = 0; idx < m->locals.size(); ++idx) {
                ...
                cJSON_AddItemToArray(
                    root, printMachineInstanceToJSON(p.machine, m->getName()));
            }
        }
    }
    char *res = cJSON_Print(root);
...
bool IODCommandSend::run(std::vector<Value> &params) {
    ...
    if (!m) {
        if ((params.size() != 2 && params.size() != 4) ||
            ...
            error_str =
                "Usage: SEND machine.command | SEND command TO machine ";
            return false;
        }
    ...
    std::stringstream ss;
    if (m) {
        m->sendMessageToReceiver(new Message(strdup(command.c_str())), m,
                                 false);
        if (m->_type == "LIST") {
            for (unsigned int i = 0; i < m->parameters.size(); ++i) {
                MachineInstance *entry = m->parameters[i].machine;
                if (entry) {
                    m->sendMessageToReceiver(
                        new Message(strdup(command.c_str())), entry);
                }
            }
        ...
        ss << "Could not find machine " << machine_name << " for command "
           << command << std::flush;
    }
    error_str = ss.str();
    return false;
...
bool IODCommandModbus::run(std::vector<Value> &params) {
    ...
    if (!params[1].asInteger(group) || !params[2].asInteger(address)) {
        std::stringstream ss;
        ss << "malformed modbus command: " << params[0] << " " << params[1]
           << " " << params[2] << " " << params[3];
        error_str = ss.str();
        MessageLog::instance()->add(ss.str().c_str());
        return false;
    }
    DBG_MODBUS << "modbus group: " << group << " addresss " << address
               << " value " << params[3] << "\n";
    ...
    if (found.getGroup() != ModbusAddress::none) {
        if (found.getOwner()) {
            ...
            if (params[3].kind == Value::t_integer) {
                found.getOwner()->modbusUpdated(
                    found, (int)(address - found.getAddress()),
                    (int)params[3].iValue);
                ...
                found.getOwner()->modbusUpdated(
                    found, (int)(address - found.getAddress()),
                    (params[3].bValue) ? 1 : 0);
            } else if (params[3].kind == Value::t_string ||
                       params[3].kind == Value::t_symbol) {
                long val;
                if (params[3].asInteger(val)) {
                    found.getOwner()->modbusUpdated(
                        found, (int)(address - found.getAddress()), (int)val);
                } else {
                    found.getOwner()->modbusUpdated(
                        found, (int)(address - found.getAddress()),
                ...
                std::stringstream ss;
                ss << "unexpected value type " << params[3].kind
                   << " for modbus value\n";
                std::cout << ss.str() << "\n";
                error_str = ss.str();
                return false;
        ...
        std::stringstream ss;
        ss << "failed to find Modbus Address matching group: " << group
           << ", address: " << address;
        error_str = ss.str();
        std::cout << error_str << "\n";
        return false;
...
bool IODCommandModbusExport::run(std::vector<Value> &params) {
    ...
    if (rename(file_name, backup_file_name) && errno != ENOENT) {
        std::cerr << "file rename error: " << strerror(errno) << " (" << errno
                  << ")"
                  << "\n";
    }
    ...
    while (m_iter != MachineInstance::end()) {
        ...
        if (m && (m->getValue("PERSISTENT") == "true")) {
            ...
            while (props_i != m->properties.end()) {
                ...
                cJSON_AddItemToArray(json_item,
                                     cJSON_CreateString(fnam.c_str()));
                cJSON_AddItemToArray(json_item,
    ...
    while (m_iter != MachineInstance::end()) {
        ...
        if (m->stable_states_stats.getCount()) {
            ...
            cJSON_AddItemToArray(stat,
                                 cJSON_CreateString(m->fullName().c_str()));
            cJSON_AddItemToArray(
                stat,
        ...
        if (m->message_handling_stats.getCount()) {
            ...
            cJSON_AddItemToArray(stat,
                                 cJSON_CreateString(m->fullName().c_str()));
            cJSON_AddItemToArray(
                stat, cJSON_CreateString(
                          m->message_handling_stats.getName().c_str()));
...
bool IODCommandChannels::run(std::vector<Value> &params) {
    ...
    while (iter != channels->end()) {
        result += (*iter).first + " " +
                  (*iter).second->getCurrentStateString(0)
...
bool IODCommandChannelRefresh::run(std::vector<Value> &params) {
    ...
    if (chn->definition()->monitorsLinked()) {
        std::set<MachineInstance *>::const_iterator iter =
            chn->channelMachines().begin();
        while (iter != chn->channelMachines().end()) {
            MachineInstance *m = *iter++;
            cJSON *obj = cJSON_CreateObject();
            cJSON_AddStringToObject(obj, "name", m->getName().c_str());
            cJSON_AddStringToObject(obj, "address",
                                    m->parameters[1].val.asString().c_str());
            long val;
            if (m->parameters.size() > 2 &&
                m->parameters[2].val.asInteger(val)) {
            ...
            if (format != SymbolTable::Null) {
                cJSON_AddStringToObject(obj, "format",
...
bool IODCommandChannel::run(std::vector<Value> &params) {
    if (params.size() >= 2) {
        ...
        if (chn) {
            ...
            if (n == 6 && params[2] == "ADD" && params[3] == "MONITOR" &&
                params[4] == "PATTERN") {
                chn->addMonitorPattern(params[5].asString().c_str());
                result_str = "OK";
                return true;
            }
            if (n == 7 && params[2] == "ADD" && params[3] == "MONITOR" &&
                params[4] == "PROPERTY") {
                chn->addMonitorProperty(params[5].asString().c_str(),
                                        params[6]);
                result_str = "OK";
                return true;
            }
            ...
            if (n == 7 && params[2] == "REMOVE" && params[3] == "MONITOR" &&
                params[5] == "PROPERTY") {
                chn->removeMonitorProperty(params[5].asString().c_str(),
                                           params[6]);
                result_str = "OK";
                return true;
            }
            ...
            if (ch_name == "PERSISTENCE_CHANNEL") {
                if (!persistent_store()) {
                    char buf[100];
                    snprintf(buf, 100,
                             "Persistence is not configured for channel %s",
            ...
            ChannelDefinition *defn =
                ChannelDefinition::find(ch_name.asString().c_str());
            if (!defn) {
                char buf[100];
                snprintf(buf, 100, "No such channel: %s",
                         ch_name.asString().c_str());
                MessageLog::instance()->add(buf);
                error_str = MessageEncoding::encodeError(buf);
                return false;
            ...
            if (!chn) {
                ...
                {
                    char buf[120];
                    snprintf(buf, 120, "Channel %s created on port %ld",
                             ch_name.asString().c_str(), portval.iValue);
                    MessageLog::instance()->add(buf);
                }
                while (true) {
                    try {
                        ...
                        assert(chn);
                        if (ch_name == "PERSISTENCE_CHANNEL") {
                            chn->setValue("PersistentStore", persistent_store(),
                                          Value::t_string);
                        }
                        chn->start();
                        chn->enable();
        ...
        cJSON_AddNumberToObject(res_json, "authority",
                                chn->definition()->getAuthority());
