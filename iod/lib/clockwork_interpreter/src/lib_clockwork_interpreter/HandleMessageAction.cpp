...
Action::Status HandleMessageAction::run() {
    ...
    DBG_M_MESSAGING << "Handling message " << *package.message << " from "
                    << package.transmitter->getName() << " to "
                    << owner->getName() << "\n";
    ...
    if (owner->io_interface && package.transmitter == owner->io_interface) {
        ...
        if (package.needs_receipt) {
            MachineInstance *mi =
                dynamic_cast<MachineInstance *>(package.transmitter);
    ...
    handler = owner->findHandler(*package.message, package.transmitter,
                                 package.needs_receipt);
    if (handler) {
        suspend();
        Status stat = (*handler)();
        if (stat == Failed) {
...
std::ostream &HandleMessageAction::operator<<(std::ostream &out) const {
    ...
    if (package.transmitter) {
        out << " from " << package.transmitter->getName() << " to "
            << owner->getName();
    }
    return out;
}
