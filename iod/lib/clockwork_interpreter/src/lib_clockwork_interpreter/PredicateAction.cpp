...
void PredicateActionTemplate::toC(std::ostream &out, std::ostream &vars) const {
    ...
    if (predicate->left_p->entry.kind == Value::t_symbol) {
        ...
        if (item != symbols.end()) {
            ...
            psd = Predicate::PredicateSymbolDetailsFromValue(
        ...
        if (ExportState::instance()->remotes().find(psd.export_name) !=
            ...
            out << machine << "->set_value(" << machine << ",\""
                << var.substr(var.rfind('.') + 1) << "\" ,"
    ...
    if (v.kind == Value::t_symbol) {
        if (v.sValue == "TIMER") {
            ...
            if (prop != SymbolTable::Null) {
                ...
                DBG_PREDICATES
                    << "Predicate action resolve failed to resolve a value: "
                    << v << "\n";
            }
        }
...
Value eval(Predicate *p, MachineInstance *m) {
    ...
    if (p->left_p || p->right_p) {
        ...
        if (m && m->debug()) {
            DBG_PREDICATES << " eval - left: " << l << " op: " << p->op
                           << " right: " << r << "\n";
        }
        Value res;
        switch (p->op) {
        ...
        case opNone:
            ...
            break;
        default:
            std::cerr << "Error: unhandled operator " << p->op
                      << " in evaluating predicate\n";
        }

        if (m && m->debug()) {
            if (p->op == opNOT || p->op == opInteger || p->op == opFloat) {
                DBG_PREDICATES << " expr: " << p->op << " " << *(p->right_p)
                               << " returns " << res << "\n";
            } else {
                if (p->left_p && p->right_p) {
...
Action::Status PredicateAction::run() {
    ...
    if (predicate->left_p && predicate->left_p->entry.kind == Value::t_symbol) {
        ...
        if (owner->getStateMachine()->global_references.count(name)) {
            MachineInstance *global_machine =
                owner->getStateMachine()->global_references[name];
            if (!global_machine) {
                std::cerr << owner->getName()
                          << " Error, can't find machine for global " << name
                          << "\n"
                          << std::flush;
                abort();
            } else {
            ...
            DBG_M_PREDICATES << "Telling " << owner->getName()
                             << " to set property " << name << " to " << val
                             << " (type: " << val.kind << ")\n";
...
std::ostream &PredicateAction::operator<<(std::ostream &out) const {
    out << "Assignment " << predicate->left_p->entry
        << " := " << *(predicate->right_p);
    return out;
}
