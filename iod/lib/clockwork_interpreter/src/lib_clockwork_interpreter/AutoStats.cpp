...
AutoStatStorage::AutoStatStorage(const char *time_name, const char *rate_name,
                                 unsigned int reset_every)
    : start_time(0), last_update(0), total_polls(0), total_time(0),
      total_delays(0), rate_property(0), time_property(0),
      reset_point(reset_every) {
    if (time_name) {
        time_property = setupPropertyRef("SYSTEM", time_name);
    }
    if (rate_name) {
        rate_property = setupPropertyRef("SYSTEM", rate_name);
    }
}
void AutoStatStorage::reset() {
    last_update = 0;
    total_polls = 0;
    total_time = 0;
}
...
const long *AutoStatStorage::setupPropertyRef(const char *machine_name,
...
AutoStat::AutoStat(AutoStatStorage &storage) : storage_(storage) {
