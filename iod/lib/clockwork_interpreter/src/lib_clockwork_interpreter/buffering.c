...
double savitsky_golay_filter(struct CircularBuffer *buf,
                             unsigned int filter_len, double *coefficients,
                             float normal) {
...
int main(int argc, const char *argv[]) {
    ...
    ++tests;
    if (sum(mybuf, size(mybuf)) / test_buffer_size !=
        average(mybuf, size(mybuf))) {
        fail(tests);
    }
    if (test_buffer_size ==
        4) { /* this test only works if the buffer is of bufferLength 4 */
        ++tests;
