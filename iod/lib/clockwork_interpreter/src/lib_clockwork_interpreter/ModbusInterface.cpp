...
std::map<std::string, ModbusAddressDetails>
    ModbusAddress::preset_modbus_mapping;
...
ModbusAddress ModbusAddress::alloc(ModbusAddress::Group g, unsigned int n,
                                   ModbusAddressable *ma,
    ...
    DBG_MODBUS << "Modbus allocated " << n << " addresses in group " << (int)g
               << " starting at " << address << "\n";
    ModbusAddress addr =
        ModbusAddress(g, address, n, ma, src, full_name, kind, 0);
...
ModbusAddress::ModbusAddress(ModbusAddress::Group g, int a, int n,
                             ModbusAddressable *o, Source src,
                             const std::string &nam, ModbusExport::Type exType,
                             int i)
    : group(g), address(a), allocated(n), exportType(exType), offset(i),
      source(src), owner(o), name(nam) {
    switch (g) {
    case discrete:
    ...
    DBG_MODBUS << "Modbus recording " << n << " addresses in group " << (int)g
               << " starting at " << address << "\n";
    int index = ((int)g << 16) + address;
    if (address < first_auto_address) {
...
void ModbusAddress::update(MachineInstance *owner, Group group, int addr,
                           const char *str_value, int len) {
...
void ModbusAddress::update(MachineInstance *owner, int index, int new_value,
                           int len) {
...
ModbusAddress ModbusAddress::lookup(int group, int address) {
    ...
    DBG_MODBUS << "looking up modbus " << group << ":" << address
               << ". Auto size: " << automatic_mappings.size()
               << " local mappings " << user_mappings.size() << "\n";
