...
Transition::Transition(State s, State d, Message t, Predicate *p,
                       bool abort_semantics)
    : source(s), dest(d), trigger(t), condition(0),
      abort_on_failure(abort_semantics) {
    if (p) {
...
Transition::Transition(const Transition &other)
    : source(other.source), dest(other.dest), trigger(other.trigger),
      condition(0), abort_on_failure(other.abort_on_failure) {
