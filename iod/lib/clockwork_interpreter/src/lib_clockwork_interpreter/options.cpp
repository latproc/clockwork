...
void set_graph_root(const char *root) { graph_root_name = strdup(root); }

const char *graph_root() { return graph_root_name; }
...
int cpu_affinity(const char *thread_name) {
    ...
    std::map<std::string, int>::iterator found =
        thread_cpu_affinities.find(property_name);
...
bool exceptions_enabled() { return throw_exceptions; }

void enable_exceptions(bool which) { throw_exceptions = which; }
