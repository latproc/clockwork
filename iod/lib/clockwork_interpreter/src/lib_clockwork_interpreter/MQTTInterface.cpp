...
void my_message_callback(struct mosquitto *mosq, void *obj,
    ...
    if (message->payloadlen) {
        ...
        if (pos != device->handlers.end()) {
            ...
            if (m->_type == "POINT" &&
                (event == "on_enter" || event == "off_enter")) {
...
void my_subscribe_callback(struct mosquitto *mosq, void *obj, int mid,
    ...
    out << "Subscribed (mid: " << mid << "): " << granted_qos[0];
...
bool MQTTModule::publish(const std::string &topic, const std::string &message,
                         MachineInstance *m) {
    pubs.add(topic.c_str(), message.c_str());
    const Value &msg_val = pubs.lookup(topic.c_str());
    int rc = mosquitto_publish(mosq, &mid_sent, topic.c_str(),
                               (int)msg_val.asString().length(),
