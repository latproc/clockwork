...
EnableActionTemplate::EnableActionTemplate(const std::string &name,
                                           const char *property,
                                           const Value *val)
...
EnableActionTemplate::EnableActionTemplate(std::list<std::string> &names,
                                           const char *property,
                                           const Value *val)
    : property_name(0), property_value(0) {
...
std::ostream &EnableAction::operator<<(std::ostream &out) const {
    out << "EnableAction ";
    typename std::list<
        std::pair<std::string, MachineInstance *>>::const_iterator iter =
        machines.begin();
...
Action::Status EnableAction::run() {
    ...
    typename std::list<std::pair<std::string, MachineInstance *>>::iterator
        iter = machines.begin();
    while (iter != machines.end()) {
        ...
        if (item.second) {
            ...
            char buf[150];
            snprintf(buf, 150, "EnableAction Error: cannot find %s from %s",
                     item.first.c_str(), owner->getName().c_str());
