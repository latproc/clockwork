...
ExecuteMessageActionTemplate::ExecuteMessageActionTemplate(CStringHolder msg,
                                                           CStringHolder dest)
    : message(msg), target(dest), target_machine(0) {}

ExecuteMessageActionTemplate::ExecuteMessageActionTemplate::
...
std::ostream &
ExecuteMessageActionTemplate::operator<<(std::ostream &out) const {
    return out << "ExecuteMessageActionTemplate " << message.get() << " on "
               << target.get();
}

ExecuteMessageAction::ExecuteMessageAction(MachineInstance *mi,
                                           ExecuteMessageActionTemplate &eat)
    : Action(mi), message(eat.message), target(eat.target),
      target_machine(eat.target_machine) {}
...
std::ostream &ExecuteMessageAction::operator<<(std::ostream &out) const {
    return out << "ExecuteMessageAction " << message.get() << " on "
               << target.get();
}
