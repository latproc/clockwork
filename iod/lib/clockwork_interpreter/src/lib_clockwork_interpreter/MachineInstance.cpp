...
uint64_t rate_calc_process_time =
    0; // used for idle calculations for rate estimation
...
class MachineInstance::Cache {
  public:
    ...
    Cache()
        : full_name(0), modbus_name(0), reported_error(false),
...
MachineInstance *
MachineInstanceFactory::create(CStringHolder name, const char *type,
                               MachineInstance::InstanceType instance_type) {
    if (instance_type == MachineInstance::MACHINE_SHADOW) {
        return new MachineShadowInstance(name, type,
    ...
    if (strcmp(type, "COUNTERRATE") == 0) {
        ...
        if (defn) {
            ...
            chn->properties.add(
                defn->properties); // overwrite with instance properties
...
void MachineInstance::setNeedsCheck() {
    ...
    if (state_machine->token_id == ClockworkToken::LIST) {
        ...
        while (dep_iter != depends.end()) {
            ...
            if (dep->is_enabled &&
                dep->needs_check < 2) { // <2 is an anti-recursion check
                dep->setNeedsCheck();
            }
        }
...
std::string MachineInstance::modbusName(const std::string &property,
                                        const Value &property_val) {
    std::string res;
...
std::list<Package *> &MachineInstance::pendingEvents() {
    return pending_events;
}
std::set<MachineInstance *> &MachineInstance::pluginMachines() {
    return plugin_machines;
}
...
void MachineInstance::checkActions() {
    ...
    SharedWorkSet::instance()->add(this);
    ProcessingThread::activate(this);
    DBG_AUTOSTATES << _name << " ADDED to machines with work "
                   << SharedWorkSet::instance()->size() << "\n";
}
...
const char *
MachineInstance::getCurrentStateString(MachineInstance *scope) const {
    return (scope == this) ? current_state.getName().c_str()
                           : visible_state.getName().c_str();
}
...
MachineInstance *MachineInstance::lookup(Value &val) {
    if (!val.cached_machine &&
        (val.kind == Value::t_symbol || val.kind == Value::t_string)) {
...
bool MachineInstance::uses(MachineInstance *other) {
    ...
    if (other->_type == "DIGITALINPUT") {
        return true;
    }
    if (other->_type == "DIGITALOUTPUT") {
        return true;
    }
    if (_type == "POINT") {
        return false;
...
MachineInstance::lookup_cache_miss(const std::string &seek_machine_name) {
    ...
    for (unsigned int i = 0; i < locals.size(); ++i) {
        ...
        if (p.val.kind == Value::t_symbol &&
            p.val.sValue == seek_machine_name) {
            //ss << "...local";
            found = p.machine;
            goto cache_local_name;
        }
...
cache_local_name:
    ...
    if (found) {
        if (seek_machine_name != found->getName()) {
            DBG_M_MACHINELOOKUPS << " linked " << seek_machine_name << " to "
                                 << found->getName() << "\n";
        }
...
std::ostream &MachineValue::operator<<(std::ostream &out) const {
    ...
    if (last_result != SymbolTable::Null) {
        ...
        out << local_name << " ("
            << ((machine_instance) ? machine_instance->getName() : "NULL");
    }
    out << ")";
    return out;
}
...
class VariableValue : public DynamicValue {
  public:
    ...
    virtual Value &operator()(MachineInstance *m) {
        if (machine_instance->getStateMachine()->token_id ==
                ClockworkToken::VARIABLE ||
            machine_instance->getStateMachine()->token_id ==
                ClockworkToken::CONSTANT) {
            last_result = machine_instance->getValue("VALUE");
            return last_result;
        } else {
            last_result =
                *machine_instance->getCurrentStateVal(machine_instance);
            return last_result;
        }
    }
  ...
  public:
    ...
    Value &operator()(MachineInstance *m) {
        ...
        if (machine_instance->enabled()) {
            ...
            uint64_t msecs =
                get_diff_in_microsecs(&now, &machine_instance->start_time) /
                1000;
            if (sizeof(long) == 4) {
                msecs = msecs & 0x7fffffff;
            }
...
MachineInstance::MachineInstance(InstanceType instance_type)
    : Receiver(""), _type("Undefined"), io_interface(0), mq_interface(0),
      owner(0), needs_check(0), uses_timer(false),
      stable_states_stats("StableState processing"),
      message_handling_stats("Message handling"),
      my_instance_type(instance_type), state_machine(0),
      current_state("undefined"), visible_state(current_state),
      is_enabled(false), state_timer(0), locked(0),
      modbus_exported(ModbusExport::none), saved_state("undefined"),
      current_state_val("undefined"), is_active(false), current_value_holder(0),
      last_state_evaluation_time(0), data(0), idle_time(0), next_poll(0),
      is_traceable(false), published(0), cache(0), action_errors(0), trigger(0),
...
MachineInstance::MachineInstance(CStringHolder name, const char *type,
                                 InstanceType instance_type)
    : Receiver(name), _type(type), io_interface(0), mq_interface(0), owner(0),
      needs_check(0), uses_timer(false), my_instance_type(instance_type),
      state_machine(0), current_state("undefined"),
      visible_state(current_state), is_enabled(false), state_timer(0),
      locked(0), modbus_exported(ModbusExport::none), saved_state("undefined"),
      current_state_val("undefined"), is_active(false), current_value_holder(0),
      last_state_evaluation_time(0),
      stable_states_stats("StableState processing"),
      message_handling_stats("Message handling"), data(0), idle_time(0),
      is_traceable(false), published(0), cache(0), action_errors(0), trigger(0),
      owner_channel(0), expected_authority(0) {
    if (!shared) {
        shared = new SharedCache;
...
void MachineInstance::describe(std::ostream &out) {
    out << "---------------\n"
        ...
        << " Class: " << _type << (enabled() ? "" : " DISABLED")
        << (isShadow() ? " SHADOW " : " ") << (isActive() ? "" : " PASSIVE");
        ...
        for (unsigned int i = 0; i < parameters.size(); i++) {
            ...
            if (p_i.kind == Value::t_symbol) {
                out << "  parameter " << (i + 1) << " " << p_i.sValue << " ("
                    ...
                    << (parameters[i].machine
                            ? parameters[i].machine->getCurrent(this).getName()
                            : "")
                    << (parameters[i].machine &&
                                !parameters[i].machine->enabled()
                            ? " DISABLED"
                            : "")
                    << "\n";
            } else {
    ...
    if (locals.size()) {
        ...
        for (unsigned int i = 0; i < locals.size(); ++i) {
            if (locals[i].machine) {
                out << "  " << locals[i].val << " ("
                    << locals[i].machine->getName()
                    << "):   " << locals[i].machine->getCurrent(this).getName()
                    << "\n";
            } else {
    ...
    if (modbus_exports.size()) {
        out << "Exports:\n  ";
        std::map<std::string, ModbusAddress>::const_iterator iter =
            modbus_exports.begin();
        ...
        while (iter != listens.end()) {
            ...
            if (machine) {
                out << "  " << machine->getName() << "[" << machine->getId()
                    << "]"
                    << ":   "
                    << (machine->enabled() ? machine->getCurrent(this).getName()
                                           : "DISABLED");
                if (machine->owner) {
                    out << " owner: "
                        << (machine->owner ? machine->owner->getName()
                                           : "null");
                }
                out << "\n";
            } else if (t) {
    ...
    if (state_machine && !state_machine->commands.empty()) {
        ...
        while (cmd_iter != state_machine->commands.end()) {
            const std::pair<std::string, MachineCommandTemplate *> &curr =
                *cmd_iter++;
    ...
    if (stable_states.size() ||
        state_machine->token_id == ClockworkToken::LIST) {
        ...
        for (unsigned int i = 0; i < stable_states.size(); ++i) {
            out << "  ";
            stable_states[i].displayName(out)
            ...
            if (stable_states[i].condition.last_result == true) {
                if (stable_states[i].subcondition_handlers &&
                    ...
                    while (iter !=
                           stable_states[i].subcondition_handlers->end()) {
                        ConditionHandler &ch = *iter++;
                        out << "      " << ch.command_name << " ";
                        if (ch.command_name != "FLAG" && ch.trigger) {
                            out << (ch.trigger->fired() ? "fired"
                                                        : "not fired");
                        }
...
bool checkDepend(std::set<Transmitter *> &checked, Transmitter *seek,
                 Transmitter *test) {
    if (seek == test) {
        return true;
    }
...
void MachineInstance::addDependancy(MachineInstance *m) {
    if (m && m != this && !depends.count(m)) {
        depends.insert(m);
        DBG_M_DEPENDANCIES << _name << " added dependant machine "
                           << m->getName() << "\n";
    }
}
...
void MachineInstance::removeDependancy(MachineInstance *m) {
    if (m && m != this && depends.count(m)) {
        ...
        depends.erase(found);
        DBG_M_DEPENDANCIES << _name << " removed dependant machine "
                           << m->getName() << "\n";
    }
}
...
bool MachineInstance::dependsOn(Transmitter *m) {
    ...
    return listens.find(mi) !=
           listens
               .end(); // TBD - shouldn't this be checking the dependancies, not listens?
}
...
void MachineInstance::idle() {
    ...
    if (!is_active) {
        if (false && !cache->reported_error) {
            char buf[100];
            snprintf(buf, 100,
                     "Error: %s::idle() called but this machine is not active",
                     _name.c_str());
            MessageLog::instance()->add(buf);
    ...
    while (!mail_queue.empty()) {
        ...
        {
            boost::mutex::scoped_lock lock(q_mutex);
            DBG_M_MESSAGING << _name << " has " << mail_queue.size()
                            << " messages waiting\n";
...
const Value *MachineInstance::getTimerVal() {
    MachineTimerValue *mtv =
        dynamic_cast<MachineTimerValue *>(state_timer.dynamicValue());
    mtv->operator()(this);
...
bool MachineInstance::processAll(std::set<MachineInstance *> &to_process,
                                 uint32_t max_time, PollType which) {
    ...
    //boost::recursive_mutex &mutex(SharedWorkSet::instance()->getMutex());
    {
        ...
        while (busy_it != to_process.end()) {
            ...
            if (mi->state_machine && mi->state_machine->plugin &&
                mi->state_machine->plugin->poll_actions) {
            ...
            if ((mi->state_machine && mi->state_machine->plugin) ||
                ...
                if (!mi->has_work && !mi->executingCommand()) {
                    ...
                    busy_it++;
                       ...
                       !mi->executingCommand()->getTrigger()) {
                ProcessingThread::activate(mi);
                busy_it++;
            } else {
                busy_it++;
...
void MachineInstance::addParameter(const Parameter &p, MachineInstance *mi,
                                   int position, bool before) {
...
MachineInstance &MachineInstance::operator=(const MachineInstance &orig) {
    ...
    std::copy(orig.parameters.begin(), orig.parameters.end(),
              std::back_inserter(parameters));
    std::copy(orig.locals.begin(), orig.locals.end(),
              std::back_inserter(locals));
    return *this;
}
...
bool MachineInstance::stateExists(State &seek) {
    ...
    if (state_machine->findState(seek)) {
        return true;
    }
...
bool MachineInstance::receives(const Message &m, Transmitter *from) {
    ...
    if (m.isSimple() || m.isEnable()) {
        if (from == this || (io_interface && from == io_interface)) {
            DBG_M_MESSAGING << "Machine " << getName() << " receiving " << m
                            << " from itself\n";
            return true;
        }
        // machines receive messages from objects they are listening to
...
Action::Status MachineInstance::setState(const char *sn, uint64_t authority,
                                         bool resume) {
    char buf[150];
    if (!state_machine) {
        snprintf(
            buf, 150,
            "Error: setting state to %s on a machine (%s) with no statemachine",
            sn, _name.c_str());
        MessageLog::instance()->add(buf);
        DBG_MSG << buf << "\n";
    ...
    if (!s) {
        snprintf(buf, 150, "Error: setting state to unknown value: '%s' on %s",
                 sn, _name.c_str());
        MessageLog::instance()->add(buf);
        DBG_MSG << buf << "\n";
        return Action::Failed;
...
Action::Status MachineInstance::setState(const State &new_state,
                                         uint64_t authority, bool resume) {
    if (expected_authority != 0 &&
        authority == 0) { //expected_authority != authority ) {
        if (isShadow()) {
    ...
    if (!machine_class_state) {
        ...
        if (!hasState(propertyState)) {
            char buf[150];
            snprintf(buf, 150, "Error: unknown state: '%s' on %s",
                     new_state.getName().c_str(), _name.c_str());
            MessageLog::instance()->add(buf);
            DBG_MSG << buf << "\n";
            return Action::Failed;
        }
...

    {
        ...
        if (enabled() && current_state != new_state &&
            current_state.getName() != "undefined") {
            ...
            if (receives(msg, this)) {
                ...
                if (stat == Action::Failed ||
                    (!isActive() && stat != Action::Complete)) {
                    char buf[200];
                    snprintf(
                        buf, 200, "%s %s leave action did not complete (%s)",
                        _name.c_str(), txt.c_str(), actionStatusName(stat));
                    MessageLog::instance()->add(buf);
                    NB_MSG << buf << "\n";
                }
            }
            ...
            while (dep_iter != depends.end()) {
                ...
                if (act) {
                    ...
                    DBG_M_MESSAGING << dep->getName() << "["
                                    << dep->getCurrent(this).getName() << "]"
                                    << " is executing " << *act << "\n";
                }
        ...
        if (published && (modbus_exported == ModbusExport::discrete ||
                          modbus_exported == ModbusExport::coil)) {
            if (!modbus_exports.count(_name)) {
                char buf[100];
                snprintf(buf, 100,
                         "%s Internal Error: modbus export info not found",
                         getName().c_str());
                MessageLog::instance()->add(buf);
                NB_MSG << buf << "\n";
            } else {
                ...
                if (ma.getSource() == ModbusAddress::machine &&
                    ...
                    DBG_M_MSG << _name << ": exported: " << modbus_exported
                              << " (" << ma
        ...
        DBG_STATECHANGES << fullName() << " changing from " << current_state
                         << " to " << new_state << "\n";
        ...
        bool should_notify =
            !next_state->isPrivate() &&
            (!current_state.isPrivate() ||
             (current_state.isPrivate() && visible_state != *next_state));
        ...
        properties.add("STATE", current_state.getName().c_str(),
        ...
        if (mq_interface && should_notify) {
            if (_type == "POINT" && properties.lookup("type") == "Output") {
                mq_interface->publish(properties.lookup("topic").asString(),
                                      new_state.getName(), this);
            }
        }
        if (published && !modbus_exports.empty() && should_notify) {
            ...
            if (modbus_exports.count(_name + "." + last)) {
                ...
                DBG_MODBUS << _name << " leaving state " << last
            ...
            if (modbus_exports.count(_name + "." + current_state.getName())) {
                ModbusAddress ma =
                    modbus_exports[_name + "." + current_state.getName()];
                DBG_MODBUS << _name << " active state "
        ...
        for (unsigned int ss_idx = 0; ss_idx < stable_states.size(); ++ss_idx) {
            StableState &s = stable_states[ss_idx];
            if (new_state.getName() == s.state_name &&
                s.timer_predicates.size()) {
                ...
                if (saved != earliestTimer) {
                    std::cout << _name << ":" << s.state_name
                              << " subcondition timer is earlier "
                              << earliestTimer << " vs " << saved << "\n";
                }
            }
        }
        ...
        if (earliestTimer != SymbolTable::Null) {
            ...
            if (earliestTimer.asInteger(timer_val)) {
                DBG_M_SCHEDULER << _name << " Scheduling timer for "
                                << timer_val << "ms\n";
                ...
                if (timer_val > 0) {
                    ...
                    Scheduler::instance()->add(
                        new ScheduledItem(timer_val * 1000, trigger));
                } else if (timer_val >= -2) {
                    ProcessingThread::activate(this);
                }
            }
...
void MachineInstance::notifyDependents(Message &msg) {
    ...
    while (dep_iter != depends.end()) {
        ...
        DBG_M_MESSAGING << _name << " should tell " << dep->getName()
                        << " it is " << current_state.getName() << " using "
                        << msg << "\n";
...
Action *MachineInstance::findMatchingCommand(const std::string &command_name) {
    ...
    std::multimap<std::string, MachineCommand *>::iterator cmd_it =
    ...
    DBG_M_ACTIONS << _name << " no command matching " << command_name
                  << " found\n";
    return 0;
}
...
Action *MachineInstance::findHandler(Message &m, Transmitter *from,
                                     bool response_required) {
    ...
    if (from) {
        DBG_M_MESSAGING << _name << " received message " << m << " from "
                        << from->getName() << "\n";
    }
    if (from == this || m.getText() == short_name) {
        DBG_M_MESSAGING << _name << " checking transitions ("
                        << transitions.size() << ")\n";
        std::list<Transition>::const_iterator iter = transitions.begin();
        while (iter != transitions.end()) {
            const Transition &t = *iter++;
            if (t.trigger.getText() == short_name &&
                ...
                if (num_commands) {
                    ...
                    for (unsigned int ss_idx = 0; ss_idx < stable_states.size();
                         ++ss_idx) {
                        StableState &s = stable_states[ss_idx];
                        if (s.state_name == t.dest.getName()) {
                            DBG_M_STATECHANGES
                                << "found stable state condition test for "
                                << t.dest.getName() << "\n";
                            ...
                            if (change_state_action == 0) {
                                ...
                                IfElseCommandActionTemplate ifecat(
                                    s.condition.predicate, &mc, &mc2);
                                change_state_action =
                                    new IfElseCommandAction(this, &ifecat);
                            } else {
                                // update the action to or with another condition
                                Predicate *p = new Predicate(
                                    change_state_action->condition.predicate,
                                    opOR,
                    ...
                    if (!found) {
                        DBG_M_STATECHANGES
                            << "no stable state condition test for "
                        ...
                        DBG_M_STATECHANGES
                            << _name << " pushed (status: " << msa->getStatus()
                            << ") " << *msa << "\n";
                        this->push(msa);
                    }
                    // if a modbus command, make sure it is turned off again in modbus
                    if (published && modbus_exports.count(
                                         _name + "." + t.trigger.getText())) {
                        ModbusAddress addr =
                            modbus_exports[_name + "." + t.trigger.getText()];
                        if (addr.getSource() == ModbusAddress::command) {
                            DBG_MODBUS << _name << " turning off command coil "
                                       << addr << "\n";
                            addr.update(this, 0);
                        } else {
                            NB_MSG << _name << " command "
                                   << t.trigger.getText()
                                   << " is linked to an improper address\n";
                        }
                    }
                    // find the correct command to use based on the current state
                    Action *matching_command =
                        findMatchingCommand(t.trigger.getText());
                    if (matching_command) {
                    ...
                    if (state_machine->name == "ETHERCAT_BUS") {
                        if (short_name == "activate" &&
                            (current_state.getName() == "CONFIG" ||
                             current_state.getName() == "CONNECTED")) {
                            ProcessingThread::instance()
                                ->machine.requestActivation(true);
                            ProcessingThread::activate(this);
                        } else if (short_name == "deactivate" &&
                                   current_state.getName() == "ACTIVE") {
                            ProcessingThread::instance()
                                ->machine.requestDeactivation(true);
                            ProcessingThread::activate(this);
                        }
                    } else
#endif
                        DBG_M_MESSAGING
                            << "No linked command for the transition, "
                ...
                if (state_change_ok) {
                    ...
                    SendMessageActionTemplate ssat(
                        "StateChangeFailureException", this);
                    return new SendMessageAction(this, ssat);
                }
            }
        }
        // no transition but this may still be a command
        DBG_M_MESSAGING << _name << " looking for a command with name "
                        << short_name << "\n";
        if (commands.count(short_name)) {
            ...
            if (matching_command) {
                ...
                DBG_M_MESSAGING << _name
                                << "found a command but it doesn't match state "
                                << current_state.getName() << "\n";
            }
        }
    } else {
        ...
        while (iter != transitions.end()) {
            const Transition &t = *iter++;
            if ((t.trigger.getText() == m.getText() ||
                 t.trigger.getText() == short_name) &&
    ...
    if (debug() && _type != "POINT" &&
        ...
        std::string message_str =
            _name + "." + current_state.getName() + "_enter";
        if (message_str != m.getText()) {
            DBG_M_MESSAGING
                << _name << ": No transition found to handle " << m << " from "
                << current_state.getName()
                << ". Searching receive_functions for a handler...\n";
        } else {
            DBG_M_MESSAGING << _name << ": Looking for ENTER method for "
                            << current_state.getName() << "\n";
        }
    }
...
void MachineInstance::prepareCompletionMessage(Transmitter *from,
                                               std::string message) {
    if (from) {
...
Action::Status MachineInstance::execute(const Message &m, Transmitter *from,
    ...
    if (!enabled()) {
        ...
        if (len > 5 && msg.substr(len - 5) == "_done") {
            ...
            if (from) {
                snprintf(buf, 120,
                         "%s failed to execute %s from %s while disabled",
    ...
    if (executingCommand()) {
        ...
        while (iter != active_actions.end()) {
            ...
            if (trigger && trigger->matches(m.getText())) {
                ...
                if (ssa) {
                    if (!ssa->getCondition().predicate ||
                        ssa->getCondition()(this)) {
                        trigger->fire();
                        DBG_M_MESSAGING << _name << " trigger on " << *a
                                        << " fired\n";
                    } else {
                        DBG_M_MESSAGING
                            << _name << " trigger message "
                            << trigger->getName() << " arrived but condition "
                    ...
                    if (!msa->getCondition().predicate ||
                        msa->getCondition()(this)) {
                        trigger->fire();
                        DBG_M_MESSAGING << _name << " trigger on " << *a
                                        << " fired\n";
                    } else {
    ...
    if ((io_interface && from == io_interface) ||
        (mq_interface && from == mq_interface)) {
        ...
        if (m.getText() == "on_enter" && current_state.getName() != "on") {
            ...
            if (on) {
                ...
                char buf[120];
                snprintf(buf, 120,
                         "%s does not have a state 'on' executing 'on_enter'",
                         _name.c_str());
                MessageLog::instance()->add(buf);
                return Action::Failed;
            }
        } else if (m.getText() == "off_enter" &&
                   current_state.getName() != "off") {
            ...
            if (off) {
                ...
                char buf[120];
                snprintf(buf, 120,
                         "%s does not have a state 'off' executing 'off_enter'",
                         _name.c_str());
                MessageLog::instance()->add(buf);
                return Action::Failed;
            }
    ...
    HandleMessageActionTemplate hmat(
        Package(from, this, new Message(event_name.c_str())));
...
void MachineInstance::handle(const Message &m, Transmitter *from,
                             bool send_receipt) {
    if (!enabled()) {
        DBG_M_MESSAGING << _name << ":" << id << " dropped: " << m << " in "
                        << getCurrent(this) << " from " << from->getName()
                        << " while disabled\n";
        if (send_receipt) {
    ...
    if (_type != "POINT") {
        DBG_M_MESSAGING << _name << ":" << id << " received: " << m << " in "
                        << getCurrent(this) << " from " << from->getName()
...
void MachineInstance::sendMessageToReceiver(Message *m, Receiver *r,
                                            bool expect_reply) {
    MachineShadowInstance *msi = dynamic_cast<MachineShadowInstance *>(r);
    if (msi) {
        DBG_MSG << _name << " sending message " << *m << " to shadow "
                << r->getName() << "\n";
        ...
        if (mi) {
            MessageHeader mh(MessageHeader::SOCK_CW, MessageHeader::SOCK_CW,
                             false);
            ...
            char buf[150];
            snprintf(buf, 150,
                     "Channel is unable to send to non machine instance");
            MessageLog::instance()->add(buf);
            DBG_MSG << buf << "\n";
        }
    } else {
        DBG_MESSAGING << _name << " sending message " << *m << " to "
                      << r->getName() << "\n";
        if (r->enabled() ||
            ...
            Package *p = new Package(
                this, this, new Message("DisabledMessageTargetException"),
                false);
            Dispatcher::instance()->deliver(p);
        }
    }
...
void MachineInstance::clearAllActions() {
    while (!active_actions.empty()) {
        ...
        if (a->getTrigger() && a->getTrigger()->enabled() &&
            !a->getTrigger()->fired()) {
            a->disableTrigger();
        }
        a->release();
...
void MachineInstance::setInitialState(bool resume) {
    ...
    if (state_machine) {
        if (io_interface) {
            const State *s =
                state_machine->findState(io_interface->getStateString());
            if (s) {
            ...
            if (state_machine->token_id == ClockworkToken::LIST) {
                ...
                setState(state_machine->initial_state, expected_authority,
                         resume);
            }
            setNeedsCheck();
        }
...
void sortDependentMachines(MachineInstance *m,
...
void MachineInstance::push(Action *new_action) {
    ...
    DBG_M_ACTIONS << _name << " ADDED to machines with work "
                  << SharedWorkSet::instance()->size() << "\n";
    return;
}
...
void MachineInstance::updateLastEvaluationTime() {
    ...
    if (idle_time) {
        ...
        if (state_machine && state_machine->polling_delay) {
            next_poll =
                last_state_evaluation_time + state_machine->polling_delay;
        } else {
            next_poll = last_state_evaluation_time +
                        MachineInstance::polling_delay->iValue;
        }
    }
}
...
Value MachineInstance::earliestScheduleTime(
    const std::list<Predicate *> &predicates) {
    Value schedule_time;
    ...
    if (num_timer_clauses) {
        ...
        while (pred_iter != predicates.end()) {
            ...
            Predicate *sub_p =
                (l && (l->entry.kind == Value::t_symbol ||
                       stringEndsWith(l->entry.sValue, ".TIMER")))
                    ? r
                    : l;
            ...
            if (source->entry.token_id ==
                ClockworkToken::TIMER) { // local timer test
                timer_machine = this;
            } else if (stringEndsWith(source->entry.sValue,
                                      ".TIMER")) { // local or remote timer test
...
bool MachineInstance::setStableState() {
    ...
    if (io_interface) {
        ...
        if (s) {
            ...
            snprintf(
                buf, 200,
                "Warning: %s does not have a state %s to match the hardware\n",
        ...
        fixListState(*this);
               ...
               (state_machine &&
                state_machine->token_id == ClockworkToken::REFERENCE)) {
        //DBG_MSG << _name << " has " << parameters.size() << " parameters\n";
        if (locals.size()) {
            const State *s = state_machine->findState("ASSIGNED");
            assert(s);
        ...
        for (unsigned int ss_idx = 0; ss_idx < stable_states.size(); ++ss_idx) {
            ...
            if (!found_match) {
                ...
                if (ss_condition_true) {
                    DBG_M_PREDICATES << _name << "." << s.state_name
                                     << " condition " << *s.condition.predicate
                                     << " returned true\n";
                    active_state = &s;
                    if (current_state.getName() != s.state_name) {
                        ...
                        if (tracing() && isTraceable()) {
                            resetTemporaryStringStream();
                            ss << current_state.getName() << "->"
                               << s.state_name << " " << *s.condition.predicate;
                            setValue("TRACE", Value(ss.str(), Value::t_string));
                        }
                        if (s.subcondition_handlers) {
                            std::list<ConditionHandler>::iterator iter =
        ...
        */
        ...
        if (schedule_time != SymbolTable::Null) {
            ...
            if (schedule_time.asInteger(t)) {
                ...
                Scheduler::instance()->add(
                    new ScheduledItem(t * 1000, trigger));
                trigger->release();
            }
        }
    }
...
void MachineInstance::resetStableStateFlags(bool changed_state,
                                            StableState *active_state) {
    for (unsigned int ss_idx = 0; ss_idx < stable_states.size(); ++ss_idx) {
        StableState &s = stable_states[ss_idx];
        ...
        if (s.subcondition_handlers && changed_state && active_state != &s) {
            ...
            while (iter != s.subcondition_handlers->end()) {
                ...
                if (ch.command_name == "FLAG") {
                    ...
                    if (flag) {
                        const State *off =
                            flag->state_machine->findState("off");
                        if (strcmp("off", flag->getCurrentStateString(this))) {
                            DBG_AUTOSTATES
                                << "turning flag off since the state "
...
void MachineInstance::setStateMachine(MachineClass *machine_class) {
    ...
    if (my_instance_type == MACHINE_INSTANCE &&
        machine_class->allow_auto_states &&
    ...
    {
        std::list<Predicate *>::const_iterator iter =
            machine_class->timer_clauses.begin();
    ...
    for (unsigned int i = 0; i < parameters.size(); ++i) {
        if (i < num_class_params) {
            if (parameters[i].val.kind == Value::t_symbol) {
                ...
                parameters[i].val =
                    state_machine->parameters[i].val.sValue.c_str();
                MachineInstance *m = lookup(parameters[i].real_name);
                DBG_M_INITIALISATION << _name << " is looking up "
                                     << parameters[i].real_name << " for param "
                                     << i << "\n";
                if (m) {
                ...
                DBG_M_MSG << "Parameter " << i << " (" << parameters[i].val
                          << ") with real name "
            ...
            // and set the default
            if (parameters[i].machine &&
                !state_machine->parameters[i].properties.empty()) {
                SymbolTableConstIterator st_iter =
                    state_machine->parameters[i].properties.begin();
                while (st_iter !=
                       state_machine->parameters[i].properties.end()) {
                    std::pair<std::string, Value> prop = *st_iter++;
                    if (!parameters[i].machine->properties.exists(
                            prop.first.c_str())) {
                        DBG_M_INITIALISATION << "copying default property "
                                             << prop.first << " on parameter "
                                             << i;
                        DBG_M_INITIALISATION
                            << " to object" << *(parameters[i].machine) << "\n";
                        parameters[i].machine->properties.add(
                            prop.first, prop.second, SymbolTable::NO_REPLACE);
                    } else {
                        DBG_M_INITIALISATION << "default property "
                                             << prop.first
                                             << " overridden by value ";
                        DBG_M_INITIALISATION
                            << parameters[i].machine->properties.lookup(
                                   prop.first.c_str())
                            << "\n";
                    }
                }
    ...
    BOOST_FOREACH (option, machine_class->options) {
        ...
        if (option.second.kind == Value::t_symbol) {
            ...
            if (v != SymbolTable::Null) {
                ...
                properties.add(option.first, option.second,
    ...
    properties.add("NAME", Value(_name.c_str(), Value::t_string),
                   SymbolTable::ST_REPLACE);
    if (locals.size() == 0) {
        BOOST_FOREACH (Parameter p, state_machine->locals) {
            ...
            if (!p.machine) {
                DBG_M_INITIALISATION
                    << "failed to clone. no instance defined for parameter "
                    << p.val.sValue << "\n";
            } else {
                newp.machine = MachineInstanceFactory::create(
                    p.val.sValue.c_str(), p.machine->_type.c_str());
                newp.machine->setProperties(p.machine->properties);
                newp.machine->setDefinitionLocation(
                    p.machine->definition_file.c_str(),
                ...
                if (c_iter == machine_classes.end())
                    DBG_M_INITIALISATION << "Warning: class "
                                         << newp.machine->_type
                                         << " not found\n";
                    ...
                    if (newsm->parameters.size() !=
                        ...
                        if (newsm->name == "LIST" ||
                            ((newsm->name == "POINT" ||
                             ...
                             newsm->parameters.size() >= 2 &&
                             newsm->parameters.size() <= 3) ||
                            (newsm->name == "COUNTERRATE" &&
                             (newsm->parameters.size() == 3 ||
                              newsm->parameters.size() == 1))) {
                        } else {
                            resetTemporaryStringStream();
                            ss << "## - Error: Machine " << newsm->name
                               << " requires " << newsm->parameters.size()
                               << " parameters but instance " << _name << "."
                    ...
                    if (p.machine->parameters.size()) {
                        std::copy(p.machine->parameters.begin(),
                                  p.machine->parameters.end(),
                                  back_inserter(newp.machine->parameters));
                        DBG_M_INITIALISATION
                            << "copied " << p.machine->parameters.size()
                            << " parameters. local has "
                            << p.machine->parameters.size() << "parameters\n";
                    }
                    if (p.machine->stable_states.size()) {
                        DBG_M_INITIALISATION
                            << " restoring stable states for " << newp.val
                            << "...before: "
                            << newp.machine->stable_states.size();
                        newp.machine->stable_states.clear();
                        BOOST_FOREACH (StableState &s,
                                       p.machine->stable_states) {
                            newp.machine->stable_states.push_back(s);
                            newp.machine
                                ->stable_states
                                    [newp.machine->stable_states.size() - 1]
                                .setOwner(this); //
                        }
                        std::copy(p.machine->stable_states.begin(),
                                  p.machine->stable_states.end(),
                                  back_inserter(newp.machine->stable_states));
                        DBG_M_INITIALISATION
                            << " after: " << newp.machine->stable_states.size()
                            << "\n";
                    }
                }
            }
    ...
    BOOST_FOREACH (Transition &transition, state_machine->transitions) {
        ...
        if (machine.find('.') != std::string::npos) {
            ...
            for (unsigned int i = 0; i < state_machine->parameters.size();
                 ++i) {
                if (parameters[i].val == machine) {
                    ...
                    transitions.push_back(Transition(transition.source,
                                                     transition.dest,
...
Trigger *MachineInstance::setupTrigger(const std::string &machine_name,
    ...
    trigger_name += suffix;
    DBG_M_MESSAGING << _name << " is waiting for message " << trigger_name
                    << " from " << machine_name << "\n";
...
const Value *MachineInstance::resolve(std::string property) {
    if (property.find('.') != std::string::npos) {
        ...
        if (state_machine->token_id == ClockworkToken::VARIABLE ||
            ...
            for (unsigned int i = 0; i < parameters.size(); ++i) {
                if (state_machine->parameters[i].val.kind == Value::t_symbol &&
                    ...
                    DBG_M_PREDICATES << _name << " found parameter " << i
                                     << " to resolve " << property << "\n";
        ...
        if (!state_machine) {
            ...
            if (m) {
                const Value *global_property_val =
                    &m->properties.lookup("VALUE");
                if (*global_property_val == SymbolTable::Null) {
                    DBG_M_PROPERTIES << _name
                                     << " using current state from global "
                                     << m->fullName() << "\n";
                    return &m->current_state_val;
                } else {
                    DBG_M_PROPERTIES << _name << " using value property "
                                     << m->getValue("VALUE") << " from global "
                                     << m->fullName() << "\n";
                ...
                resetTemporaryStringStream();
                ss << fullName()
                   << " failed to find the machine for global: " << property;
                DBG_PROPERTIES << ss.str() << "\n";
                error_messages.push_back(ss.str());
                MessageLog::instance()->add(ss.str().c_str());
                ++num_errors;
        ...
        if ((res = lookupState(property_val)) != &SymbolTable::Null) {
            ...
            if (*res == SymbolTable::Null) {
                ...
                DBG_M_PROPERTIES << "found property " << property
                                 << " (type: " << res->kind << ") "
                                 << res->asString() << "\n";
                return res;
            }
        ...
        if (m) {
            if (m->state_machine &&
                (m->state_machine->token_id == ClockworkToken::VARIABLE ||
                 m->state_machine->token_id == ClockworkToken::CONSTANT)) {
                return &m->properties.lookup("VALUE");
            } else if (m->getStateMachine()) {

                if (m->getStateMachine()->token_id ==
                        ClockworkToken::VARIABLE ||
                    m->getStateMachine()->token_id ==
                        ClockworkToken::CONSTANT) {
                    return &m->properties.lookup("VALUE");
                } else if (m->getStateMachine()->token_id ==
                               ClockworkToken::LIST ||
                           m->getStateMachine()->token_id ==
                               ClockworkToken::REFERENCE) {
    ...
    char buf[200];
    snprintf(buf, 200, "%s: no such property or machine: %s",
             fullName().c_str(), property.c_str());
    MessageLog::instance()->add(buf);
    NB_MSG << buf << "\n";
    return &SymbolTable::Null;
}
...
const Value *MachineInstance::getValuePtr(Value &property) {
    ...
    assert(property.kind == Value::t_symbol ||
           property.kind == Value::t_string);
...
const Value &MachineInstance::getValue(const std::string &property) {
    if (property.find('.') != std::string::npos) {
        ...
        if (other) {
            ...
            resetTemporaryStringStream();
            ss << _name << " getValue() could not find machine named " << name
        ...
        Value property_val(
            property); // use this to avoid string comparisions on property
                ...
                if (state_machine) {
                    if (!state_machine->properties.exists(property.c_str())) {
                        DBG_M_PROPERTIES
                            ...
                            << " found in class, looking in globals\n";
                        if (globals.exists(property.c_str())) {
                            DBG_PROPERTIES << _name << " found global for "
                                           << property << "\n";
                            return globals.lookup(property.c_str());
                        }
                    } else {
                        DBG_M_PROPERTIES << "using property " << property
                                         << "from class\n";
                        return state_machine->properties.lookup(
                            property.c_str());
                    }
                }
            } else {
                DBG_M_PROPERTIES << "found property " << property
                                 << " (type: " << x.kind << ") " << x.asString()
                                 << "\n";
                return x;
            }
        }
...
bool MachineInstance::hasState(const State &test) const {
    if (state_machine) {
        ...
        char buf[100];
        snprintf(buf, 100, "warning: %s does not have a state; %s",
                 fullName().c_str(), test.getName().c_str());
        MessageLog::instance()->add(buf);
    }
    return false;
}
...
void MachineInstance::sendModbusUpdate(const std::string &property_name,
                                       const Value &new_value) {
    if (false) {
        FileLogger fl(program_name);
        fl.f() << _name << " Sending modbus update " << property_name << " "
               << new_value << " (kind:" << new_value.kind << ")"
               << "\n";
    }
    ...
    case ModbusAddress::holding_register: {
        ...
        } else if (new_value.kind == Value::t_string ||
                   new_value.kind == Value::t_symbol) {
            long val;
            char *res;
...
bool MachineInstance::setValue(const std::string &property,
                               const Value &new_value, uint64_t authority) {

    if (property.length() == 0) {
        char buf[100];
        snprintf(buf, 100, "%s: attempt to set property with empty name",
                 _name.c_str());
        MessageLog::instance()->add(buf);
        DBG_MSG << buf << "\n";
        return false;
    ...
    if (property.find('.') != std::string::npos) {
        ...
        if (other) {
            if (prop.length()) {
                ...
                char buf[100];
                snprintf(buf, 100, "%s bad request to set property %s",
                         _name.c_str(), property.c_str());
                MessageLog::instance()->add(buf);
                DBG_MSG << buf << "\n";
                return false;
            }
        ...
        if (expected_authority != 0 &&
            ...
            if (isShadow()) {
                Channel *chn = ownerChannel();
                if (chn &&
                    chn->current_state != ChannelImplementation::DISCONNECTED) {
                    chn->sendPropertyChangeMessage(this, fullName(), property,
                                                   new_value, authority);
                    return true;
        ...
        Value property_val(
            property); // use this value in comparisons for performance
        ...
        if (property_val.token_id == ClockworkToken::POLLING_DELAY) {
            ...
            // TODO: bug? shouldn't we return here?
        ...
        if (global_var != state_machine->global_references.end()) {
            ...
            if (global_machine && global_machine->state_machine->token_id !=
                                      ClockworkToken::CONSTANT) {
                DBG_PROPERTIES << global_machine->getName()
                               << " setting property VALUE to " << new_value
                               << "\n";
                return global_machine->setValue("VALUE", new_value);
            }
            NB_MSG << "attempt to set a value on global constant " << property
                   << ". ignored\n";
            return false;
        }
        // try the current instance ofthe machine, then the machine class and finally the global symbols
        DBG_PROPERTIES << getName() << " setting property " << property
                       << " to " << new_value << "\n";
        ...
        if (prev_value == SymbolTable::Null &&
            ...
            if (global_machine) {
                if (global_machine->state_machine->token_id !=
                    ClockworkToken::CONSTANT) {
                    return global_machine->setValue("VALUE", new_value);
                }
                NB_MSG << "attempt to set a value on constant " << property
                       << ". ignored\n";
                return false;
            }
        }
        ...
        if ((new_value.kind == Value::t_integer ||
             new_value.kind != Value::t_float) &&
            state_machine && state_machine->plugin &&
            state_machine->plugin->filter) {
            int filtered_value =
                state_machine->plugin->filter(this, new_value.iValue);
            was_changed =
                (prev_value != new_value || (new_value != SymbolTable::Null &&
                                             prev_value == SymbolTable::Null));
            if (was_changed) {
                properties.add(property, filtered_value,
                               SymbolTable::ST_REPLACE);
            }
        } else {
            was_changed =
                (prev_value != new_value || prev_value.kind != new_value.kind ||
                 (new_value != SymbolTable::Null &&
                  prev_value == SymbolTable::Null));
            if (was_changed) {
        ...
        if (property_val.token_id == ClockworkToken::tokVALUE &&
            ...
            if (entry) {
                ...
                if (entry->ready() && io_value != new_value) {
                    DBG_MSG << "updating " << _name << " io: " << io_value
                            << " new: " << new_value << "\n";
                    ECInterface::instance()->queueInitialisationRequest(
                        entry, new_value);
                }
            }
        } else
        ...
        if (state_machine->token_id == ClockworkToken::MQTTPUBLISHER &&
            mq_interface && property == "message") {
            //std::string old_val(properties.lookup(property.c_str()).asString());
            mq_interface->publish(properties.lookup("topic").asString(),
                                  new_value.asString(), this);
        }
        ...
        }
        {
            ...
            if (receives_functions.count(changed_msg) &&
                !hasPending(changed_msg)) {
...
void MachineInstance::refreshModbus(cJSON *json_array) {
    ...
    while (iter != modbus_addresses.end()) {
        ...
        if ((int)info.getGroup() != group) {
            NB_MSG << full_name << " index (" << group << "," << addr << ")"
                   << (*iter).first << " should be " << (*iter).second << "\n";
        }
        ...
        case ModbusAddress::state:
            value = (_name + "." + current_state.getName() == (*iter).second)
                        ? 1
                        : 0;
            break;
        ...
        case ModbusAddress::property:
            if (properties.exists(short_name.c_str())) {
                ...
                if (mi &&
                    (mi->_type == "VARIABLE" || mi->_type == "CONSTANT")) {
        ...
        if (value.kind == Value::t_string || value.kind == Value::t_symbol) {
            cJSON_AddItemToArray(item,
                                 cJSON_CreateString(value.sValue.c_str()));
...
ModbusAddress MachineInstance::addModbusExport(
    std::string name, ModbusAddress::Group g, unsigned int n,
    ...
    if (ModbusAddress::preset_modbus_mapping.count(full_name) != 0) {
        ModbusAddressDetails info =
            ModbusAddress::preset_modbus_mapping[full_name];
        DBG_MODBUS << _name << " " << name
                   << " has predefined address: " << info.group << ":"
                   << std::setfill('0') << std::setw(5) << info.address << "\n";
        ModbusAddress addr(ModbusAddress::toGroup(info.group), info.address,
                           info.len, owner, src, full_name, kind);
        modbus_exports[name] = addr;
        return addr;
    } else {
        ModbusAddress addr =
            ModbusAddress::alloc(g, n, this, src, full_name, kind);
...
void MachineInstance::setupModbusPropertyExports(std::string property_name,
    ...
    switch (grp) {
    case ModbusAddress::discrete:
        addModbusExport(name, ModbusAddress::discrete, size, this, kind,
                        ModbusAddress::property, full_name + name);
        break;
    case ModbusAddress::coil:
        addModbusExport(name, ModbusAddress::coil, size, this, kind,
                        ModbusAddress::property, full_name + name);
        break;
    case ModbusAddress::input_register:
...
void MachineInstance::setupModbusInterface() {
    ...
    if (_type != "VARIABLE" &&
        _type !=
            "CONSTANT") { // export property refers to VALUE export type, not the machine
    ...
    if (_type != "VARIABLE" && _type != "CONSTANT") {
        ...
        if (self_coil) {
            modbus_address =
                addModbusExport(_name, ModbusAddress::coil, 1, this,
                                export_type, ModbusAddress::machine, full_name);
        } else if (self_discrete) {
            modbus_address =
                addModbusExport(_name, ModbusAddress::discrete, 1, this,
                                export_type, ModbusAddress::machine, full_name);
            ...
            modbus_address =
                addModbusExport(_name, ModbusAddress::input_register, len, this,
                                export_type, ModbusAddress::machine, full_name);
            ...
            addModbusExport(_name, ModbusAddress::holding_register, len, this,
                            export_type, ModbusAddress::machine, full_name);
    ...
    BOOST_FOREACH (std::string s, state_machine->state_exports) {
        ...
        addModbusExport(name, ModbusAddress::discrete, 1, this,
                        ModbusExport::discrete, ModbusAddress::state,
                        full_name + "." + s);
    ...
    BOOST_FOREACH (std::string s, state_machine->state_exports_rw) {
        std::string name = _name + "." + s;
        addModbusExport(name, ModbusAddress::coil, 1, this,
                        ModbusExport::discrete, ModbusAddress::state,
                        full_name + "." + s);
        //modbus_exports[name(name].setName(full_name+"."+s);
    }
    ...
    BOOST_FOREACH (std::string s, state_machine->command_exports) {
        std::string name = _name + "." + s;
        addModbusExport(name, ModbusAddress::coil, 1, this,
                        ModbusExport::discrete, ModbusAddress::command,
                        full_name + "." + s);
    ...
    if (_type == "VARIABLE" || _type == "CONSTANT") {
        ...
        case ModbusExport::coil:
            setupModbusPropertyExports(property_name, ModbusAddress::coil,
                                       ModbusExport::coil, 1);
            break;
        case ModbusExport::reg:
            setupModbusPropertyExports(property_name,
                                       ModbusAddress::input_register,
                                       ModbusExport::reg, 1);
            break;
        case ModbusExport::rw_reg:
            setupModbusPropertyExports(property_name,
                                       ModbusAddress::holding_register,
    ...
    BOOST_FOREACH (ModbusAddressTemplate mat, state_machine->exports) {
        ...
        setupModbusPropertyExports(mat.property_name, mat.grp, mat.exType,
                                   mat.size);
    }

    assert(modbus_addresses.size() == 0);
    std::map<std::string, ModbusAddress>::iterator iter =
        modbus_exports.begin();
    while (iter != modbus_exports.end()) {
        const ModbusAddress &ma = (*iter).second;
        int index = ((int)ma.getGroup() << 16) + ma.getAddress();
        if (modbus_addresses.count(index) != 0) {
            NB_MSG << _name << " " << (*iter).first << " address " << ma
                   << " already recorded as " << modbus_addresses[index]
                   << "\n";
        }
    ...
    if (modbus_addresses.size() != modbus_exports.size()) {
        NB_MSG << _name << " modbus export inconsistent. addresses: "
               << modbus_addresses.size() << " indexes "
               << modbus_exports.size() << "\n";
    }
    assert(modbus_addresses.size() == modbus_exports.size());
}

void MachineInstance::modbusUpdated(ModbusAddress &base_addr,
                                    unsigned int offset,
                                    const char *new_value) {
    std::string name = fullName();
    DBG_MODBUS << name << " modbusUpdated " << base_addr << " " << offset << " "
               << new_value << "\n";
    ...
    if (addr.getGroup() == ModbusAddress::holding_register) {
        ...
        DBG_MODBUS << _name << " set property " << property_name
                   << " via modbus index " << index << " (" << addr << ")\n";
        ...
        NB_MSG << name << " unexpected modbus group for write operation "
               << addr << "\n";
    }
}

void MachineInstance::modbusUpdated(ModbusAddress &base_addr,
                                    unsigned int offset, int new_value) {
    std::string name(fullName());
    DBG_MODBUS << name << " modbusUpdated " << base_addr << " " << offset << " "
               << new_value << "\n";
    int index = (base_addr.getGroup() << 16) + base_addr.getAddress() + offset;
    ...
    if (addr.getGroup() == ModbusAddress::coil ||
        addr.getGroup() == ModbusAddress::discrete) {
        ...
        if (addr.getSource() == ModbusAddress::machine) {
            ...
            DBG_MODBUS << "setting state of " << name
                       << " due to modbus command\n";
            if (new_value) {
            ...
            DBG_MODBUS << _name << " received update for out-of-range coil"
                       << offset << "\n";
        }
...
void MachineInstance::modbusUpdated(ModbusAddress &base_addr,
                                    unsigned int offset, float new_value) {
    std::string name(fullName());
    DBG_MODBUS << name << " modbusUpdated " << base_addr << " " << offset << " "
               << new_value << "\n";
    int index = (base_addr.getGroup() << 16) + base_addr.getAddress() + offset;
...
int MachineInstance::getModbusValue(ModbusAddress &addr, unsigned int offset,
                                    int len) {
    int pos = (addr.getGroup() << 16) + addr.getAddress() + offset;
    if (!modbus_addresses.count(pos)) {
        DBG_M_MODBUS << _name << " unknown modbus address offset requested "
                     << offset << "\n";
        return 0;
    }
    // self
    std::string devname = modbus_addresses[pos];
    DBG_M_MODBUS << _name << " found device " << devname << " matching address "
                 << addr.getGroup() << ":" << addr.getAddress() + offset
                 << "\n";
