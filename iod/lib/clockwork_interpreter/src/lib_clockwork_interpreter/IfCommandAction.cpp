...
IfCommandAction::IfCommandAction(MachineInstance *mi,
                                 IfCommandActionTemplate *t)
...
Action::Status IfCommandAction::run() {
    ...
    if (!condition(owner)) {
        DBG_M_ACTIONS << owner->getName() << " IF " << *condition.predicate
                      << " returned false"
                      << "\n";
        status = Complete;
        owner->stop(this);
...
IfElseCommandAction::IfElseCommandAction(MachineInstance *mi,
                                         IfElseCommandActionTemplate *t)
    : Action(mi), condition(t->condition) {
    command = dynamic_cast<MachineCommand *>(t->command->factory(mi));
