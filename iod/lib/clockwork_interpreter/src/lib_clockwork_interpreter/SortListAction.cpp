...
SortListActionTemplate::SortListActionTemplate(Value list_name, Value property,
    ...
    : list_machine_name(list_name.asString()),
      property_name(property.asString()), ascending(asc) {}
...
SortListAction::SortListAction(MachineInstance *m,
                               const SortListActionTemplate *dat, bool asc)
    : Action(m), list_machine_name(dat->list_machine_name),
      property_name(dat->property_name), list_machine(0), ascending(asc) {}

SortListAction::SortListAction() : list_machine(0), ascending(true) {}

std::ostream &SortListAction::operator<<(std::ostream &out) const {
    return out << "Sort List Action " << list_machine->getName()
...
class PropertyValueStringSorter {
  public:
    PropertyValueStringSorter(std::string property_name, bool asc = true)
        : property(property_name), ascending(asc) {}
    bool operator()(const Parameter &a, const Parameter &b) const {
        if (!a.machine) {
            return true;
        }
        ...
        if (ascending) {
            return a.machine->getValue(property) <
                   b.machine->getValue(property);
        } else {
            return b.machine->getValue(property) <
                   a.machine->getValue(property);
        }
    }
...
class PropertyValueNumericSorter {
  public:
    PropertyValueNumericSorter(std::string property_name, bool asc = true)
        : property(property_name), ascending(asc) {}
    bool operator()(const Parameter &a, const Parameter &b) const {
        if (!a.machine) {
            return true;
        ...
        if (!b.machine->getValue(property).asInteger(b_val)) {
            return false;
        }
        if (ascending) {
            return a_val < b_val;
    ...
    if (list_machine && list_machine->_type == "LIST") {
        ...
        if (list_machine->parameters.empty()) {
            ...
            for (unsigned int i = 0; i < list_machine->parameters.size(); ++i) {
                ...
                if (entry &&
                    !entry->getValue(property_name.asString()).asInteger(val)) {
                    use_integer_sort = false;
                    break;
                }
            }
            if (use_integer_sort) {
                PropertyValueNumericSorter property_sort(
                    property_name.asString(), ascending);
                std::sort(list_machine->parameters.begin(),
                          list_machine->parameters.end(), property_sort);
            } else {
                PropertyValueStringSorter property_sort(
                    property_name.asString(), ascending);
                std::sort(list_machine->parameters.begin(),
                          list_machine->parameters.end(), property_sort);
            }
            ...
            for (unsigned int i = 0; i < list_machine->parameters.size(); ++i) {
                if (list_machine->parameters.at(i).val.kind !=
                    Value::t_integer) {
                    use_integer_sort = false;
                    break;
                }
            ...
            if (use_integer_sort) {
                ...
                std::sort(list_machine->parameters.begin(),
                          list_machine->parameters.end(), numeric_sort);
            } else {
                StringValueSorter string_sort(ascending);
                std::sort(list_machine->parameters.begin(),
                          list_machine->parameters.end(), string_sort);
            }
        }
