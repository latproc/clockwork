...
class ChannelImplementation {
  ...
  protected:
    ...
    uint64_t
        last_modified; // if the modified time > check time, a full check will be used
...
class ChannelMessage {
  public:
    ...
    bool done;
    ChannelMessage(int msg_id, const char *msg)
        : message_id(msg_id), text(msg), done(false) {}
...
class Channel : public MachineInstance, public ChannelImplementation {
  public:
    ...
    void requestStateChange(MachineInstance *machine, std::string new_state,
                            uint64_t authority);

    const std::string &getName() const { return name; }
    const std::set<MachineInstance *> &channelMachines() const {
        return channel_machines;
    }
    ...
    static int uniquePort(unsigned int range_start = 7600,
                          unsigned int range_end = 7799);
    static void sendPropertyChange(MachineInstance *machine, const Value &key,
                                   const Value &val, uint64_t authority = 0);
    static void sendStateChange(MachineInstance *machine, std::string new_state,
                                uint64_t authority);
    static void sendModbusUpdate(MachineInstance *machine,
                                 const std::string &property_name,
                                 const Value &new_value);
    static void sendCommand(MachineInstance *machine, std::string cmd,
    ...
    void startServer(
        ProtocolType proto =
            eZMQ); // used by shared (publish/subscribe) and one-to-one channels
    void startClient(); // used by shared (publish/subscribe) channels
    void startSubscriber();
    void stopSubscriber();
    void stopServer();
    bool
    isClient(); // does this channel connect to another instance of clockwork?
    ...
    void checkStateChange(
        std::string
            event); // change state after receiving a done from current state
    ...
    bool sendMessage(const char *msg, zmq::socket_t &sock,
    ...
    void sendPropertyChangeMessage(MachineInstance *m, const std::string &name,
  ...
  private:
    ...
    bool receivedStateChangeMessage(zmq::socket_t &remote_sock, char *&data,
                                    size_t &len);
