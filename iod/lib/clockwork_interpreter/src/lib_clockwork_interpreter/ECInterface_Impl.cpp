...
static int slaves_not_operational =
    1; // initialise to nonzero until we know for sure
...
ECModule::ECModule()
    : pdo_entries(0), pdos(0), syncs(0), num_entries(0), entry_details(0) {
...
SDOEntry::SDOEntry(std::string nam, uint16_t index, uint8_t subindex,
    ...
    : name(nam), module_(0), index_(index), subindex_(subindex),
      offset_(offset), data_(0), size_(size), realtime_request(0),
      sync_done(false), error_count(0), op(READ), machine_instance(0) {
    if (data && size != 0) {
...
void SDOEntry::success() {
    ...
    if (op == WRITE) {
        op =
            READ; // value has been successfully written, switch back to reading
    }
}
...
void SDOEntry::resolveSDOModules() {
    ...
    while (iter != new_sdo_entries.end()) {
        ...
        if (entry->getModule()) {
            ...
            std::cout << "Module already linked to SDO entry "
                      << entry->getName() << "\n";
        ...
        std::cout << "Attempting to prepare SDO entry: " << entry->getName()
                  << "\n";
        MachineInstance *mi =
            MachineInstance::find(entry->getModuleName().c_str());
        if (mi) {
            int module_position = mi->properties.lookup("position").iValue;
            ECModule *module = 0;
            if (module_position >= 0) {
                module = dynamic_cast<ECModule *>(
                    ECInterface::findModule(module_position));
            }
            if (module && entry->prepareRequest(module)) {
                ...
                if (entry->machineInstance() &&
                    ...
                    std::cout << "setting default value of " << entry->getName()
                              << " to " << val << "\n";
                ...
                std::cout << "Warning: failed to prepare SDO entry: "
                          << entry->getName() << "\n";
                iter++;
            }
        } else {
...
ECInterface::ECInterface()
    : initialised(0), process_data(0), process_mask(0), update_data(0),
      update_mask(0), reference_time(0),
#ifndef EC_SIMULATOR
#ifdef USE_SDO
...
bool ECModule::ecrtMasterSlaveConfig(ec_master_t *master) {
    ...
    if (master) {
        slave_config = ecrt_master_slave_config(master, alias, position,
...
bool ECModule::ecrtSlaveConfigPdos() {
    ...
    if (res) {
        std::cerr << "Error: " << res << " attempting to configure slave '"
                  << name << "'\n";
        assert(false);
    }
    return true;
...
std::ostream &ECModule::operator<<(std::ostream &out) const {
    out << "Slave " << name << ": " << alias << ", " << position << ", "
        << std::hex << vendor_id << ", " << product_code << std::dec << "\n";
    for (unsigned int i = 0; i < sync_count; ++i) {
        out << "  SM" << i << ": " << (int)syncs[i].index << ", "
            << syncs[i].dir << ", " << syncs[i].n_pdos << "\n";
        for (unsigned int j = 0; j < syncs[i].n_pdos; ++j) {
            ...
            for (unsigned int k = 0; k < syncs[i].pdos[j].n_entries; ++k) {
                ...
                out << "       " << k << ", " << std::hex << e->index
                    << std::dec << ", " << (int)e->subindex << ", "
                    << (int)e->bit_length << "\n";
            }
        }
    }
...
ec_sdo_request_t *SDOEntry::prepareRequest(ECModule *module) {
    ...
    ec_slave_config_t *x =
        ecrt_master_slave_config(ECInterface::master, 0, module->position,
    ...
    std::cerr << "Creating SDO request " << module->getName() << " 0x"
              << std::hex << index_ << ":" << subindex_ << std::dec << " ("
              << sz << ")"
              << "\n";
    realtime_request =
        ecrt_slave_config_create_sdo_request(x, index_, subindex_, sz);
...
void SDOEntry::syncValue() {
    if (size_ == 32) {
        ...
        if (machine_instance) {
            machine_instance->setValue(
                "VALUE",
                EC_READ_BIT(ecrt_sdo_request_data(realtime_request), offset_));
        }
    }
}
...
void ECInterface::checkSDOUpdates() {
    ...
    if (current_update_entry != sdo_update_entries.end()) {
        ...
        case EC_REQUEST_ERROR:
            std::cerr << "Failed to read SDO!" << std::hex << "0x"
                      << entry->getIndex() << ":" << (int)entry->getSubindex()
                      << std::dec << "\n";
            entry->failure();
...
bool ECInterface::
    checkSDOInitialisation() // returns true when no more initialisation is required
{
    ...
    if (current_init_entry != initialisation_entries.end()) {
        ...
        case EC_REQUEST_SUCCESS:
            ...
            // prepare to get the next entry
            current_init_entry =
                initialisation_entries.erase(current_init_entry);
            sdo_entry_state = e_None;
            break;
        case EC_REQUEST_ERROR:
            if (entry->operation() == SDOEntry::READ) {
            ...
            for (unsigned int j = 0; j < m->syncs[i].n_pdos; ++j) {
                for (unsigned int k = 0; k < m->syncs[i].pdos[j].n_entries;
                    ...
                    if (res < 0) {
                        ...
                        std::cerr
                            << "Successfully added item " << module_offset_idx
                            << " at index " << std::hex << "0x"
                            << m->syncs[i].pdos[j].index << " " << std::dec
                            << " subindex "
                            << (int)m->syncs[i].pdos[j].entries[k].subindex
                            << " length "
                            << (int)m->syncs[i].pdos[j].entries[k].bit_length
                            << " offset: " << res << " bitpos: "
                            << m->bit_positions[module_offset_idx] << " "
    ...
    for (unsigned int mi = 0; mi < modules.size(); ++mi) {
        ...
        */
        ...
        for (unsigned int i = 0; i < m->sync_count; ++i) {
            ...
            res = ecrt_slave_config_sync_manager(
                m->slave_config, m->syncs[i].index, m->syncs[i].dir,
                m->syncs[i].watchdog_mode);
            if (res < 0) {
                char buf[100];
                snprintf(buf, 100,
            ...
            if (m->syncs[i].n_pdos && m->syncs[i].pdos) {
                ecrt_slave_config_pdo_assign_clear(m->slave_config,
                                                   m->syncs[i].index);
            }
            ...
            for (unsigned int j = 0; j < m->syncs[i].n_pdos; ++j) {
                ...
                int res = ecrt_slave_config_pdo_assign_add(
                    m->slave_config, m->syncs[i].index,
                    m->syncs[i].pdos[j].index);
                if (res < 0) {
                    std::cerr << "Error: " << res
                              << " appending pdo assignment "
                              << " to sm " << i << " pdo: " << std::hex << "0x"
                              << m->syncs[i].pdos[j].index << std::dec << "\n";
                } else {
                    std::cerr << "**** added pdo assignment "
                              << " to sm " << i << " pdo: " << std::hex << "0x"
                              << m->syncs[i].pdos[j].index << std::dec
                              << " adding " << m->syncs[i].pdos[j].n_entries
                              << " entries\n"
                              << "\n";
                ...
                for (unsigned int k = 0; k < m->syncs[i].pdos[j].n_entries;
                    ...
                    if (res < 0) {
                        std::cerr
                            << "Error: " << res << " adding pdo entry mapping "
                            << std::hex << "0x"
                            << m->syncs[i].pdos[j].entries[k].index << " "
                            << std::dec
                            << m->syncs[i].pdos[j].entries[k].subindex << " "
                        ...
                        std::cerr
                            << "Successfully added entry item "
                            << module_offset_idx << " at index " << std::hex
                            ...
                            << (int)m->syncs[i].pdos[j].entries[k].bit_length
                            << " offset: " << res << " bitpos: "
                            << m->bit_positions[module_offset_idx] << std::dec
                            << " " << m->entry_details[module_offset_idx].name
                            << "\n";
                    }
#if 0
    ...
    if (module) {
        ...
        while (iter != modules.end()) {
            ...
            if (m->alias == module->alias && m->position == module->position) {
                std::cout << "module not added; another exists at "
                          << module->position << "\n";
                return false;
            }
        }
...
bool ECInterface::deactivate() {
    ...
    setProcessData(0);
    {
        boost::recursive_mutex::scoped_lock lock(modules_mutex);
        snprintf(buf, 200,
                 "EtherCAT interface: removing ethercat modules instances");
        MessageLog::instance()->add(buf);
        std::cout << buf << "\n";
        std::vector<ECModule *>::iterator iter = modules.begin();
        while (iter != modules.end()) {
            ECModule *m = *iter++;
            snprintf(buf, 200, "EtherCAT interface: deleting module %s",
                     m->name.c_str());
            MessageLog::instance()->add(buf);
            std::cout << buf << "\n";
            delete m;
    ...
    if (!domain1) {
        ...
        snprintf(
            buf, 200,
            "EtherCAT interface: domain1 successfully created with size %ld",
...
bool ECInterface::activate() {
    ...
    while (res >= 0 && pos < master_info.slave_count) {
        ECModule *module =
            dynamic_cast<ECModule *>(ECInterface::findModule(pos));
        if (module) {
    ...
    if ((res = ecrt_master_activate(master))) {
        snprintf(buf, 200,
                 "EtherCAT interface: Activating master failed with code: %d",
                 res);
        MessageLog::instance()->add(buf);
        std::cout << buf << "\n";
        return false;
...
#else
IOModule *ECInterface::findModule(unsigned int pos) { return 0; }
#endif
...
int ECInterface::collectState() {
    ...
    for (unsigned int i = 0; i < domain_size; ++i) {
        ...
        if (!last_pd) { // first time through, copy all the domain data and mask
            ...
            affected_bits++;
#if VERBOSE_DEBUG
            std::cout << "init update data from process byte " << i << ": "
                      << std::hex << (int)domain1_pd[i] << std::dec << "\n";
#endif
...
#if VERBOSE_DEBUG
    std::cout << "copied new domain data: ";
    display(pd, domain_size);
    std::cout << "\n";
#endif
...
void ECInterface::sendUpdates() {
    ...
    if (!master || !initialised || !active || !all_ok || !domain1) {
        if (now.tv_sec + 5 < last_warning) {
            ...
            char buf[100];
            snprintf(buf, 100,
                     "EtherCAT master is not ready to send updates\n");
            MessageLog::instance()->add(buf);
            std::cout << buf << "\n";
        }
        return;
...
void ECInterface::check_master_state(void) {
    ...
    if (ms.slaves_responding != master_state.slaves_responding) {
        ...
        if (ms.slaves_responding >= expected_slaves) {
            ...
            snprintf(buf, 100, "Number of slaves %d less than expected %d",
    ...
    if (ms.al_states != master_state.al_states) {
        ...
        std::cout << "AL states: 0x" << std::ios::hex << ms.al_states
                  << std::ios::dec << "\n";
        char buf[100];
        snprintf(buf, 100, "EtherCAT state change: was 0x%04x now 0x%04x",
                 master_state.al_states, ms.al_states);
        MessageLog::instance()->add(buf);
        std::cout << buf << "\n";
    }
    if (master_was_running && ms.al_states != 0x8) {
        if (failure_tolerance && all_ok &&
            ++failure_count > *failure_tolerance) {
            all_ok = false;
#if 0
            if (ethercat_status) {
                SetStateActionTemplate ssat = SetStateActionTemplate("SELF", "ERROR");
...
void ECInterface::check_slave_config_states(void) {
    ...
    while (iter != modules.end()) {
        ...
        if (res == 0) {
            char buf[200];
            snprintf(buf, 200,
                     "Slave %d (%s) reported error: %0x%0x%0x%0x %0x%0x%0x%0x",
                     i, m->name.c_str(), errbuf[0], errbuf[1], errbuf[2],
                     errbuf[3], errbuf[4], errbuf[5], errbuf[6], errbuf[7]);
            MessageLog::instance()->add(buf);
        }
        ...
        if (s.al_state != m->slave_config_state.al_state) {
            //MEMCHECK();
            std::cout << "ecat_thread: " << m->name << ": State 0x"
                      << std::ios::hex << s.al_state << ".\n";
            snprintf(
                buf, BUFSIZE, "Slave %d (%s) changed state was 02x%x now 02x%x",
                i, m->name.c_str(), m->slave_config_state.al_state, s.al_state);
            MessageLog::instance()->add(buf);
            //MEMCHECK();
        }
        if (s.online != m->slave_config_state.online) {
            ...
            snprintf(buf, BUFSIZE,
                     "Slave %d (%s) changed online state: was %s, now %s", i,
                     m->name.c_str(),
                     m->slave_config_state.online ? "online" : "offline",
                     s.online ? "online" : "offline");
            MessageLog::instance()->add(buf);
            //MEMCHECK();
        }
...
cJSON *generateSlaveCStruct(MasterDevice &m, const ec_ioctl_slave_t &slave,
                            bool reconfigure) {
    ...
    if (slave.sync_count) {
        ...
        const int c_entries_size =
            sizeof(ec_pdo_entry_info_t) * estimated_max_entries;
        ...
        for (i = 0; i < slave.sync_count; i++) {
            ...
            c_syncs[i].dir = EC_READ_BIT(&sync.control_register, 2)
                                 ? EC_DIR_OUTPUT
                                 : EC_DIR_INPUT;
            ...
            c_syncs[i].watchdog_mode = EC_READ_BIT(&sync.control_register, 6)
                                           ? EC_WD_ENABLE
                                           : EC_WD_DISABLE;
                ...
                for (j = 0; j < sync.pdo_count; j++) {
                    ...
                    if (pdo.entry_count) {
                        ...
                        for (k = 0; k < pdo.entry_count; k++) {
                            ...
                            cJSON_AddNumberToObject(json_entry, "pos",
                                                    entry_pos);
                            cJSON_AddNumberToObject(json_entry, "index",
                                                    entry.index);
                            cJSON_AddStringToObject(
                                json_entry, "name",
                                c_entry_details[entry_pos].name.c_str());
                            cJSON_AddNumberToObject(json_entry, "subindex",
                                                    entry.subindex);
                            cJSON_AddNumberToObject(json_entry, "bit_length",
                                                    entry.bit_length);
                            ++entry_pos;

                            cJSON_AddItemToArray(json_entries, json_entry);
                        }
                        cJSON_AddItemToObject(json_pdo, "entries",
                                              json_entries);
                    }
                    //std::cout << "\n";
...
char *collectSlaveConfig(bool reconfigure) {
    ...
    for (unsigned int i = 0; i < master.slave_count; i++) {
        ...
        if (!module) {
            ...
            std::cout << "Skipped scanning of module at position " << i
                      << " already loaded\n";
        }
    }
#endif
...
bool IODCommandMasterInfo::run(std::vector<Value> &params) {
    ...
    const ec_master_state_t *master_state =
        ECInterface::instance()->getMasterState();
    cJSON *root = cJSON_CreateObject();
    cJSON_AddNumberToObject(root, "slave_count",
