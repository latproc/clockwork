...
MachineShadowInstance::MachineShadowInstance(CStringHolder name,
                                             const char *type,
...
Action::Status MachineShadowInstance::setState(const State &new_state,
                                               uint64_t authority,
                                               bool resume) {
    DBG_CHANNELS << _name << " (shadow) setState(" << current_state << "->"
                 << new_state << ")\n";
    return MachineInstance::setState(new_state, authority, resume);
}

Action::Status MachineShadowInstance::setState(const char *new_state,
                                               uint64_t authority,
                                               bool resume) {
